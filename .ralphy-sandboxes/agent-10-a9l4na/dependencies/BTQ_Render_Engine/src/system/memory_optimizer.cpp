...
void MemoryOptimizer::enable_tracking(bool enabled) { impl_->tracking_enabled = enabled; }
...
void MemoryOptimizer::clear() {
  ...
  impl_->total_deallocated = 0;
  impl_->current_allocated = 0;
  impl_->peak_usage = 0;
  impl_->fragmentation = 0;
  impl_->fragmentation_ratio = 0.0f;

  // Clear recent allocations history
  impl_->recent_allocations.clear();

  // Clear active allocations map
  impl_->active_allocations.clear();

  // Clear memory pools (but preserve the pools themselves if they're in use)
  // For safety, we'll clear the tracking but not deallocate the actual memory pools
  for (auto& pool : impl_->memory_pools) {
    pool.used = 0;
    pool.is_compacted = true;
  }
...
std::vector<MemoryOptimizer::AllocationRecord> MemoryOptimizer::get_recent_allocations(
    size_t count) const {
  std::lock_guard<std::mutex> lock(impl_->allocation_mutex);
