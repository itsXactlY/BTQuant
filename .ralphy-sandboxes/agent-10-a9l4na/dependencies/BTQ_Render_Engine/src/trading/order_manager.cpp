...
void OrderManager::add_execution(const OrderExecution &execution) {
  ...
  if (order_it != orders_.end()) {
    ...
    if (order.filled_quantity >= order.quantity) {
      ...
      order.average_fill_price = (order.average_fill_price * (order.filled_quantity - execution.quantity) +
...
std::vector<OrderManager::OrderExecution> OrderManager::get_executions(const std::string &order_id) const {
...
void OrderManager::process_iceberg_order(const Order &order) {
  ...
  double visible_quantity = order.iceberg_visible_quantity > 0
                                ? order.iceberg_visible_quantity
                                : order.quantity / 10.0;
