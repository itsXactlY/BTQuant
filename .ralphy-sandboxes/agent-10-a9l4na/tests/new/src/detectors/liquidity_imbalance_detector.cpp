...
LiquidityImbalanceDetector::LiquidityImbalanceDetector(HotSpineExtendedReader& reader)
    : reader_(reader) {}

double LiquidityImbalanceDetector::calculate_orderbook_depth(
...
double LiquidityImbalanceDetector::calculate_bid_ask_imbalance(const OrderbookData& ob) {
    double mid = ob.mid_price();
    if (mid == 0.0) return 0.0;
...
std::optional<LiquiditySignal> LiquidityImbalanceDetector::detect(const std::string& symbol) {
    ...
    auto min_it = std::min_element(depths.begin(), depths.end(),
    ...
    if (depth_ratio > depth_ratio_threshold_) {
        ...
        if (imbalance > imbalance_threshold_) {
            signal.direction = ImbalanceDirection::SHORT;  // Heavy bids = potential dump
        } else if (imbalance < -imbalance_threshold_) {
            signal.direction = ImbalanceDirection::LONG;   // Heavy asks = potential pump
        } else {
...
std::string LiquiditySignal::to_string() const {
    ...
    oss << "LiquidityImbalance(symbol=" << symbol
        << ", thin=" << thin_exchange << "(depth=" << thin_depth << ")"
        << ", thick=" << thick_exchange << "(depth=" << thick_depth << ")"
        << ", ratio=" << depth_ratio << "x"
        << ", direction=";
    
    switch (direction) {
        case ImbalanceDirection::LONG: oss << "LONG"; break;
        case ImbalanceDirection::SHORT: oss << "SHORT"; break;
        case ImbalanceDirection::NEUTRAL: oss << "NEUTRAL"; break;
    }
