...
WhaleFrontRunDetector::WhaleFrontRunDetector(HotSpineExtendedReader& reader)
    : reader_(reader),
      whale_threshold_usd_(100000.0),
      min_lagging_exchanges_(2),
      reaction_threshold_bps_(50.0),
      reaction_window_us_(1000000),
      detections_(0) {}
...
void WhaleFrontRunDetector::PriceHistory::add(uint64_t timestamp_us, double price) {
...
double WhaleFrontRunDetector::PriceHistory::get_volatility_bps(uint64_t lookback_us) {
...
std::optional<WhaleSignal> WhaleFrontRunDetector::detect(const std::string& symbol) {
    ...
    for (const auto& exchange : exchanges) {
        ...
        for (const auto& trade : recent_trades) {
            ...
            if (now - trade.ts_local > 5'000'000) continue;
            ...
            if (trade_size_usd >= whale_threshold_usd_) {
                ...
                for (const auto& other_exchange : exchanges) {
                    ...
                    if (!other_price) continue;
                    
                    double price_diff_bps = std::abs((*other_price - trade.price) / trade.price) * 10000.0;
...
std::string WhaleSignal::to_string() const {
    ...
    oss << "WhaleDetected(symbol=" << symbol
