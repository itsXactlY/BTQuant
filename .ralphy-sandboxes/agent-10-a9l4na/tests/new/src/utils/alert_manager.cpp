...
std::string AlertUtils::type_to_string(AlertType type) {
    switch (type) {
        case AlertType::SHM_FILE_DELETED: return "SHM_FILE_DELETED";
        case AlertType::SHM_FILE_CORRUPTED: return "SHM_FILE_CORRUPTED";
        case AlertType::SHM_PERMISSION_DENIED: return "SHM_PERMISSION_DENIED";
        case AlertType::SHM_ATTACH_FAILED: return "SHM_ATTACH_FAILED";
        case AlertType::SHM_HEADER_INVALID: return "SHM_HEADER_INVALID";
        case AlertType::DETECTOR_INIT_FAILED: return "DETECTOR_INIT_FAILED";
        case AlertType::DETECTOR_EXECUTION_ERROR: return "DETECTOR_EXECUTION_ERROR";
        case AlertType::DETECTOR_TIMEOUT: return "DETECTOR_TIMEOUT";
        case AlertType::DATA_STREAM_INTERRUPTED: return "DATA_STREAM_INTERRUPTED";
        case AlertType::DATA_LOSS_DETECTED: return "DATA_LOSS_DETECTED";
        case AlertType::BUFFER_OVERFLOW: return "BUFFER_OVERFLOW";
        case AlertType::SYSTEM_DEGRADED: return "SYSTEM_DEGRADED";
        case AlertType::SYSTEM_RECOVERY: return "SYSTEM_RECOVERY";
        case AlertType::SYSTEM_SHUTDOWN: return "SYSTEM_SHUTDOWN";
...
AlertType AlertUtils::type_from_string(const std::string& str) {
    if (str == "SHM_FILE_DELETED") return AlertType::SHM_FILE_DELETED;
    if (str == "SHM_FILE_CORRUPTED") return AlertType::SHM_FILE_CORRUPTED;
    if (str == "SHM_PERMISSION_DENIED") return AlertType::SHM_PERMISSION_DENIED;
    if (str == "SHM_ATTACH_FAILED") return AlertType::SHM_ATTACH_FAILED;
    if (str == "SHM_HEADER_INVALID") return AlertType::SHM_HEADER_INVALID;
    if (str == "DETECTOR_INIT_FAILED") return AlertType::DETECTOR_INIT_FAILED;
    if (str == "DETECTOR_EXECUTION_ERROR") return AlertType::DETECTOR_EXECUTION_ERROR;
    if (str == "DETECTOR_TIMEOUT") return AlertType::DETECTOR_TIMEOUT;
    if (str == "DATA_STREAM_INTERRUPTED") return AlertType::DATA_STREAM_INTERRUPTED;
    if (str == "DATA_LOSS_DETECTED") return AlertType::DATA_LOSS_DETECTED;
    if (str == "BUFFER_OVERFLOW") return AlertType::BUFFER_OVERFLOW;
    if (str == "SYSTEM_DEGRADED") return AlertType::SYSTEM_DEGRADED;
    if (str == "SYSTEM_RECOVERY") return AlertType::SYSTEM_RECOVERY;
    if (str == "SYSTEM_SHUTDOWN") return AlertType::SYSTEM_SHUTDOWN;
    return AlertType::CUSTOM;
}

Alert AlertUtils::create_shm_deleted_alert(const std::string& path) {
    return Alert(
        AlertType::SHM_FILE_DELETED,
        AlertSeverity::CRITICAL,
        "Shared memory file has been deleted: " + path,
        "SHM-MONITOR"
    );
}
...
Alert AlertUtils::create_shm_corrupted_alert(const std::string& reason) {
    return Alert(
        AlertType::SHM_FILE_CORRUPTED,
        AlertSeverity::CRITICAL,
        "Shared memory corruption detected: " + reason,
        "SHM-MONITOR"
    );
}

Alert AlertUtils::create_detector_error_alert(const std::string& detector, const std::string& error) {
    return Alert(
        AlertType::DETECTOR_EXECUTION_ERROR,
        AlertSeverity::ERROR,
        "Detector error in " + detector + ": " + error,
        detector
    );
}
...
Alert AlertUtils::create_recovery_failed_alert(const std::string& action, const std::string& reason) {
    return Alert(
        AlertType::SYSTEM_DEGRADED,
        AlertSeverity::ERROR,
        "Recovery failed for " + action + ": " + reason,
        "RECOVERY"
    );
}
...
void ConsoleAlertSink::send(const Alert& alert) {
    if (alert.severity < min_level_) return;
...
void FileAlertSink::send(const Alert& alert) {
    ...
    fprintf(file_, "[%s.%06ld][%s][%s][%s] %s\n",
            ...
            alert.severity_string().c_str(),
            alert.type_string().c_str(),
            alert.component.c_str(),
            alert.message.c_str());
...
bool AlertManager::initialize() {
    ...
    auto console_sink = std::make_shared<ConsoleAlertSink>(AlertSeverity::WARNING);
    register_sink(console_sink);
    
    Logging::DynamicLogger::instance().info("ALERT-MGR", "AlertManager initialized");
    return true;
}
...
void AlertManager::unregister_sink(const std::string& name) {
    ...
    sinks_.erase(
        std::remove_if(sinks_.begin(), sinks_.end(),
            [&name](const auto& sink) { return sink->get_name() == name; }),
        sinks_.end()
    );
}
...
void AlertManager::raise_alert(AlertType type, AlertSeverity severity, const std::string& message,
...
uint64_t AlertManager::get_alerts_by_severity_count(AlertSeverity severity) const {
    std::lock_guard<std::mutex> lock(mutex_);
    uint64_t count = 0;
...
void AlertManager::clear_expired_alerts() {
    ...
    uint64_t one_hour_ago = []() -> uint64_t {
        struct timeval tv;
        gettimeofday(&tv, nullptr);
        return static_cast<uint64_t>(tv.tv_sec) * 1000000ULL + tv.tv_usec - 3600000000ULL;
    }();
    
    while (!alert_history_.empty() && alert_history_.front().timestamp_us < one_hour_ago) {
        alert_history_.pop_front();
    }
}
