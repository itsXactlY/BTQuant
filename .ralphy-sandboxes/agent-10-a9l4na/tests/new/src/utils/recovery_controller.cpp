...
RecoveryController::RecoveryController(const RecoveryConfig& config)
    : config_(config) {
    Logging::DynamicLogger::instance().info("RECOVERY", "RecoveryController initialized");
}

RecoveryController::~RecoveryController() {
    Logging::DynamicLogger::instance().info("RECOVERY", "RecoveryController destroyed");
}
...
RecoveryController::RecoveryAction RecoveryController::determine_action(
    ...
    if (should_abort_recovery()) {
        Logging::DynamicLogger::instance().error("RECOVERY", 
            "Recovery aborted - maximum retries exceeded");
...
void RecoveryController::record_attempt(RecoveryAction action, bool success, 
    ...
    state_.last_attempt_us = static_cast<uint64_t>(tv.tv_sec) * 1000000ULL + tv.tv_usec;
...
std::string RecoveryController::action_to_string(RecoveryAction action) {
    switch (action) {
        ...
        case RecoveryAction::RETRY_ATTACH: return "RETRY_ATTACH";
        case RecoveryAction::RECREATE_SHM: return "RECREATE_SHM";
        case RecoveryAction::RESTART_COLLECTOR: return "RESTART_COLLECTOR";
        case RecoveryAction::FAIL_OVER: return "FAIL_OVER";
        case RecoveryAction::EXIT: return "EXIT";
        default: return "UNKNOWN";
    }
}

bool RecoveryController::retry_attach() {
    Logging::DynamicLogger::instance().warning("RECOVERY", "Attempting to retry SHM attach");
    
    // Wait before retry
    std::this_thread::sleep_for(std::chrono::microseconds(config_.retry_delay_us));
    ...
    if (stat("/dev/shm/btq_hotspine", &st) == 0) {
        Logging::DynamicLogger::instance().info("RECOVERY", "SHM file now accessible");
        return true;
    }
    
    Logging::DynamicLogger::instance().warning("RECOVERY", "SHM file still not accessible");
    return false;
}

bool RecoveryController::recreate_shared_memory() {
    Logging::DynamicLogger::instance().warning("RECOVERY", "Attempting to recreate SHM");
    ...
    if (!config_.collector_command.empty()) {
        ...
        if (result == 0) {
            Logging::DynamicLogger::instance().info("RECOVERY", 
                "Collector restart command executed");
            return true;
        }
    }
...
bool RecoveryController::restart_collector() {
    Logging::DynamicLogger::instance().warning("RECOVERY", "Attempting to restart collector");
    
    if (config_.collector_command.empty()) {
        Logging::DynamicLogger::instance().warning("RECOVERY", 
            "No collector command configured");
        return false;
    }
    ...
    Logging::DynamicLogger::instance().error("RECOVERY", 
        "Failed to restart collector: " + std::to_string(result));
    return false;
}

bool RecoveryController::activate_failover() {
    Logging::DynamicLogger::instance().warning("RECOVERY", "Activating fail-over mode");
...
bool RecoveryController::exit_application() {
    Logging::DynamicLogger::instance().error("RECOVERY", 
        "Unrecoverable failure - application will exit");
...
void RecoveryController::raise_recovery_alert(RecoveryAction action, bool success,
    ...
    if (success) {
        Alerting::AlertManager::instance().raise_alert(
            Alerting::AlertType::SYSTEM_RECOVERY,
        ...
        Alerting::AlertManager::instance().raise_alert(
            Alerting::AlertType::SYSTEM_DEGRADED,
...
bool RecoveryOrchestrator::execute_handlers(RecoveryController::RecoveryAction action) {
