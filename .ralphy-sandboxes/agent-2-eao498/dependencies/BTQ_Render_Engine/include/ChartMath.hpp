...
public:
  ...
  static glm::vec2 ScreenToMarket(const glm::vec2 &screenPos,
                                  const ViewPort &view, float windowWidth,
                                  float windowHeight) {
    if (!view.isValid() || windowWidth <= 0 || windowHeight <= 0)
      return glm::vec2(0.0f);
