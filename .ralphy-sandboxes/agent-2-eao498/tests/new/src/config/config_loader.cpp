...
bool ConfigLoader::set(const std::string& section, 
...
std::optional<ConfigSource> ConfigLoader::get_source(const std::string& section,
                                                       const std::string& key) const {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    for (char** env = environ; *env != nullptr; env++) {
        ...
        if (env_str.rfind(ENV_PREFIX, 0) == 0) {
            ...
            if (eq_pos != std::string::npos) {
                ...
                std::transform(section_key.begin(), section_key.end(), section_key.begin(), ::tolower);
                ...
                if (underscore_pos != std::string::npos) {
                   ...
                   if (value == "true" || value == "false") {
                       ...
                       try {
                           config_[section][key] = ConfigValue(static_cast<int64_t>(std::stoll(value)));
                       } catch (...) {
                           config_[section][key] = ConfigValue(value);
                       }
                   }
    ...
    while (std::getline(stream, line)) {
        ...
        // Key-value pair
           ...
           if (value == "true" || value == "false") {
               ...
               try {
                   config_[current_section][key] = ConfigValue(static_cast<int64_t>(std::stoll(value)));
               } catch (...) {
                   config_[current_section][key] = ConfigValue(value);
               }
           }
...
bool ConfigLoader::get_env_var_as_bool(const std::string& name, bool default_val) {
    std::string value = get_env_var(name);
    if (value.empty()) return default_val;
    return value == "true" || value == "1" || value == "yes";
}

int64_t ConfigLoader::get_env_var_as_int(const std::string& name, int64_t default_val) {
    std::string value = get_env_var(name);
    if (value.empty()) return default_val;
    try {
...
double ConfigLoader::get_env_var_as_double(const std::string& name, double default_val) {
    std::string value = get_env_var(name);
    if (value.empty()) return default_val;
    try {
