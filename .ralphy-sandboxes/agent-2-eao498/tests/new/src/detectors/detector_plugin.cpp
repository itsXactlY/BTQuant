...
void IDetector::update_orderbook(const std::string& exchange, const std::string& symbol) {
    // Default: no action
}
...
std::vector<std::string> IDetector::get_dependencies() const {
    return {};
}

void IDetector::set_dependency(const std::string& name, std::shared_ptr<void> dependency) {
...
bool DetectorRegistry::register_detector(const std::string& name, DetectorFactory factory) {
    std::lock_guard<std::mutex> lock(mutex_);
    
    if (plugins_.find(name) != plugins_.end()) {
        std::cerr << "Warning: Detector '" << name << "' already registered" << std::endl;
        return false;
    }
...
bool DetectorRegistry::register_detector(const PluginInfo& plugin) {
    ...
    if (plugins_.find(plugin.name) != plugins_.end()) {
        std::cerr << "Warning: Detector '" << plugin.name << "' already registered" << std::endl;
        return false;
    }
...
std::shared_ptr<IDetector> DetectorRegistry::load_plugin(const std::string& path) {
    ...
    if (!create_func) {
        std::cerr << "Error: create_detector function not found in plugin" << std::endl;
        dlclose(handle);
        return nullptr;
    }
    ...
    auto info_func = reinterpret_cast<PluginInfo(*)()>(
...
std::shared_ptr<IDetector> DetectorRegistry::get_detector(const std::string& name) {
    std::lock_guard<std::mutex> lock(mutex_);
