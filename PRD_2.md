...
- [x] Delete legacy footprint artifacts: remove `src/widgets/FootprintChart.cpp` and `include/widgets/FootprintChart.h` after migration verification to eliminate BTQ namespace

## Phase 2: Render-Loop Decoupling (Performance Critical)

- [x] Extract indicator calculations from render loop: move calculate_sma, calculate_ema, calculate_rsi out of render() in `src/components/chart_panel.cpp` (lines 185-350) into cached map updated only on new data
- [x] Optimize crosshair search: replace linear O(n) candle search with std::lower_bound binary search O(log n) in render_crosshair_info method (line 436) in `src/components/chart_panel.cpp`
