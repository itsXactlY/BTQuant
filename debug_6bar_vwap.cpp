...
int main() {
    std::cout << "Debugging 6-bar Rolling VWAP implementation..." << std::endl;
    ...
    for (int i = 0; i < 6; ++i) {
        std::cout << "Bar " << i << ": H=" << bars[i].high << ", L=" << bars[i].low << ", C=" << bars[i].close 
                  << ", typical=" << (bars[i].high + bars[i].low + bars[i].close) / 3.0 << std::endl;
    }
    ...
    std::cout << "\nNumber of VWAP values calculated: " << vwapValues.size() << std::endl;
    ...
    std::cout << "\nExpected first VWAP: " << expectedFirst << std::endl;
