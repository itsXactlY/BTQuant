...
struct FootprintCell {
  ...
  // Constructor
  FootprintCell(double x_pos, double y_pos, double w, double h,
              double bid_vol, double ask_vol, uint32_t count, double vwap_price)
      : x(x_pos), y(y_pos), width(w), height(h),
        bid_volume(bid_vol), ask_volume(ask_vol),
        delta(bid_vol - ask_vol), trade_count(count), vwap(vwap_price) {}
};
