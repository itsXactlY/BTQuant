...
struct GlobalAlert {
    ...
    GlobalAlert(const std::string& alert_id, const std::string& alert_name, uint32_t sym_id,
        ...
        : id(alert_id), name(alert_name), symbol_id(sym_id), symbol_name(sym_name),
          type(alert_type), threshold_value(threshold), current_value(0.0),
          status(AlertStatus::ACTIVE), created_at(std::chrono::system_clock::now()),
...
private:
    ...
    bool should_trigger_alert(const GlobalAlert& alert, double current_price, double current_volume) const;
