...
class TimeAndSalesPanel : public PanelBase {
 public:
  TimeAndSalesPanel(const PanelConfig& config, std::shared_ptr<HotSpineDataBridge> bridge,
 ...
 private:
  ...
  std::vector<LogBucket> computeLogarithmicTradeSizeHistogram(
      const std::vector<RenderEngine::TradeData>& trades) const;

  // Methods for trade pace calculation
  double calculateTradesPerMinute(const std::vector<RenderEngine::TradeData>& trades,
                                  uint64_t window_microseconds) const;
