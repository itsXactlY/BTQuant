...
class VolumeProfilePanel : public PanelBase {
 public:
  ...
  // Set cluster engine for live data
  void set_cluster_engine(ClusterEngine* engine) { cluster_engine_ = engine; }
  ...
  void render_step_profile_histograms(ImDrawList* draw_list,
                                      ...
                                      const std::vector<double>& y_coords_high,
                                      const std::vector<double>& y_coords_low,
                                      bool show_poc_line = true, int num_buckets = 8);
  ...
  void render_step_profile_on_candles(ImDrawList* draw_list,
                                      ...
                                      const std::vector<double>& y_coords_high,
                                      const std::vector<double>& y_coords_low,
                                      bool show_poc_line = true, int num_buckets_per_candle = 8);
  ...
  static void render_step_profile_on_candles_static(
      ...
      const std::vector<double>& y_coords_low, const std::vector<TradeData>& trades,
      bool show_poc_line = true, int num_buckets_per_candle = 8);

  // Enhanced method to render step profile histograms on candlesticks with additional features
  static void render_enhanced_step_profile_on_candles(
      ImDrawList* draw_list, const std::vector<RenderEngine::OHLCVCandle>& candles,
      const std::vector<double>& x_coords, const std::vector<double>& y_coords_high,
      const std::vector<double>& y_coords_low, const std::vector<TradeData>& trades,
      bool show_poc_line = true, int num_buckets_per_candle = 8, float opacity_factor = 1.0f);

  // Main method to implement Step Profile rendering: draw mini histogram overlay on each
  // candlestick bar showing volume distribution for that bar's price range
  void render_step_profile_on_candles_with_volume_distribution(
      ImDrawList* draw_list, const std::vector<RenderEngine::OHLCVCandle>& candles,
      const std::vector<double>& x_coords, const std::vector<double>& y_coords_high,
  ...
  void render_enhanced_step_profile_with_volume_distribution(
      ImDrawList* draw_list, const std::vector<RenderEngine::OHLCVCandle>& candles,
      const std::vector<double>& x_coords, const std::vector<double>& y_coords_high,
      const std::vector<double>& y_coords_low, bool show_poc_line = true,
      int num_buckets_per_candle = 8, float bar_opacity = 1.0f,
  ...
  // candlestick bar showing volume distribution for that bar's price range - this is the main
  // method for the task requirement
 ...
 private:
  ClusterEngine* cluster_engine_ = nullptr;
  ...
  std::vector<DailyVolumeProfile>
      daily_profiles_;  // Individual daily profiles for composite aggregation
  ...
  void render_yesterday_split_profile(const double* xs, const double* buy_vols,
                                      const double* sell_vols, int count, double height,
                                      float opacity);
  void highlight_profile_divergence(
      ImDrawList* draw_list);  // Highlight zones where profiles diverge significantly
  void calculate_value_area();
  void calculate_value_area_for_session(
      SessionProfile& session);               // Calculate value area for a specific session profile
  ...
  void add_daily_profile(const std::vector<VolumeLevel>& daily_profile,
                         time_t date);               // Add daily profile to composite
  ...
  bool is_new_session_boundary(double current_timestamp,
                               double previous_timestamp);  // Check if new session boundary
  void create_new_session_profile(double start_time, double end_time,
                                  const std::string& session_name);  // Create new session profile
