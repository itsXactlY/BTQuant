...
class DynamicLogger {
 public:
  ...
  void critical(const std::string& msg, const char* file = "", int line = 0) {
...
#define BTQ_LOG_DEBUG(msg) \
  BTQuant::Utils::DynamicLogger::getInstance().debug(msg, __FILE__, __LINE__)
#define BTQ_LOG_INFO(msg) BTQuant::Utils::DynamicLogger::getInstance().info(msg, __FILE__, __LINE__)
#define BTQ_LOG_WARNING(msg) \
  BTQuant::Utils::DynamicLogger::getInstance().warning(msg, __FILE__, __LINE__)
#define BTQ_LOG_ERROR(msg) \
  BTQuant::Utils::DynamicLogger::getInstance().error(msg, __FILE__, __LINE__)
#define BTQ_LOG_CRITICAL(msg) \
  BTQuant::Utils::DynamicLogger::getInstance().critical(msg, __FILE__, __LINE__)
