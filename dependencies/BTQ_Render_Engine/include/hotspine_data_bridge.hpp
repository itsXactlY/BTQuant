...
enum class MarketDataType { TRADE, ORDERBOOK, TICKER };
...
public:
   ...
   * @param symbols_file Path to symbol mappings file (e.g.,
   * "/dev/shm/btquant_symbols.json")
   */
  explicit HotSpineDataBridge(
...
private:
  ...
  void updateSymbolMarketData(uint32_t symbol_id, double price, double size,
                              uint64_t timestamp);
  void updateSymbolOrderbookData(uint32_t symbol_id, double bid_price,
                                 double ask_price, uint64_t timestamp);
