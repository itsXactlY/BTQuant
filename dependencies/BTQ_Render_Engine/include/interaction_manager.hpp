...
enum class DragDropType { Symbol, Panel, Chart, Indicator, Layout };
...
public:
  using DropCallback =
      std::function<bool(const DragDropData &, const glm::vec2 &)>;
...
public:
  ...
  void register_hotkey(const std::string &name, int key_code,
                       uint32_t modifiers, std::function<void()> action,
...
private:
  ...
  void dispatch_event_to_component(UIComponent *component,
