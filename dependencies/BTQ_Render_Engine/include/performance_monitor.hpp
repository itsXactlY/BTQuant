...
// Forward declarations if needed, but they are already in BTQuant namespace
using BTQuant::ComponentResourceUtilization;
using BTQuant::LatencyBreakdown;
using BTQuant::SpineHealthMetrics;
...
struct SystemMetrics {
  ...
  SpineHealthMetrics spine_health;

  // Latency breakdown
  LatencyBreakdown latency_breakdown;

  // Component resource utilization
  std::vector<ComponentResourceUtilization> resource_utilization;
...
struct PerformanceStatistics {
  ...
  double min_health_score = 0.0;
  double max_health_score = 0.0;

  // Latency breakdown statistics
  double avg_data_processing_us = 0.0;
  double avg_gpu_transfer_us = 0.0;
  double avg_display_update_us = 0.0;
...
public:
  ...
  void updateNetworkStatus(bool connected, double latency_ms,
                           double throughput_mbps);
  void updateSystemHealth(double cpu_usage_percent, double gpu_usage_percent,
                          double temperature_celsius);

  /**
   * Update spine health metrics
   */
  void updateSpineHealth(const SpineHealthMetrics &health_metrics);
  void updateLatencyBreakdown(const LatencyBreakdown &latency_breakdown);
  void updateComponentResourceUtilization(
      const std::vector<ComponentResourceUtilization> &resource_utilization);
  ...
  getComponentResourceUtilization() const;
  ...
  /**
   * Get specialized history
   */
  std::vector<MetricSample> getHealthScoreHistory() const;
  std::vector<MetricSample> getDataProcessingLatencyHistory() const;
  std::vector<MetricSample> getGPUTransferLatencyHistory() const;
  std::vector<MetricSample> getDisplayUpdateLatencyHistory() const;
   ...
   * Configuration
   */
  void setAlertThresholds(double fps_threshold, double latency_ms_threshold,
...
private:
  ...
  std::vector<MetricSample> health_score_history_;
  std::vector<MetricSample> data_processing_latency_history_;
  std::vector<MetricSample> gpu_transfer_latency_history_;
  std::vector<MetricSample> display_update_latency_history_;
  ...
  double calculatePercentile(std::vector<double> values,
  ...
  std::string formatTimestamp(
      const std::chrono::high_resolution_clock::time_point &tp) const;
};
