...
static_assert(GpuAlignable<GpuOrderBookLevel>,
              "GpuOrderBookLevel must be GPU-alignable");
...
struct alignas(16) HotspineOrderBookSnapshot {
  ...
  static constexpr size_t calculateBufferSize(uint32_t priceLevelCount) {
    return sizeof(HotspineOrderBookSnapshot) +
           priceLevelCount * sizeof(GpuOrderBookLevel);
  }
};
...
static_assert(GpuAlignable<CandleCluster>,
              "CandleCluster must be GPU-alignable");
...
static_assert(GpuAlignable<HotspineTradeTick>,
              "HotspineTradeTick must be GPU-alignable");
...
serializeOrderBookSnapshot(const HotspineOrderBookSnapshot &snapshot) {
...
inline uint32_t getPriceLevelIndex(float price, float basePrice,
                                   float priceRange, uint32_t bucketCount) {
