...
int main() {
    ...
    std::vector<double> data3(size);
    std::iota(data3.begin(), data3.end(), 1.0);
    
    // Test async reduce
    start = std::chrono::high_resolution_clock::now();
    auto future_sum = scheduler.parallel_reduce_async(data3.begin(), data3.end(), 0.0);
    double async_sum = future_sum.get(); // Wait for result
    end = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    std::cout << "Async parallel reduce took: " << duration.count() << " ms\n";
    std::cout << "Async sum: " << async_sum << "\n";
