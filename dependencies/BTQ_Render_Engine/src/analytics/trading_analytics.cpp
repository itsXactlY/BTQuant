  ...
  static std::vector<IndicatorResult> macd(const std::vector<OHLCV> &data,
    ...
    if (macd_line.values.size() >= static_cast<size_t>(signal_period)) {
      ...
      for (size_t i = 1; i < macd_line.values.size(); ++i) {
        signal =
            (macd_line.values[i] * multiplier) + (signal * (1 - multiplier));
        signal_line.values.push_back(signal);
    ...
    for (size_t i = 0;
         i < std::min(macd_line.values.size(), signal_line.values.size());
         ++i) {
...
public:
  ...
  VolumeProfile
  calculate_volume_profile(const std::vector<TechnicalIndicators::OHLCV> &data,
...
public:
  ...
  detect_patterns(const std::vector<TechnicalIndicators::OHLCV> &data) {
    ...
    patterns.insert(patterns.end(), double_bottoms.begin(),
                    double_bottoms.end());

    auto head_shoulders = detect_head_and_shoulders(data);
    patterns.insert(patterns.end(), head_shoulders.begin(),
                    head_shoulders.end());
...
private:
      ...
      if (price_diff < 0.02) {
        ...
        if (valley_depth > 0.03) {
          ...
          pattern.target_price =
              valley_price -
              (std::max(peak1_price, peak2_price) - valley_price);
  ...
  detect_double_bottom(const std::vector<TechnicalIndicators::OHLCV> &data) {
    ...
    for (size_t i = 1; i < troughs.size(); ++i) {
      ...
      double price_diff =
          std::abs(trough1_price - trough2_price) / trough1_price;
      if (price_diff < 0.02) {
        ...
        if (peak_height > 0.03) {
          ...
          pattern.target_price =
              peak_price +
              (peak_price - std::min(trough1_price, trough2_price));
  ...
  double calculate_double_bottom_confidence(
    ...
    double trough_similarity =
        1.0 -
        std::abs(data[trough1].low - data[trough2].low) / data[trough1].low;
    double peak_height =
