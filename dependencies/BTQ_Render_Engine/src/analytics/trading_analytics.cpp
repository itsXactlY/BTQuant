...
std::vector<Trade> TradingAnalytics::get_trades() const {
    return impl_->trades;
}
...
void TradingAnalytics::clear_trades() {
    impl_->trades.clear();
}
...
PerformanceMetrics TradingAnalytics::calculate_performance_metrics(const std::string& symbol) const {
...
TradeStatistics TradingAnalytics::calculate_trade_statistics() const {
    ...
    stats.win_rate = stats.total_trades > 0 ? 
        static_cast<double>(winning_trades) / stats.total_trades : 0.0;
    ...
    stats.average_trade_duration_seconds = stats.total_trades > 0 ? 
        total_duration / stats.total_trades : 0.0;
...
EquityCurve TradingAnalytics::calculate_equity_curve(double initial_capital) const {
    ...
    for (const auto& trade : sorted_trades) {
        ...
        double drawdown = peak_equity > 0 ? 
            (peak_equity - equity) / peak_equity : 0.0;
...
EquityCurve TradingAnalytics::calculate_equity_curve(const std::string& symbol, double initial_capital) const {
    std::vector<Trade> symbol_trades = get_trades(symbol);
    if (symbol_trades.empty()) {
        return EquityCurve{};
    }
...
double TradingAnalytics::calculate_volatility(int period_days) const {
    ...
    for (const auto& trade : impl_->trades) {
        ...
        std::string day_key = std::to_string(tm.tm_year) + "-" + 
...
std::unordered_map<std::string, TradeStatistics> 
TradingAnalytics::get_statistics_by_symbol() const {
