...
void MarketMicrostructureRenderer::updateTradeData(std::span<const HotspineTradeTick> trades) {
  ...
  // Process trades with the cluster engine for footprint analysis
  for (const auto& t : trades) {
    ...
    trade_data.price = static_cast<double>(t.price);
    trade_data.quantity = static_cast<double>(t.size);  // Changed from 'volume' to 'size'
    trade_data.timestamp_us = t.timestamp;
    trade_data.is_buyer_maker = t.isBuy;  // Changed from 'is_buy' to 'isBuy'
...
void MarketMicrostructureRenderer::notifyPriceAggregationChanged() {
  ...
  // The cluster engine doesn't have a reset method, so we just clear the data
  // New data will be processed with the updated aggregation settings
}
