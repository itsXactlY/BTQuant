...
ChartPanel::ChartPanel(const PanelConfig& config, std::shared_ptr<HotSpineDataBridge> bridge,
                       ...
                       ChartManager* chart_manager, PanelManager* panel_manager)
    : PanelBase(config),
      bridge_(bridge),
...
void ChartPanel::render_chart_controls() {
  ...
  BTQuant::UI::show_control_tooltip("chart_timeframe_selector");
  ...
  BTQuant::UI::show_control_tooltip("chart_window_size");

  // Auto-follow checkbox
  ImGui::SameLine();
  if (ImGui::Checkbox("Auto-follow", &follow_latest_)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
  }
  BTQuant::UI::show_control_tooltip("chart_autofollow_toggle");
...
void ChartPanel::render_liquidity_bars_controls() {
  ...
  if (ImGui::Checkbox("Show Liquidity Bars", &show_liquidity_bars_)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
  }
  BTQuant::UI::show_control_tooltip("chart_show_liquidity_bars");

  // Slider for adjusting bar width
  if (ImGui::SliderFloat("Bar Width", &liquidity_bar_width_, 5.0f, 30.0f, "%.1f px")) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
  }
  BTQuant::UI::show_control_tooltip("chart_liquidity_bar_width");

  // Slider for adjusting opacity
  if (ImGui::SliderFloat("Opacity", &liquidity_bar_opacity_, 0.1f, 1.0f, "%.2f")) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
  }
  BTQuant::UI::show_control_tooltip("chart_liquidity_opacity");
...
void ChartPanel::render_indicator_selector() {
  ...
  BTQuant::UI::show_control_tooltip("enable_rsi");
  ImGui::SameLine();
  if (ImGui::Checkbox("MACD", &indicator_config_.show_macd)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    sync_active_indicators_with_config();
  }
  BTQuant::UI::show_control_tooltip("enable_macd");
  ImGui::SameLine();
  if (ImGui::Checkbox("Stochastic", &indicator_config_.show_stochastic)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    sync_active_indicators_with_config();
  }
  BTQuant::UI::show_control_tooltip("enable_stochastic");
  ImGui::SameLine();
  if (ImGui::Checkbox("ATR", &indicator_config_.show_atr)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    sync_active_indicators_with_config();
  }
  BTQuant::UI::show_control_tooltip("enable_atr");
  ...
  BTQuant::UI::show_control_tooltip("enable_bollinger_bands");
  ImGui::SameLine();
  if (ImGui::Checkbox("Fibonacci", &indicator_config_.show_fibonacci)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    sync_active_indicators_with_config();
  }
  BTQuant::UI::show_control_tooltip("enable_fibonacci");
  ImGui::SameLine();
  if (ImGui::Checkbox("Vol Profile", &indicator_config_.show_volume_profile)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    sync_active_indicators_with_config();
  }
  BTQuant::UI::show_control_tooltip("enable_volume_profile");
  ImGui::SameLine();
  if (ImGui::Checkbox("Crosshair Info", &indicator_config_.show_crosshair_info)) {
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    sync_active_indicators_with_config();
  }
  BTQuant::UI::show_control_tooltip("enable_crosshair_info");
