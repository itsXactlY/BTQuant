...
void ChartPanelSettings::render() {
  ...
  if (ImGui::Begin((title_ + "###chart_settings_modal").c_str(), &is_modal_open_,
    ...
    if (temp_config_.show_rsi) {
      ...
      double rsi_ob_min = 50.0, rsi_ob_max = 100.0;
      ImGui::SliderScalar("Overbought", ImGuiDataType_Double, &temp_config_.rsi_overbought,
                          &rsi_ob_min, &rsi_ob_max);
      double rsi_os_min = 0.0, rsi_os_max = 50.0;
      ImGui::SliderScalar("Oversold", ImGuiDataType_Double, &temp_config_.rsi_oversold, &rsi_os_min,
                          &rsi_os_max);
    }
    ...
    if (temp_config_.show_bollinger) {
      ...
      double bb_std_min = 0.5, bb_std_max = 5.0;
      ImGui::SliderScalar("Std Dev", ImGuiDataType_Double, &temp_config_.bollinger_std_dev,
...
void ChartPanelSettings::save_settings() {
  ...
  std::string settings_key =
      "chart_panel_" + std::to_string(reinterpret_cast<uintptr_t>(chart_panel));
...
void ChartPanelSettings::load_settings() {
  ...
  if (!file.is_open()) {
    ...
    std::cout << "Chart panel settings file not found: " << filename << ", using defaults"
              << std::endl;
    return;
  }
  ...
  try {
    ...
    if (settings_json.contains("show_sma_20"))
      temp_config_.show_sma_20 = settings_json["show_sma_20"];
    if (settings_json.contains("show_sma_50"))
      temp_config_.show_sma_50 = settings_json["show_sma_50"];
    if (settings_json.contains("show_sma_200"))
      temp_config_.show_sma_200 = settings_json["show_sma_200"];
    if (settings_json.contains("show_ema_9")) temp_config_.show_ema_9 = settings_json["show_ema_9"];
    if (settings_json.contains("show_ema_21"))
      temp_config_.show_ema_21 = settings_json["show_ema_21"];
    if (settings_json.contains("show_ema_50"))
      temp_config_.show_ema_50 = settings_json["show_ema_50"];
    if (settings_json.contains("show_ema_200"))
      temp_config_.show_ema_200 = settings_json["show_ema_200"];
    ...
    if (settings_json.contains("show_stochastic"))
      temp_config_.show_stochastic = settings_json["show_stochastic"];
    if (settings_json.contains("show_atr")) temp_config_.show_atr = settings_json["show_atr"];
    if (settings_json.contains("show_volume_profile"))
      temp_config_.show_volume_profile = settings_json["show_volume_profile"];
    if (settings_json.contains("show_fibonacci"))
      temp_config_.show_fibonacci = settings_json["show_fibonacci"];
    if (settings_json.contains("show_crosshair_info"))
      temp_config_.show_crosshair_info = settings_json["show_crosshair_info"];
    if (settings_json.contains("rsi_period")) temp_config_.rsi_period = settings_json["rsi_period"];
    if (settings_json.contains("rsi_overbought"))
      temp_config_.rsi_overbought = settings_json["rsi_overbought"];
    if (settings_json.contains("rsi_oversold"))
      temp_config_.rsi_oversold = settings_json["rsi_oversold"];
    if (settings_json.contains("bollinger_period"))
      temp_config_.bollinger_period = settings_json["bollinger_period"];
    if (settings_json.contains("bollinger_std_dev"))
      temp_config_.bollinger_std_dev = settings_json["bollinger_std_dev"];
    if (settings_json.contains("macd_fast_period"))
      temp_config_.macd_fast_period = settings_json["macd_fast_period"];
    if (settings_json.contains("macd_slow_period"))
      temp_config_.macd_slow_period = settings_json["macd_slow_period"];
    if (settings_json.contains("macd_signal_period"))
      temp_config_.macd_signal_period = settings_json["macd_signal_period"];
    if (settings_json.contains("stochastic_k_period"))
      temp_config_.stochastic_k_period = settings_json["stochastic_k_period"];
    if (settings_json.contains("stochastic_d_period"))
      temp_config_.stochastic_d_period = settings_json["stochastic_d_period"];
    if (settings_json.contains("stochastic_slow_period"))
      temp_config_.stochastic_slow_period = settings_json["stochastic_slow_period"];
    ...
    std::cerr << "Error loading chart panel settings from " << filename << ": " << e.what()
              << std::endl;
  }
}
