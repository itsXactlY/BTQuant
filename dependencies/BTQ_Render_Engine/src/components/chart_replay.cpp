...
void ChartReplay::start_replay() {
  ...
  if (replay_chart_id_ == 0) {
    replay_chart_id_ = chart_manager_->create_chart(config_.symbol_name, config_.exchange_name,
                                                    config_.symbol_id, config_.timeframe);
  }
...
void ChartReplay::pause_replay() { is_paused_ = true; }
...
void ChartReplay::update_performance_metrics() {
  ...
  if (metrics_.total_bars > 0) {
    metrics_.progress_percentage =
        (static_cast<double>(current_bar_index_) / static_cast<double>(metrics_.total_bars)) *
        100.0;
  }

  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(metrics_.current_time -
                                                                        metrics_.start_time)
                      .count();
  ...
  if (metrics_.elapsed_seconds > 0) {
    metrics_.replay_speed_actual =
        static_cast<double>(current_bar_index_) / metrics_.elapsed_seconds;
  }
...
bool ChartReplay::load_historical_data(const std::string& symbol, const std::string& exchange,
                                       RenderEngine::TimeFrame timeframe, uint64_t start_time,
                                       uint64_t end_time) {
  ...
  while (current_time <= end_time && historical_candles_.size() < 10000) {  // Limit to 10k bars
    ...
    double momentum =
        (static_cast<double>(rand()) / RAND_MAX - 0.5) * 0.02;  // Small random momentum
    ...
    double upper_wick_ratio = is_bullish ? 0.1 + (static_cast<double>(rand()) / RAND_MAX) * 0.3
                                         :  // 10-40% of range for upper wick in bullish
                                  0.3 + (static_cast<double>(rand()) / RAND_MAX) *
                                            0.4;  // 30-70% of range for upper wick in bearish

    double lower_wick_ratio = is_bullish ? 0.3 + (static_cast<double>(rand()) / RAND_MAX) * 0.4
                                         :  // 30-70% of range for lower wick in bullish
                                  0.1 + (static_cast<double>(rand()) / RAND_MAX) *
                                            0.3;  // 10-40% of range for lower wick in bearish
    ...
    double volatility_multiplier =
        0.8 + volatility_factor * 10.0;  // Higher volume with higher volatility
    double movement_multiplier =
        1.0 + std::abs(rand_change) * 50.0;  // Higher volume with larger moves

    candle.volume = volume_base * volume_multiplier * volatility_multiplier * movement_multiplier;
    candle.trade_count =
        static_cast<uint64_t>(candle.volume * (0.1 + (static_cast<double>(rand()) / RAND_MAX) *
                                                         0.5));  // 10-60% of volume as trade count
  ...
  std::cout << "Loaded " << historical_candles_.size() << " historical candles for replay."
            << std::endl;
...
bool ChartReplay::load_from_csv(const std::string& csv_file_path, const std::string& symbol,
  ...
  std::cout << "Loaded " << historical_candles_.size()
            << " candles from CSV file: " << csv_file_path << std::endl;
...
void ChartReplay::replay_thread_func() {
  ...
  while (!should_stop_ && current_bar_index_ < historical_candles_.size()) {
    ...
    while (accumulated_time_ >= interval_double &&
           current_bar_index_ < historical_candles_.size()) {
      process_next_bar();
...
void ChartReplay::update_chart_with_current_data() {
  ...
  auto& charts =
      const_cast<std::unordered_map<uint32_t, ChartInstance>&>(chart_manager_->get_charts());
  ...
  size_t end_index =
      std::min(current_bar_index_ + 1, historical_candles_.size());  // Include current bar
...
void ChartReplay::render_replay_controls() {
  ...
  // Control buttons
  bool play_pause_clicked = ImGui::Button(is_playing_ ? "Pause" : "Play");
  ...
  ImGui::SameLine();
  bool stop_clicked = ImGui::Button("Stop");
  if (stop_clicked) {
    stop_replay();
    reset_replay();
    // Add haptic feedback for button interaction
  ...
  ImGui::SameLine();
  bool reset_clicked = ImGui::Button("Reset");
  if (reset_clicked) {
    reset_replay();
    // Add haptic feedback for button interaction
    BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
  ...
  if (config_.enable_manual_control) {
    ImGui::SameLine();
    bool prev_clicked = ImGui::Button("<< Prev");
    if (prev_clicked) {
      step_backward();
      // Add haptic feedback for button interaction
      BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
    ...
    ImGui::SameLine();
    bool next_clicked = ImGui::Button("Next >>");
    if (next_clicked) {
      step_forward();
      // Add haptic feedback for button interaction
      BTQuant::UI::HapticFeedback::getInstance().triggerForSubtleInteraction();
  ...
  // Status display
  ImGui::Text("Status: %s", is_playing_ ? "Playing" : is_paused_ ? "Paused" : "Stopped");
  ...
  if (ImGui::Button("Load from CSV")) {
    ...
    if (load_from_csv(std::string(csv_path_buffer), std::string(symbol_buffer),
                      config_.timeframe)) {
  ...
  if (processor_) {
    ...
    const char* timeframes2[] = {"1ms", "10ms", "100ms", "500ms", "1s",  "3s",  "5s",
                                 "15s", "30s",  "1m",    "2m",    "5m",  "15m", "30m",
                                 "1h",  "2h",   "4h",    "6h",    "12h", "1d",  "1w"};
    ...
    if (ImGui::Button("Load from Market Data")) {
      ...
      if (load_from_market_data_processor(symbol_id_input, config_.timeframe, start_time_input2,
                                          end_time_input2)) {
...
bool ChartReplay::load_from_market_data_processor(uint32_t symbol_id,
                                                  RenderEngine::TimeFrame timeframe,
  ...
  try {
    ...
    std::cout << "Loaded " << historical_candles_.size()
              << " historical candles from market data processor." << std::endl;
