...
DataGridComponent::DataGridComponent(const glm::vec2& position, const glm::vec2& size, 
...
void DataGridComponent::set_cell_data(size_t row, size_t col, const CellData& data) {
    if (row >= rows_ || col >= columns_) return;
    
    grid_data_[row][col] = data;
    dirty_ = true;
}

void DataGridComponent::set_row_data(size_t row, const std::vector<CellData>& row_data) {
    if (row >= rows_) return;
...
void DataGridComponent::set_column_header(size_t col, const std::string& header) {
...
void DataGridComponent::render(VkCommandBuffer cmd) {
    ...
    ubo.projection = glm::ortho(0.0f, 1920.0f, 1080.0f, 0.0f, -1.0f, 1.0f); // TODO: Get from viewport
    ubo.view = glm::mat4(1.0f);
    ubo.grid_size = glm::vec2(columns_, rows_);
    ubo.cell_size = glm::vec2(size_.x / columns_, 25.0f); // Fixed row height for now
    ...
    // Draw the grid
    uint32_t index_count = static_cast<uint32_t>((rows_ + 1) * (columns_ + 1) * 6); // Approximate
...
void DataGridComponent::rebuild_geometry() {
    ...
    for (size_t col = 0; col < columns_; ++col) {
        ...
        vertices.push_back({{x, y}, {0.0f, 0.0f}, theme_.background_secondary, vertex_index});
        vertices.push_back({{x + cell_width, y}, {1.0f, 0.0f}, theme_.background_secondary, vertex_index});
        vertices.push_back({{x + cell_width, y + header_height}, {1.0f, 1.0f}, theme_.background_secondary, vertex_index});
        vertices.push_back({{x, y + header_height}, {0.0f, 1.0f}, theme_.background_secondary, vertex_index});
    ...
    for (size_t row = 0; row < rows_; ++row) {
        for (size_t col = 0; col < columns_; ++col) {
            ...
            glm::vec4 bg_color = (row % 2 == 0) ? theme_.background_panel : theme_.background_primary;
            ...
            vertices.push_back({{x + cell_width, y}, {1.0f, 0.0f}, bg_color, vertex_index});
            vertices.push_back({{x + cell_width, y + cell_height}, {1.0f, 1.0f}, bg_color, vertex_index});
            vertices.push_back({{x, y + cell_height}, {0.0f, 1.0f}, bg_color, vertex_index});
    ...
    for (size_t col = 0; col <= columns_; ++col) {
        ...
        vertices.push_back({{x + 1.0f, y2}, {1.0f, 1.0f}, line_color, vertex_index});
    ...
    for (size_t row = 0; row <= rows_ + 1; ++row) { // +1 for header
        float y = position_.y + (row == 0 ? 0 : header_height + (row - 1) * cell_height);
        float x1 = position_.x;
        float x2 = position_.x + size_.x;
        
        vertices.push_back({{x1, y}, {0.0f, 0.0f}, line_color, vertex_index});
        vertices.push_back({{x2, y + 1.0f}, {1.0f, 1.0f}, line_color, vertex_index});
...
void DataGridComponent::sort_data() {
    if (sort_column_ < 0 || sort_column_ >= static_cast<int>(columns_)) return;
    
    std::sort(grid_data_.begin(), grid_data_.end(), 
