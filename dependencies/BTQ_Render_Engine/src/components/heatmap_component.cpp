  ...
  for (size_t y = 0; y < grid_height_; ++y) {
    for (size_t x = 0; x < grid_width_; ++x) {
      ...
      float gradient_factor =
          (static_cast<float>(x + y) / (grid_width_ + grid_height_)) * 0.1f;
      cell_color = glm::mix(cell_color, glm::vec4(1.0f), gradient_factor);

      uint32_t base_vertex = static_cast<uint32_t>(vertices.size());

      // Create quad for cell (using instanced rendering, so just one quad
      // template)
      vertices.push_back({{cell_x, cell_y},
      ...
      vertices.push_back({{cell_x + cell_width, cell_y},
                          {cell_width, cell_height},
                          cell_color,
                          {1.0f, 0.0f},
                          cell_data.value,
      ...
      vertices.push_back({{cell_x + cell_width, cell_y + cell_height},
                          {cell_width, cell_height},
                          cell_color,
                          {1.0f, 1.0f},
                          cell_data.value,
      ...
      vertices.push_back({{cell_x, cell_y + cell_height},
                          {cell_width, cell_height},
                          cell_color,
                          {0.0f, 1.0f},
                          cell_data.value,
