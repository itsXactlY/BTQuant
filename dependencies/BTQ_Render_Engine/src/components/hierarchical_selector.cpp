...
bool HierarchicalSelector::render(HierarchicalSelectorState& state) {
  ...
  if (!state.available_exchanges.empty()) {
    const char* current_exchange =
        state.selected_exchange.empty() ? "Select Exchange" : state.selected_exchange.c_str();
  ...
  if (!state.available_symbols.empty() && !state.selected_exchange.empty()) {
    const char* current_symbol =
        state.selected_symbol.empty() ? "Select Symbol" : state.selected_symbol.c_str();
  ...
  if (!state.chart_ids.empty()) {
    char chart_label[32];
    snprintf(chart_label, sizeof(chart_label), "Chart #%u", state.selected_chart_id);
...
void HierarchicalSelector::refresh_data(HierarchicalSelectorState& state,
  ...
  if (!state.selected_exchange.empty()) {
    state.available_symbols =
        SymbolRegistry::instance().get_exchange_symbols(state.selected_exchange);
...
int HierarchicalSelector::get_timeframe_count() { return 21; }
