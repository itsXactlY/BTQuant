...
void MultiVWAPPanel::add_daily_vwap() {
  ...
  auto today_start_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(today_start_tp.time_since_epoch())
          .count();

  std::string id = generate_unique_id();
  VWAPInstance instance(id, "Daily VWAP", today_start_ms, ImVec4(1.0f, 1.0f, 0.0f, 1.0f),
                        "daily");  // Yellow
...
void MultiVWAPPanel::add_weekly_vwap() {
  ...
  auto week_start_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(week_start_tp.time_since_epoch())
          .count();

  std::string id = generate_unique_id();
  VWAPInstance instance(id, "Weekly VWAP", week_start_ms, ImVec4(0.0f, 1.0f, 1.0f, 1.0f),
                        "weekly");  // Cyan
...
void MultiVWAPPanel::add_monthly_vwap() {
  ...
  auto month_start_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(month_start_tp.time_since_epoch())
          .count();

  std::string id = generate_unique_id();
  VWAPInstance instance(id, "Monthly VWAP", month_start_ms, ImVec4(1.0f, 0.5f, 0.0f, 1.0f),
                        "monthly");  // Orange
...
void MultiVWAPPanel::add_custom_vwap() {
  std::string id = generate_unique_id();
  VWAPInstance instance(id, new_vwap_name_, new_vwap_anchor_time_, new_vwap_color_, "custom");
...
void MultiVWAPPanel::remove_vwap(const std::string& id) {
  vwap_instances_.erase(
      std::remove_if(vwap_instances_.begin(), vwap_instances_.end(),
                     [&id](const VWAPInstance& instance) { return instance.id == id; }),
      vwap_instances_.end());
}
...
uint64_t MultiVWAPPanel::get_start_of_day(uint64_t timestamp) {
  ...
  return std::chrono::duration_cast<std::chrono::milliseconds>(start_of_day_tp.time_since_epoch())
      .count();
}
...
uint64_t MultiVWAPPanel::get_start_of_week(uint64_t timestamp) {
  ...
  return std::chrono::duration_cast<std::chrono::milliseconds>(start_of_week_tp.time_since_epoch())
      .count();
}
...
uint64_t MultiVWAPPanel::get_start_of_month(uint64_t timestamp) {
  ...
  return std::chrono::duration_cast<std::chrono::milliseconds>(start_of_month_tp.time_since_epoch())
      .count();
}
