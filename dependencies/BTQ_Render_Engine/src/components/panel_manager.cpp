...
void PanelManager::update(float dt) {
  // Check for repaint requests from background threads
  if (requesting_repaint_.exchange(false, std::memory_order_acquire)) {
    for (auto& [id, panel] : panels_) {
      ...
      if (group) {
        for (uint32_t group_panel_id : group->panel_ids) {
          ...
          if (group_panel_it != panels_.end()) {
            ...
            std::cout << "[PanelManager] Rendering group panel: " << group_panel->get_title()
                      << std::endl;
            // Render the panel
            group_panel->render();
            std::cout << "[PanelManager] Finished rendering group panel: "
                      << group_panel->get_title() << std::endl;
...
uint32_t PanelManager::add_panel(PanelType type, const std::string& title, int grid_x, int grid_y,
  ...
  switch (type) {
    case PanelType::CHART:
      panel = std::make_shared<ChartPanel>(config, bridge_, processor_, chart_manager_.get(), this);
...
// Now handles smart removal of "connected" or dependent panels
      ...
      for (int x = new_grid_x; x < new_group_right; x++) {
        for (int y = new_grid_y; y < new_group_bottom; y++) {
          ...
          for (const auto& [id, panel] : panels_) {
            ...
            if (x >= config.grid_x && x < panel_right && y >= config.grid_y && y < panel_bottom) {
...
void PanelManager::save_default_layout() {
  try {
    ...
    if (file.is_open()) {
      file << serialize_layout();
      file.close();
      std::cout << "[Layout] Current panel arrangement saved as default startup layout"
                << std::endl;
...
std::string PanelManager::serialize_layout() const {
  ...
  for (const auto& [id, panel] : panels_) {
    ...
    panel_json["original_id"] =
        static_cast<int>(id);  // Store original ID for deserialization mapping
          ...
          if (p.contains("settings")) {
            ...
            // DOM Surface Panel specific settings
              ...
              if (settings.contains("persistence_threshold_ms")) {
                dom_panel->setPersistenceThresholdMs(
                    settings["persistence_threshold_ms"].get<uint64_t>());
              }
              if (settings.contains("persistence_timeout_ms")) {
                dom_panel->setPersistenceTimeoutMs(
                    settings["persistence_timeout_ms"].get<double>());
              }
              ...
              if (settings.contains("tabbed_panels")) {
                ...
                for (const auto& tabbed_id_val : tabbed_panels_array) {
                  ...
                  if (id_it != original_to_new_id_map.end()) {
                    ...
                    // If we can't find the mapping, try using the ID directly (for backward
                    // compatibility)
    ...
    if (j.contains("panels")) {
      for (const auto& p : j["panels"]) {
        if (p.contains("tabbed_group_id")) {
          ...
          if (tabbed_group_it != original_to_new_id_map.end() &&
              panel_it != original_to_new_id_map.end()) {
    ...
    if (j.contains("groups")) {
      for (const auto& g : j["groups"]) {
        ...
        bool prevent_overlap =
            g.value("prevent_overlap", true);  // Default to true for backward compatibility
...
bool PanelManager::add_panel_to_group(uint32_t group_id, uint32_t panel_id) {
  ...
  int new_max_x =
      std::max(group->min_grid_x + group->total_width, config.grid_x + config.grid_width);
  int new_max_y =
      std::max(group->min_grid_y + group->total_height, config.grid_y + config.grid_height);
...
bool PanelManager::remove_panel_from_group(uint32_t group_id, uint32_t panel_id) {
  ...
  panel_list.erase(std::remove(panel_list.begin(), panel_list.end(), panel_id), panel_list.end());
...
uint32_t PanelManager::create_super_panel_from_adjacent(uint32_t panel1_id, uint32_t panel2_id) {
  ...
  if (panels_.find(panel1_id) == panels_.end() || panels_.find(panel2_id) == panels_.end()) {
    return 0;
  }
...
uint32_t PanelManager::create_super_panel_from_rectangular_region(int start_x, int start_y,
                                                                  int width, int height) {
  // Validate region bounds
  if (start_x < 0 || start_y < 0 || start_x + width > grid_layout_.columns ||
  ...
  for (const auto& [id, panel] : panels_) {
    ...
    if (config.grid_x >= start_x && config.grid_y >= start_y &&
...
uint32_t PanelManager::create_tabbed_group(uint32_t target_panel_id) {
  ...
  uint32_t tabbed_group_id = add_panel(PanelType::TABBED_GROUP, "Tabbed Group", config.grid_x,
                                       config.grid_y, config.grid_width, config.grid_height);
  ...
  if (tabbed_group_id != 0) {
    ...
    if (auto* tabbed_panel = dynamic_cast<TabbedPanel*>(get_panel_by_id(tabbed_group_id))) {
      ...
      if (tabbed_panel->add_panel_to_tab(target_panel_id)) {
        ...
        target_config.size =
            ImVec2(config.size.x, config.size.y - 30);  // Account for tab bar height
...
bool PanelManager::remove_panel_from_tabbed_group(uint32_t tabbed_group_id,
                                                  uint32_t panel_to_remove_id) {
  // Check if both panels exist
  if (panels_.find(tabbed_group_id) == panels_.end() ||
  ...
  for (const auto& [id, panel] : panels_) {
    if (panel->get_config().type == PanelType::TABBED_GROUP) {
      if (auto* tabbed_panel = dynamic_cast<TabbedPanel*>(panel.get())) {
        ...
        if (std::find(tabbed_panels.begin(), tabbed_panels.end(), panel_id) !=
            tabbed_panels.end()) {
          return true;
        }
      }
...
bool PanelManager::can_drag_panel_to_target(uint32_t source_panel_id,
                                            uint32_t target_panel_id) const {
  // Check if both panels exist
...
std::pair<int, int> PanelManager::find_auto_dock_position(int width, int height) const {
    ...
    if (right_edge + width <= max_cols) {
      ...
      if (can_place) {
        ...
        if (full_fit) {
          ...
          int score =
              1000 - (right_edge + config.grid_y * 10);  // Prefer positions closer to top-left
    ...
    if (bottom_edge + height <= max_rows) {
      ...
      if (can_place) {
        ...
        if (full_fit) {
          ...
          int score =
              800 - (config.grid_x + bottom_edge * 10);  // Prefer positions closer to top-left
    ...
    if (left_edge >= 0) {
      ...
      if (can_place) {
        ...
        if (full_fit) {
          ...
          int score =
              600 - (left_edge + config.grid_y * 10);  // Prefer positions closer to top-left
  ...
  if (!potential_positions.empty()) {
    ...
    auto best_pos = std::max_element(
        potential_positions.begin(), potential_positions.end(),
    ...
    for (auto& [panel_id, panel] : panels_) {
      ...
      if (mouse_pos.x >= panel_pos.x && mouse_pos.x <= panel_pos.x + panel_size.x &&
        ...
        if (can_drag_panel_to_target(dragged_panel_id_, panel_id)) {
          ...
          draw_list->AddRect(p_min, p_max, IM_COL32(0, 255, 0, 255), 0.0f, 0,
                             6.0f);  // Thicker green border

          // Draw a semi-transparent overlay to highlight the area
          draw_list->AddRectFilled(p_min, p_max,
                                   IM_COL32(0, 255, 0, 50));  // Semi-transparent green fill

          // Draw a label indicating this is a valid drop zone
          std::string label = "Drop to create tabbed group";
          ImVec2 label_size = ImGui::CalcTextSize(label.c_str());
          ImVec2 label_pos = ImVec2(panel_pos.x + (panel_size.x - label_size.x) * 0.5f,
...
void PanelManager::propagate_symbol_to_linked_panels(uint32_t source_panel_id,
