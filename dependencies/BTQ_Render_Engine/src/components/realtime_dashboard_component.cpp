...
RealtimeDashboardComponent::RealtimeDashboardComponent(
    ...
    add_panel(DashboardPanelType::PRICE_CHART, "BTC-USDT Price", ImVec2(10, 10), ImVec2(800, 400));
    add_panel(DashboardPanelType::VOLUME_CHART, "Volume", ImVec2(820, 10), ImVec2(400, 200));
    add_panel(DashboardPanelType::INDICATORS, "Technical Indicators", ImVec2(10, 420), ImVec2(800, 200));
    add_panel(DashboardPanelType::ORDER_BOOK, "Order Book", ImVec2(820, 220), ImVec2(400, 400));
    add_panel(DashboardPanelType::RECENT_TRADES, "Recent Trades", ImVec2(10, 630), ImVec2(400, 200));
    add_panel(DashboardPanelType::MARKET_STATS, "Market Stats", ImVec2(420, 630), ImVec2(400, 200));
    add_panel(DashboardPanelType::PERFORMANCE_METRICS, "Performance", ImVec2(1230, 10), ImVec2(400, 300));
    add_panel(DashboardPanelType::MULTI_SYMBOL_OVERVIEW, "Multi-Symbol", ImVec2(1230, 320), ImVec2(400, 300));
}
...
void RealtimeDashboardComponent::add_panel(DashboardPanelType type, const std::string& title, ImVec2 pos, ImVec2 size) {
...
void RealtimeDashboardComponent::render_panel(const DashboardPanel& panel) {
    ...
    std::string window_title = panel.title + "###panel_" + std::to_string(&panel - &panels_[0]);
    if (ImGui::Begin(window_title.c_str(), nullptr, ImGuiWindowFlags_NoCollapse)) {
        switch (panel.type) {
...
void RealtimeDashboardComponent::render_price_chart_panel(const DashboardPanel& panel) {
    ...
    if (ImPlot::BeginPlot("Price Chart", ImVec2(-1, -1))) {
        ...
        // Plot candlesticks
        ImPlot::PlotCandlestick("BTC-USDT", timestamps.data(), opens.data(), highs.data(),
                               lows.data(), closes.data(), timestamps.size());

        ImPlot::EndPlot();
    }
}

void RealtimeDashboardComponent::render_volume_chart_panel(const DashboardPanel& panel) {
    auto ohlcv_data = get_ohlcv_data(panel.symbol, panel.timeframe, 500);
    ...
    if (ImPlot::BeginPlot("Volume", ImVec2(-1, -1))) {
        ...
        ImPlot::PlotBars("Volume", timestamps.data(), volumes.data(), timestamps.size());

        ImPlot::EndPlot();
    }
}

void RealtimeDashboardComponent::render_indicators_panel(const DashboardPanel& panel) {
    ...
    if (ImPlot::BeginPlot("Indicators", ImVec2(-1, -1))) {
        ...
        if (!rsi_result.values.empty()) {
            ...
            ImPlot::PlotLine("RSI", rsi_timestamps.data(), rsi_result.values.data(), rsi_timestamps.size());
        }
        ...
        if (macd_results.size() >= 2) {
            ...
            ImPlot::PlotLine("MACD", macd_timestamps.data(), macd_results[0].values.data(), macd_timestamps.size());
            ImPlot::PlotLine("Signal", macd_timestamps.data(), macd_results[1].values.data(), macd_timestamps.size());
        }
...
void RealtimeDashboardComponent::render_order_book_panel(const DashboardPanel& panel) {
    ...
    for (size_t i = 0; i < max_levels; ++i) {
        if (i < orderbook.bids.size()) {
            ImGui::Text("%.2f", orderbook.bids[i].price); ImGui::NextColumn();
            ImGui::Text("%.4f", orderbook.bids[i].size); ImGui::NextColumn();
        } else {
        ...
        if (i < orderbook.asks.size()) {
            ImGui::Text("%.2f", orderbook.asks[i].price); ImGui::NextColumn();
        } else {
            ImGui::Text(""); ImGui::NextColumn();
        }
    }
...
void RealtimeDashboardComponent::render_recent_trades_panel(const DashboardPanel& panel) {
    ...
    for (const auto& trade : trades) {
        auto time_t = std::chrono::system_clock::to_time_t(std::chrono::system_clock::time_point(std::chrono::milliseconds(trade.timestamp)));
        std::tm* tm = std::localtime(&time_t);
        char time_str[9];
        std::strftime(time_str, sizeof(time_str), "%H:%M:%S", tm);

        ImGui::Text("%s", time_str); ImGui::NextColumn();
        ImGui::Text("%.2f", trade.price); ImGui::NextColumn();
        ImGui::Text("%.4f", trade.size); ImGui::NextColumn();
        ImGui::TextColored(trade.is_buy ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), trade.is_buy ? "BUY" : "SELL"); ImGui::NextColumn();
    }
    ImGui::Columns(1);
}

void RealtimeDashboardComponent::render_market_stats_panel(const DashboardPanel& panel) {
    auto ohlcv_data = get_ohlcv_data(panel.symbol, RenderEngine::TimeFrame::TF_1DAY, 2);
    ...
    ImGui::TextColored(price_change >= 0 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1),
    ...
    ImGui::TextColored(volume_change >= 0 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1),
                      "Volume Change: %+.4f (%+.2f%%)", volume_change, volume_change_percent);
}

void RealtimeDashboardComponent::render_performance_metrics_panel(const DashboardPanel& panel) {
...
void RealtimeDashboardComponent::render_multi_symbol_overview_panel(const DashboardPanel& panel) {
    ...
    ImGui::Text("Price"); ImGui::NextColumn();
    ImGui::Text("24h Change"); ImGui::NextColumn();
    ...
    for (const auto& symbol : symbols) {
        ...
        ImGui::Text("%s", symbol.c_str()); ImGui::NextColumn();

        if (data.size() >= 1) {
            ImGui::Text("%.2f", data.back().close); ImGui::NextColumn();
            ...
            if (data.size() >= 2) {
                ...
                ImGui::TextColored(change >= 0 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1),
                                  "%+.2f%%", change_percent); ImGui::NextColumn();
                ImGui::Text("%.1fM", data.back().volume / 1000000.0); ImGui::NextColumn();
            } else {
    ...
    if (ImGui::Begin("Dashboard Configuration", &show_panel_config_)) {
        ...
        if (ImGui::BeginPopup("AddPanelPopup")) {
            if (ImGui::Selectable("Price Chart")) {
                add_panel(DashboardPanelType::PRICE_CHART, "New Price Chart", ImVec2(50, 50), ImVec2(400, 300));
            }
            if (ImGui::Selectable("Volume Chart")) {
                add_panel(DashboardPanelType::VOLUME_CHART, "New Volume Chart", ImVec2(50, 50), ImVec2(400, 200));
            }
...
std::vector<TechnicalIndicators::OHLCV> RealtimeDashboardComponent::get_ohlcv_data(const std::string& symbol, RenderEngine::TimeFrame timeframe, int limit) {
    ...
    // Get symbol ID
    auto symbol_id_opt = SymbolRegistry::instance().get_symbol_id(exchange, symbol_name);
    if (!symbol_id_opt) {
    ...
    size_t start = candles.size() > static_cast<size_t>(limit) ? candles.size() - limit : 0;
...
RenderEngine::OrderbookData RealtimeDashboardComponent::get_orderbook_data(const std::string& symbol) {
    // Parse symbol
...
std::vector<RenderEngine::TradeData> RealtimeDashboardComponent::get_recent_trades(const std::string& symbol, int limit) {
    // Parse symbol
    std::string exchange = "Binance";
    std::string symbol_name = symbol;
    ...
    size_t start = analytics.recent_trades.size() > static_cast<size_t>(limit) ?
                   analytics.recent_trades.size() - limit : 0;
