...
void TimeAndSalesPanel::checkForLargeTradesAndAlert() {
...
void TimeAndSalesPanel::playTradeAlertSound(bool is_buy) {
...
#ifdef __linux__
void TimeAndSalesPanel::generateAndPlayTone(int frequency, int duration_ms, const std::string& type) {
...
void TimeAndSalesPanel::writeInt16(std::ofstream& file, int16_t value) {
  file.put(value & 0xFF);
  file.put((value >> 8) & 0xFF);
}

void TimeAndSalesPanel::writeInt32(std::ofstream& file, int32_t value) {
  file.put(value & 0xFF);
  file.put((value >> 8) & 0xFF);
...
void TimeAndSalesPanel::render_panel_header() {
...
void TimeAndSalesPanel::render_controls() {
  ImGui::SameLine();
...
void TimeAndSalesPanel::render_search_controls() {
  ImGui::Separator();
...
void TimeAndSalesPanel::render_trade_size_histogram() {
  ImGui::Separator();
...
uint64_t TimeAndSalesPanel::parseTimeString(const std::string& time_str) {
...
void TimeAndSalesPanel::render_trade_table() {
  // Unique table ID per panel instance to avoid ID conflicts
  char table_id[64];
  snprintf(table_id, sizeof(table_id), "TimeAndSalesTable##%s", config_.title.c_str());
...
std::vector<TimeAndSalesPanel::LogBucket> TimeAndSalesPanel::computeLogarithmicTradeSizeHistogram(const std::vector<RenderEngine::TradeData>& trades) const {
...
bool TimeAndSalesPanel::isTradeClustered(int index, const std::vector<RenderEngine::TradeData>& trades) const {
...
double TimeAndSalesPanel::calculateTradesPerMinute(const std::vector<RenderEngine::TradeData>& trades, uint64_t window_microseconds) const {
    if (trades.empty()) {
        return 0.0;
    }
...
void TimeAndSalesPanel::updateTradePaceHistory() {
    if (cached_trades_.empty()) {
        return;
    }
...
void TimeAndSalesPanel::renderTradePaceChart() {
...
void TimeAndSalesPanel::exportTradesToCSV() {
    if (cached_trades_.empty()) {
        // Nothing to export
        return;
    }
