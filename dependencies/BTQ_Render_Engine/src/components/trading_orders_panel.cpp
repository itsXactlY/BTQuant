...
TradingOrdersPanel::TradingOrdersPanel(const PanelConfig &config, 
    ...
    : PanelBase(config), order_manager_(order_manager), 
      position_manager_(position_manager) {}

void TradingOrdersPanel::initialize() {
    ...
    if (ImGui::BeginTable("OrdersTable", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
        ...
        if (order_manager_) {
            ...
            for (const auto &order : orders) {
                ...
                std::string type_str;
                switch(order.type) {
                    case OrderManager::OrderType::Market: type_str = "Market"; break;
                    case OrderManager::OrderType::Limit: type_str = "Limit"; break;
                    case OrderManager::OrderType::Stop: type_str = "Stop"; break;
                ...
                switch(order.status) {
                    case OrderManager::OrderStatus::Pending: status_str = "Pending"; break;
                    case OrderManager::OrderStatus::PartiallyFilled: status_str = "Partial"; break;
                    case OrderManager::OrderStatus::Filled: status_str = "Filled"; break;
                    case OrderManager::OrderStatus::Cancelled: status_str = "Cancelled"; break;
