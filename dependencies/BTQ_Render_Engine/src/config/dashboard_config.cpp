...
bool DashboardConfig::loadConfiguration(const std::string& config_file) {
    ...
    if (!file.is_open()) {
        std::cout << "[DashboardConfig] Config file not found, using defaults: " << config_file_ << std::endl;
        return false;
    }
...
bool DashboardConfig::saveConfiguration() const {
    ...
    if (!file.is_open()) {
        std::cerr << "[DashboardConfig] Failed to open config file for writing: " << config_file_ << std::endl;
        return false;
    }
    ...
    try {
        ...
        file << "auto_reconnect=" << (data_source_config_.auto_reconnect ? "true" : "false") << std::endl;
        ...
        file << "monitoring_enabled=" << (performance_config_.monitoring_enabled ? "true" : "false") << std::endl;
        ...
        file << "latency_alert_threshold_ms=" << performance_config_.latency_alert_threshold_ms << std::endl;
        file << "memory_alert_threshold_mb=" << performance_config_.memory_alert_threshold_mb << std::endl;
        file << "enable_profiling=" << (performance_config_.enable_profiling ? "true" : "false") << std::endl;
        file << std::endl;
        ...
        file << "auto_save_layout=" << (user_preferences_.auto_save_layout ? "true" : "false") << std::endl;
...
// Getters
const DataSourceConfig& DashboardConfig::getDataSourceConfig() const {
    return data_source_config_;
}

const DisplayConfig& DashboardConfig::getDisplayConfig() const {
    return display_config_;
}

const ThemeConfig& DashboardConfig::getThemeConfig() const {
    return theme_config_;
}

const LayoutConfig& DashboardConfig::getLayoutConfig() const {
...
const UserPreferences& DashboardConfig::getUserPreferences() const {
...
void DashboardConfig::setDisplayConfig(const DisplayConfig& config) {
    display_config_ = config;
}

void DashboardConfig::setThemeConfig(const ThemeConfig& config) {
    theme_config_ = config;
}

void DashboardConfig::setLayoutConfig(const LayoutConfig& config) {
...
void DashboardConfig::enableAutoSave(bool enabled) {
...
void DashboardConfig::parseConfigValue(const std::string& section, const std::string& key, const std::string& value) {
    if (section == "data_source") {
        if (key == "hotspine_shm_name") data_source_config_.hotspine_shm_name = value;
        else if (key == "symbols_file") data_source_config_.symbols_file = value;
        else if (key == "auto_reconnect") data_source_config_.auto_reconnect = (value == "true");
        else if (key == "reconnect_interval_ms") data_source_config_.reconnect_interval_ms = std::stoul(value);
        else if (key == "max_symbols") data_source_config_.max_symbols = std::stoul(value);
    } else if (section == "display") {
        if (key == "window_width") display_config_.window_width = std::stoul(value);
        else if (key == "window_height") display_config_.window_height = std::stoul(value);
        else if (key == "fullscreen") display_config_.fullscreen = (value == "true");
        else if (key == "vsync") display_config_.vsync = (value == "true");
        else if (key == "target_fps") display_config_.target_fps = std::stoul(value);
        else if (key == "msaa_samples") display_config_.msaa_samples = std::stoul(value);
    } else if (section == "theme") {
        if (key == "name") theme_config_.name = value;
        else if (key == "background_color") theme_config_.background_color = parseColor(value);
        else if (key == "text_color") theme_config_.text_color = parseColor(value);
        else if (key == "accent_color") theme_config_.accent_color = parseColor(value);
        else if (key == "positive_color") theme_config_.positive_color = parseColor(value);
        else if (key == "negative_color") theme_config_.negative_color = parseColor(value);
        else if (key == "neutral_color") theme_config_.neutral_color = parseColor(value);
        else if (key == "grid_line_color") theme_config_.grid_line_color = parseColor(value);
        else if (key == "font_size") theme_config_.font_size = std::stoul(value);
        else if (key == "line_height") theme_config_.line_height = std::stof(value);
    } else if (section == "layout") {
        if (key == "grid_columns") layout_config_.grid_columns = std::stoul(value);
        else if (key == "grid_rows") layout_config_.grid_rows = std::stoul(value);
        else if (key == "show_grid") layout_config_.show_grid = (value == "true");
        else if (key == "show_heatmap") layout_config_.show_heatmap = (value == "true");
        else if (key == "show_charts") layout_config_.show_charts = (value == "true");
        else if (key == "show_orderbook") layout_config_.show_orderbook = (value == "true");
        else if (key == "show_logs") layout_config_.show_logs = (value == "true");
        else if (key == "show_performance") layout_config_.show_performance = (value == "true");
        else if (key == "panel_spacing") layout_config_.panel_spacing = std::stof(value);
        else if (key == "panel_padding") layout_config_.panel_padding = std::stof(value);
    } else if (section == "performance") {
        if (key == "monitoring_enabled") performance_config_.monitoring_enabled = (value == "true");
        else if (key == "monitoring_interval_ms") performance_config_.monitoring_interval_ms = std::stoul(value);
        else if (key == "history_size") performance_config_.history_size = std::stoul(value);
        else if (key == "fps_alert_threshold") performance_config_.fps_alert_threshold = std::stod(value);
        else if (key == "latency_alert_threshold_ms") performance_config_.latency_alert_threshold_ms = std::stod(value);
        else if (key == "memory_alert_threshold_mb") performance_config_.memory_alert_threshold_mb = std::stod(value);
        else if (key == "enable_profiling") performance_config_.enable_profiling = (value == "true");
    } else if (section == "user_preferences") {
        if (key == "default_exchange") user_preferences_.default_exchange = value;
        else if (key == "default_symbol") user_preferences_.default_symbol = value;
        else if (key == "auto_save_layout") user_preferences_.auto_save_layout = (value == "true");
        else if (key == "show_tooltips") user_preferences_.show_tooltips = (value == "true");
        else if (key == "animation_speed") user_preferences_.animation_speed = std::stof(value);
        else if (key == "update_frequency_hz") user_preferences_.update_frequency_hz = std::stod(value);
    }
}
