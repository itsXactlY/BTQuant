...
bool ExchangeAggregator::initialize() {
  ...
  BTQ_LOG_INFO_EX("ExchangeAggregator initialized successfully", "thread_id",
...
void ExchangeAggregator::processDataUpdate(const std::string& exchange, const std::string& symbol,
  ...
  if (!isValidData(update)) {
    BTQ_LOG_WARNING(
        std::format("Invalid data received from exchange {} for symbol {}", exchange, symbol));
    return;
  }
...
uint64_t ExchangeAggregator::calculateSynchronizedTimestamp(const std::string& symbol) const {
  ...
  switch (sync_strategy_) {
    ...
    case TimeSyncStrategy::ADAPTIVE_SYNC: {
      ...
      uint64_t mean_ts =
          std::accumulate(timestamps.begin(), timestamps.end(), 0ULL) / timestamps.size();
      uint64_t variance = 0;
      for (auto ts : timestamps) {
        variance +=
            (ts > mean_ts) ? (ts - mean_ts) * (ts - mean_ts) : (mean_ts - ts) * (mean_ts - ts);
      }
...
void ExchangeAggregator::synchronizeTimestamps(AggregatedMarketData& data) const {
  ...
  std::vector<std::pair<uint64_t, std::string>>
      timestamp_exchange_pairs;  // To track which exchange each timestamp belongs to
  ...
  switch (data.sync_strategy) {
    ...
    case TimeSyncStrategy::WINDOWED_SYNC: {
      ...
      if (!recent_timestamps.empty()) {
        data.synchronized_timestamp =
            std::accumulate(recent_timestamps.begin(), recent_timestamps.end(), 0ULL) /
            recent_timestamps.size();
      } else {
...
bool ExchangeAggregator::isValidData(const RenderEngine::MarketDataUpdate& data) const {
  ...
  if (std::isnan(data.price) || std::isnan(data.size) || std::isinf(data.price) ||
      std::isinf(data.size)) {
    return false;
  }
...
void ExchangeAggregator::calculateExchangeCorrelations(
    const std::string& symbol,
  ...
  // Calculate correlation matrix (for now just store individual z-scores as a proxy for
  // correlation)
  ...
  for (double z_score : z_scores) {
    correlation_sum +=
        z_score * 0.0;  // Correlation to mean is always 1.0, so we use 0.0 as baseline
  }
  result.overall_correlation =
      1.0 - (std::abs(correlation_sum) / z_scores.size());  // Simplified correlation measure
}

void ExchangeAggregator::detectArbitrageOpportunities(
    const std::unordered_map<std::string, RenderEngine::MarketDataUpdate>& exchange_data,
    AggregatedMarketData& result) const {
  if (exchange_data.size() < 2) {
...
std::optional<MultiExchangeData> ExchangeAggregator::getMultiExchangeView(
  ...
  for (const auto& [exchange, data] : multi_exchange_data.exchange_data) {
    ...
    if (!prices.empty()) {
      ...
      stats.percent_price_deviation =
          (avg_price > 0) ? (stats.price_deviation_from_avg / avg_price) * 100.0 : 0.0;
    }
...
void ExchangeAggregator::applyExchangeSpecificAdjustments(RenderEngine::MarketDataUpdate& data,
  ...
  if (features.latency_offset_us != 0.0) {
    ...
    data.timestamp = static_cast<uint64_t>(static_cast<int64_t>(data.timestamp) -
                                           static_cast<int64_t>(features.latency_offset_us));
  }
  ...
  if (features.trading_fee_rate > 0.0) {
    ...
    BTQ_LOG_DEBUG(std::format("Processing data from {} with {} fee rate", exchange,
                              features.trading_fee_rate));
  }
  ...
  if (features.max_order_size > 0) {
    ...
    if (data.size > features.max_order_size) {
      BTQ_LOG_WARNING(std::format(
          "Adjusting order size from {} to {} for exchange {} due to max order size limit",
  ...
  if (features.min_order_size > 0 && data.size < features.min_order_size) {
    BTQ_LOG_WARNING(std::format(
        "Adjusting order size from {} to {} for exchange {} due to min order size requirement",
  ...
  if (features.timezone != "UTC") {
    ...
    BTQ_LOG_DEBUG(
        std::format("Exchange {} operates in timezone {}, data timestamp adjustment may be needed",
...
void ExchangeAggregator::validateExchangeSpecificConstraints(
  ...
  if (!features.supported_symbols.empty()) {
    bool symbol_supported =
        std::find(features.supported_symbols.begin(), features.supported_symbols.end(), symbol) !=
  ...
  if (!features.supported_data_types.empty()) {
    ...
    BTQ_LOG_DEBUG(
        std::format("Checking if exchange {} supports data type for symbol {}", exchange, symbol));
  }
}
...
void ExchangeAggregator::updateExchangeSpecificFeatures(const std::string& exchange,
  ...
  BTQ_LOG_INFO(std::format("Updated features for exchange {}: latency_offset={}us, reliability={}",
                           exchange, new_features.latency_offset_us,
                           new_features.reliability_score));
...
ExchangeAggregator::calculateDataQualityMetrics(const std::string& symbol) const {
    ...
    if (isExchangeDataValid(exchange, data)) {
      ...
      if (last_update_it != exchange_last_update_.end()) {
        ...
        exchange_quality.freshness_score =
            std::exp(-age_ms / 1000.0);  // Exponential decay over 1 second
      }
      ...
      exchange_quality.overall_quality_score =
          (exchange_quality.freshness_score + exchange_quality.completeness_score +
  ...
  if (prices.size() < 2) {
    ...
    quality_metrics.data_reliability_score =
        prices.empty() ? 0.0 : 0.8;  // Assume moderate reliability
  } else {
    ...
    quality_metrics.data_reliability_score =
        (quality_metrics.cross_exchange_consistency + avg_quality) / 2.0;
  }
...
ExchangeAggregator::getMultiExchangeConsolidatedView(const std::string& symbol) const {
  ...
  for (const auto& [exchange, data] : symbol_it->second) {
    if (isExchangeDataValid(exchange, data)) {
      ...
      if (!all_prices.empty()) {
        double avg_price =
            std::accumulate(all_prices.begin(), all_prices.end(), 0.0) / all_prices.size();
        exchange_data.stats.price_deviation_from_avg = data.price - avg_price;
        exchange_data.stats.percent_price_deviation =
            (avg_price > 0) ? (exchange_data.stats.price_deviation_from_avg / avg_price) * 100.0
                            : 0.0;
        ...
        exchange_data.stats.is_outlier =
            (std::abs(exchange_data.stats.price_deviation_from_avg) > 2 * std_dev);
      }
  ...
  if (!prices.empty()) {
    ...
    consolidated_view.market_metrics.volatility =
        consolidated_view.market_metrics.spread / prices.front();
  ...
  if (prices.size() > 1) {
    ...
    consolidated_view.risk_metrics.coefficient_of_variation =
        (avg_price > 0) ? consolidated_view.risk_metrics.price_volatility / avg_price : 0.0;
  }
  ...
  if (consolidated_view.exchange_data.size() > 1) {
    ...
    if (correlation_count > 0) {
      consolidated_view.market_metrics.cross_exchange_correlation =
          total_correlation / correlation_count;
    }
  }
...
ExchangeAggregator::getCrossExchangeAnalytics(const std::string& symbol) const {
  ...
  if (analytics.max_price > 0 && analytics.min_price > 0) {
    ...
    analytics.relative_arbitrage_potential =
        (analytics.max_arbitrage_potential / analytics.mean_price) * 100.0;
  }
...
ExchangeAggregator::performAdvancedAggregation(const std::string& symbol) const {
  ...
  if (!prices.empty()) {
    ...
    result.dispersion_metrics.coefficient_of_variation =
        (mean > 0) ? result.dispersion_metrics.standard_deviation / mean : 0.0;
  }
...
void ExchangeAggregator::handleExchangeSpecificFeatures(
  ...
  if (update.size > features.max_order_size) {
    BTQ_LOG_WARNING(std::format("Order size {} exceeds max allowed {} for exchange {}", update.size,
                                features.max_order_size, exchange));
  }

  if (update.size < features.min_order_size) {
    BTQ_LOG_WARNING(std::format("Order size {} below min allowed {} for exchange {}", update.size,
                                features.min_order_size, exchange));
  }
...
std::optional<AggregatedMarketData> ExchangeAggregator::getExchangeSpecificAggregatedData(
  ...
  // Calculate aggregated values using enhanced methods
  aggregated_data.aggregated_price =
      calculateWeightedAveragePriceWithValidationForExchanges(symbol, exchanges);
...
std::optional<AggregatedMarketData> ExchangeAggregator::getUnifiedView(
    const std::string& symbol) const {
...
std::optional<AggregatedMarketData> ExchangeAggregator::getAdvancedAggregatedData(
  ...
  for (const auto& [exchange, data] : symbol_it->second) {
    ...
    bool include_exchange = exchanges.empty() || std::find(exchanges.begin(), exchanges.end(),
                                                           exchange) != exchanges.end();
...
std::optional<ComprehensiveExchangeView> ExchangeAggregator::getComprehensiveExchangeView(
  ...
  for (const auto& [exchange, data] : symbol_it->second) {
    if (isExchangeDataValid(exchange, data)) {
      ...
      if (!all_prices.empty()) {
        ...
        detailed_data.stats.percent_price_deviation =
            (avg_price > 0) ? (detailed_data.stats.price_deviation_from_avg / avg_price) * 100.0
                            : 0.0;
        ...
        detailed_data.stats.is_outlier =
            (std::abs(detailed_data.stats.price_deviation_from_avg) > 2 * std_dev);
      }
  ...
  if (!prices.empty()) {
    ...
    comprehensive_view.market_metrics.volatility =
        comprehensive_view.market_metrics.spread / prices.front();
...
ExchangeAggregator::synchronizeTimestampsAcrossExchanges(const std::string& symbol,
  ...
  for (auto& [timestamp, exchange] : compensated_pairs) {
    ...
    if (features_it != exchange_features_.end()) {
      ...
      if (offset != 0.0) {
        ...
        timestamp =
            static_cast<uint64_t>(static_cast<int64_t>(timestamp) + static_cast<int64_t>(offset));
      }
    }
  }
  ...
  for (const auto& [original_ts, exchange] : timestamp_exchange_pairs) {
    int64_t diff =
        static_cast<int64_t>(result.synchronized_timestamp) - static_cast<int64_t>(original_ts);
  ...
  for (const auto& [symbol, exchange_data_map] : exchange_data_) {
    ...
    for (size_t i = 0; i < exchange_updates.size(); ++i) {
      for (size_t j = i + 1; j < exchange_updates.size(); ++j) {
        ...
        uint64_t ts_diff =
            std::abs(static_cast<int64_t>(data1.timestamp) - static_cast<int64_t>(data2.timestamp));
        double timing_correlation =
            std::exp(-static_cast<double>(ts_diff) / 1000000.0);  // Decay factor of 1 second

        // Calculate volume correlation
        double volume_diff = std::abs(data1.size - data2.size);
        double avg_volume = (data1.size + data2.size) / 2.0;
        double volume_correlation =
            (avg_volume > 0) ? 1.0 - std::min(1.0, volume_diff / avg_volume) : 1.0;

        // Combined correlation
        double combined_correlation =
            (price_correlation + timing_correlation + volume_correlation) / 3.0;
...
std::optional<ExchangeRiskMetrics> ExchangeAggregator::calculateRiskMetrics(
    ...
    if (isExchangeDataValid(exchange, data)) {
      ...
      if (features_it != exchange_features_.end()) {
        ...
        exchange_risk.latency_risk =
            features_it->second.latency_offset_us / 1000.0;  // Convert to ms
  ...
  if (prices.size() > 1) {
    ...
    // Calculate coefficient of variation
    risk_metrics.coefficient_of_variation =
        (avg_price > 0) ? risk_metrics.price_volatility / avg_price : 0.0;

    // Calculate price spread across exchanges
    double min_price = *std::min_element(prices.begin(), prices.end());
    double max_price = *std::max_element(prices.begin(), prices.end());
    risk_metrics.price_spread = max_price - min_price;
    risk_metrics.normalized_price_spread =
        (avg_price > 0) ? risk_metrics.price_spread / avg_price : 0.0;

    // Calculate additional risk metrics
    risk_metrics.price_variance = variance;
    risk_metrics.max_price_deviation =
        std::max(std::abs(max_price - avg_price), std::abs(min_price - avg_price));
  }
...
std::optional<MultiExchangeTimeSyncResult> ExchangeAggregator::performMultiExchangeTimeSync(
  ...
  for (const auto& [exchange, data] : symbol_it->second) {
    if (isExchangeDataValid(exchange, data)) {
      ...
      if (features_it != exchange_features_.end()) {
        ...
        if (offset != 0.0) {
          // Compensate by adding the offset to align with a reference time
          compensated_timestamp = static_cast<uint64_t>(static_cast<int64_t>(data.timestamp) +
  ...
  result.average_original_timestamp =
      std::accumulate(timestamps.begin(), timestamps.end(), 0ULL) / timestamps.size();
...
ExchangeAggregator::getComprehensiveMultiExchangeView(const std::string& symbol) const {
  ...
  if (!prices.empty()) {
    ...
    view.market_metrics.median_price =
        (prices.size() % 2 == 0) ? (prices[prices.size() / 2 - 1] + prices[prices.size() / 2]) / 2.0
  ...
  if (prices.size() > 1) {
    ...
    view.risk_metrics.coefficient_of_variation =
        (avg_price > 0) ? view.risk_metrics.price_volatility / avg_price : 0.0;
  }
...
void ExchangeAggregator::calculateEnhancedRiskMetrics(
  ...
  for (const auto& [exchange, data] : exchange_data) {
    ...
    result.exchange_correlations[exchange] =
        1.0 - price_deviation;  // Lower deviation = higher correlation
  }
}
...
void ExchangeAggregator::handleEnhancedExchangeSpecificFeatures(
  ...
  if (features.max_order_size > 0 && update.size > features.max_order_size) {
    BTQ_LOG_WARNING(std::format(
        "Normalizing order size from {} to {} for exchange {} due to max order size limit",
  ...
  if (features.min_order_size > 0 && update.size < features.min_order_size) {
    BTQ_LOG_WARNING(std::format(
        "Normalizing order size from {} to {} for exchange {} due to min order size requirement",
  ...
  if (features.trading_fee_rate > 0.0) {
    ...
    // This is a simplified approach - in practice, you might want to adjust differently based on
    // direction
    update.price = update.price * (1.0 + features.trading_fee_rate /
                                             2.0);  // Half the fee rate as a simple adjustment
  }
  ...
  if (features.api_endpoint != "") {
    ...
    BTQ_LOG_DEBUG(std::format("Processing data from exchange {} via endpoint {}", exchange,
...
bool ExchangeAggregator::performExchangeSpecificQualityChecks(
  ...
  if (features.reliability_score < 0.1) {
    BTQ_LOG_WARNING(
        std::format("Exchange {} has low reliability score ({}), rejecting data for symbol {}",
...
void ExchangeAggregator::processEnhancedDataUpdate(const std::string& exchange,
  ...
  if (!performExchangeSpecificQualityChecks(exchange, symbol, update)) {
    BTQ_LOG_WARNING(
        std::format("Data failed exchange-specific quality checks for exchange {} and symbol {}",
                    exchange, symbol));
    return;
  }
  ...
  for (const auto& [exchange, data] : symbol_it->second) {
    if (isExchangeDataValid(exchange, data)) {
      ...
      if (!all_prices.empty()) {
        ...
        exchange_data_item.stats.percent_price_deviation =
            (avg_price > 0)
                ? (exchange_data_item.stats.price_deviation_from_avg / avg_price) * 100.0
                : 0.0;
        ...
        exchange_data_item.stats.is_outlier =
            (std::abs(exchange_data_item.stats.price_deviation_from_avg) > 2 * std_dev);
      }
...
void ExchangeAggregator::handleAdvancedExchangeSpecificFeatures(
  ...
  if (features.trading_fee_rate > 0.0) {
    ...
    if (update.side == "BUY") {
      ...
      update.price =
          update.price * (1.0 - features.trading_fee_rate);  // Fees decrease selling revenue
    }
  }
  ...
  if (features.withdrawal_fee > 0.0) {
    ...
    BTQ_LOG_DEBUG(
        std::format("Exchange {} has withdrawal fee of {}, factoring into risk calculations",
  ...
  if (timestamp_exchange_pairs.size() >= 2) {
    ...
    for (size_t i = 0; i < timestamp_exchange_pairs.size(); ++i) {
      for (size_t j = i + 1; j < timestamp_exchange_pairs.size(); ++j) {
        ...
        double correlation =
            std::exp(-static_cast<double>(diff) / 1000000.0);  // Decay factor of 1 second
...
std::optional<AggregatedMarketData> ExchangeAggregator::getUltimateAggregatedData(
  ...
  if (advanced_result.has_value()) {
    aggregated_data.aggregated_price =
        advanced_result->consensus_price;  // Use the most robust estimate
  }
...
ExchangeAggregator::getAdvancedUnifiedMultiExchangeView(const std::string& symbol) const {
  ...
  for (const auto& [exchange, data] : symbol_it->second) {
    if (isExchangeDataValid(exchange, data)) {
      ...
      // Store detailed exchange data with enhanced features
      ExchangeConsolidatedData exchange_data_item;
      exchange_data_item.update = data;
  ...
  // Calculate consolidated market metrics with enhanced analytics
  std::vector<double> prices;
  std::vector<double> volumes;
  double total_volume = 0.0;
  ...
  if (!prices.empty()) {
    ...
    view.market_metrics.coefficient_of_variation = (avg_price > 0) ? std_dev / avg_price : 0.0;
    ...
    if (prices.size() >= 3) {
      ...
      view.market_metrics.skewness = skewness_sum / prices.size();
    }
  }
  ...
  // Perform comprehensive time synchronization with enhanced features
...
// Enhanced method to process data updates with comprehensive exchange-specific feature handling
...
// Enhanced method to get a comprehensive multi-exchange view with all analytics
std::optional<ExchangeAggregator::ComprehensiveMultiExchangeView>
...
ExchangeAggregator::getEnhancedMultiExchangeView(const std::string& symbol) const {
  ...
  if (!prices.empty()) {
    ...
    // Calculate additional statistical metrics
    enhanced_view.market_metrics.median_price =
        (prices.size() % 2 == 0) ? (prices[prices.size() / 2 - 1] + prices[prices.size() / 2]) / 2.0
                                 : prices[prices.size() / 2];
    ...
    std_dev = std::sqrt(std_dev / prices.size());
    enhanced_view.market_metrics.coefficient_of_variation =
        (avg_price > 0) ? std_dev / avg_price : 0.0;
  ...
  if (prices.size() > 1) {
    ...
    enhanced_view.risk_metrics.coefficient_of_variation =
        (avg_price > 0) ? enhanced_view.risk_metrics.price_volatility / avg_price : 0.0;
  }
  ...
  if (enhanced_view.exchange_data.size() > 1) {
    ...
    if (correlation_count > 0) {
      enhanced_view.market_metrics.cross_exchange_correlation =
          total_correlation / correlation_count;
    }
  }
...
ExchangeAggregator::getEnhancedAggregatedDataWithAnalytics(const std::string& symbol) const {
  ...
  // Enhanced time synchronization - calculate timestamp from exchange data
  ...
  enhanced_data.synchronized_timestamp = latest_timestamp;
  ...
  double bid_price = calculateBestBid(valid_exchange_data);
  double ask_price = calculateBestAsk(valid_exchange_data);
  if (bid_price > ask_price && ask_price > 0) {
    enhanced_data.arbitrage_opportunity = true;
    enhanced_data.arbitrage_profit = bid_price - ask_price;
  }

  // Calculate overall correlation
  enhanced_data.overall_correlation = 1.0;  // Default to perfectly correlated
  ...
  if (!prices.empty()) {
    ...
    enhanced_data.statistical_metrics.price_range =
        enhanced_data.statistical_metrics.max_price - enhanced_data.statistical_metrics.min_price;
    enhanced_data.statistical_metrics.coefficient_of_variation =
        (mean > 0) ? enhanced_data.statistical_metrics.std_deviation / mean : 0.0;
    ...
    enhanced_data.statistical_metrics.median_price =
        (n % 2 == 0) ? (sorted_prices[n / 2 - 1] + sorted_prices[n / 2]) / 2.0
                     : sorted_prices[n / 2];
  }
...
// Enhanced method to handle sophisticated exchange-specific features
void ExchangeAggregator::handleSophisticatedExchangeSpecificFeatures(
  ...
  // Apply exchange-specific fee adjustments based on market conditions
  if (features.trading_fee_rate > 0.0) {
    // Adjust price to account for fees - this could be bid/ask specific
    if (update.side == "BUY") {
      update.price = update.price * (1.0 + features.trading_fee_rate);  // Fees increase buying cost
    ...
    // Apply fee-based risk adjustment
    BTQ_LOG_DEBUG(std::format("Applied {}% fee adjustment for exchange {} on symbol {}",
                              features.trading_fee_rate * 100, exchange, symbol));
  }
  ...
  // Apply dynamic reliability-based adjustments
  if (features.reliability_score < 0.5) {
    // For lower reliability exchanges, we might want to apply additional filtering
    BTQ_LOG_DEBUG(
        std::format("Exchange {} has low reliability ({}), applying additional validation",
                    exchange, features.reliability_score));
  }
...
std::optional<AggregatedMarketData> ExchangeAggregator::getAdvancedMultiExchangeAggregatedData(
  ...
  // Copy valid exchange data to aggregated data
  aggregated_data.exchange_data = valid_exchange_data;

  // Calculate various aggregated metrics
  aggregated_data.aggregated_price = calculateSimpleAveragePrice(valid_exchange_data);
  aggregated_data.weighted_price = calculateVolumeWeightedPrice(valid_exchange_data);
  aggregated_data.vwap = calculateVWAP(valid_exchange_data);
  aggregated_data.median_price = calculateMedianPrice(valid_exchange_data);
  aggregated_data.trimmed_mean_price = calculateTrimmedMean(valid_exchange_data, 0.1);
