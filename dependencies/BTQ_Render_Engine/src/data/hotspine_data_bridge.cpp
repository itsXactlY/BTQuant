...
HotSpineDataBridge::HotSpineDataBridge(const std::string& shm_name, const std::string& symbols_file)
    : shm_name_(shm_name)
    , symbols_file_(symbols_file)
    , running_(false)
    , last_trade_count_(0)
    , last_orderbook_count_(0)
    , total_trades_processed_(0)
    , total_orderbooks_processed_(0)
    , data_latency_us_(0)
    , update_frequency_hz_(0.0)
{
    ...
    try {
        ...
        std::cerr << "[HotSpineDataBridge] Failed to initialize: " << e.what() << std::endl;
        throw;
    }
...
bool HotSpineDataBridge::start() {
    ...
    perf_thread_ = std::thread(&HotSpineDataBridge::performanceMonitoringLoop, this);
    
    std::cout << "[HotSpineDataBridge] Started data processing threads" << std::endl;
    return true;
}
    ...
    // Add to trade buffer
    {
        ...
        if (trade_buffer_.size() > MAX_BUFFER_SIZE) {
            trade_buffer_.erase(trade_buffer_.begin(), 
                               trade_buffer_.begin() + (trade_buffer_.size() - MAX_BUFFER_SIZE));
        }
    }
...
void HotSpineDataBridge::processOrderbook(const HotSpine::HotOrderbookSnapshot& orderbook) {
    ...
    // Add to orderbook buffer
    {
        ...
        // Limit buffer size
        if (orderbook_buffer_.size() > MAX_BUFFER_SIZE / 10) {  // Smaller buffer for orderbooks
            orderbook_buffer_.erase(orderbook_buffer_.begin(), 
                                   orderbook_buffer_.begin() + (orderbook_buffer_.size() - MAX_BUFFER_SIZE / 10));
        }
    }
...
void HotSpineDataBridge::updateSymbolMarketData(uint32_t symbol_id, double price, double size, uint64_t timestamp) {
    std::lock_guard<std::mutex> lock(symbol_data_mutex_);
    ...
    data.momentum = data.momentum_history.empty() ? 0.0 : total_momentum / data.momentum_history.size();
}

void HotSpineDataBridge::updateSymbolOrderbookData(uint32_t symbol_id, double bid_price, double ask_price, uint64_t timestamp) {
    std::lock_guard<std::mutex> lock(symbol_data_mutex_);
    ...
    data.spread_percent = (bid_price > 0) ? (data.spread / bid_price) * 100.0 : 0.0;
    data.last_update_time = timestamp;
}

void HotSpineDataBridge::performanceMonitoringLoop() {
    std::cout << "[HotSpineDataBridge] Performance monitoring loop started" << std::endl;
    ...
    while (running_) {
        ...
        if (elapsed.count() > 0) {
            ...
            double trades_per_sec = (current_trades - last_trades) * 1000.0 / elapsed.count();
            double orderbooks_per_sec = (current_orderbooks - last_orderbooks) * 1000.0 / elapsed.count();
    ...
    std::cout << "[HotSpineDataBridge] Performance monitoring loop stopped" << std::endl;
}
...
void HotSpineDataBridge::printStatistics() {
    ...
    std::cout << "  Trades processed: " << performance_metrics_.total_trades_processed << std::endl;
    std::cout << "  Orderbooks processed: " << performance_metrics_.total_orderbooks_processed << std::endl;
    std::cout << "  Trades/sec: " << performance_metrics_.trades_per_second << std::endl;
    std::cout << "  Orderbooks/sec: " << performance_metrics_.orderbooks_per_second << std::endl;
    std::cout << "  Avg processing latency: " << performance_metrics_.avg_processing_latency_us << " µs" << std::endl;
    std::cout << "  Buffer utilization: " << performance_metrics_.buffer_utilization_percent << "%" << std::endl;
    std::cout << "  Connection healthy: " << (performance_metrics_.connection_healthy ? "Yes" : "No") << std::endl;
}
...
bool HotSpineDataBridge::reconnect() {
    ...
    try {
        ...
        std::cerr << "[HotSpineDataBridge] Reconnection failed: " << e.what() << std::endl;
        return false;
    }
}
...
void HotSpineDataBridge::reloadSymbolMappings() {
    if (!symbols_file_.empty()) {
        std::cout << "[HotSpineDataBridge] Reloading symbol mappings from " << symbols_file_ << std::endl;
        symbol_registry_->load_from_file(symbols_file_);
    }
}
