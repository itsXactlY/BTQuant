...
void processTradeIncrementally(SymbolAnalytics& symbol_data, const TradeData& trade,
    ...
    if (symbol_data.price_max > symbol_data.price_min) {
        symbol_data.price_position =
            ((trade.price - symbol_data.price_min) / (symbol_data.price_max - symbol_data.price_min)) * 100.0;
    }

    // Update trade size metrics incrementally
    symbol_data.avg_trade_size =
        (symbol_data.avg_trade_size * (symbol_data.trade_count - 1) + trade.size) / symbol_data.trade_count;
    ...
    if (symbol_data.log_returns.size() >= 2) {
        double mean = std::accumulate(symbol_data.log_returns.begin(),
                                    symbol_data.log_returns.end(), 0.0) /
