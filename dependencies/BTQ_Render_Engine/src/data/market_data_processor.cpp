...
MarketSummary MarketDataProcessor::getMarketSummary() const {
  ...
  for (auto& shard_ptr : shards_) {
    ...
    for (const auto& pair : shard_ptr->data) {
      ...
      if (pair.second.momentum > 0.01) summary.trending_up++;
      else if (pair.second.momentum < -0.01) summary.trending_down++;
...
uint64_t MarketDataProcessor::getTimeFrameDuration(TimeFrame timeframe) {
  switch (timeframe) {
    case TimeFrame::TF_1MS: return 1000;
    case TimeFrame::TF_10MS: return 10000;
    case TimeFrame::TF_100MS: return 100000;
    case TimeFrame::TF_500MS: return 500000;
    case TimeFrame::TF_1SEC: return 1000000;
    case TimeFrame::TF_3SEC: return 3000000;
    case TimeFrame::TF_5SEC: return 5000000;
    case TimeFrame::TF_15SEC: return 15000000;
...
void MarketDataProcessor::clearIndicatorCache(uint32_t symbol_id, const std::string& indicator_name) {
...
void MarketDataProcessor::updateVWAP(SymbolAnalytics& symbol_data) {
  ...
  for (size_t i = symbol_data.recent_trades.size() - count; i < symbol_data.recent_trades.size(); ++i) {
  ...
  if (total_volume > 0.0) {
    ...
    symbol_data.vwap_deviation = symbol_data.last_trade_price > 0.0 ?
        ((symbol_data.last_trade_price - symbol_data.vwap) / symbol_data.vwap) * 100.0 : 0.0;
  }
}
...
void MarketDataProcessor::updateVolatility(SymbolAnalytics& symbol_data) {
  ...
  for (size_t i = symbol_data.recent_trades.size() - count + 1; i < symbol_data.recent_trades.size(); ++i) {
...
void MarketDataProcessor::updateTradingMetrics(SymbolAnalytics& symbol_data, const TradeData& trade) {
  ...
  symbol_data.buy_sell_ratio = (symbol_data.buy_count + symbol_data.sell_count) > 0 ?
      static_cast<double>(symbol_data.buy_count) / (symbol_data.buy_count + symbol_data.sell_count) : 0.5;
  ...
  if (symbol_data.price_max > symbol_data.price_min) {
    symbol_data.price_position = ((trade.price - symbol_data.price_min) /
                                 (symbol_data.price_max - symbol_data.price_min)) * 100.0;
  }

  // Update average trade size
  symbol_data.avg_trade_size = (symbol_data.avg_trade_size * (symbol_data.trade_count - 1) + trade.size) /
                               symbol_data.trade_count;
...
void MarketDataProcessor::updateSpreadAnalysis(SymbolAnalytics& symbol_data) {
  ...
  for (size_t i = symbol_data.recent_orderbooks.size() - count; i < symbol_data.recent_orderbooks.size(); ++i) {
...
void MarketDataProcessor::updateCandles(SymbolAnalytics& symbol_data, const TradeData& trade) {
  for (auto timeframe : {TimeFrame::TF_1MS, TimeFrame::TF_10MS, TimeFrame::TF_100MS,
...
void MarketDataProcessor::updateCandleForTimeframe(SymbolAnalytics& symbol_data, const TradeData& trade, TimeFrame timeframe) {
...
bool MarketDataProcessor::isTradeInCurrentCandle(const OHLCVCandle& candle, uint64_t trade_timestamp, TimeFrame timeframe) const {
  uint64_t duration_us = getTimeFrameDuration(timeframe);
...
void MarketDataProcessor::processUpdate(const MarketDataUpdate& update) {
  ...
  if (update.type == MarketDataType::TRADE) {
    ...
    orderbook.imbalance = orderbook.total_depth > 0.0 ?
        (orderbook.bid_depth - orderbook.ask_depth) / orderbook.total_depth : 0.0;
