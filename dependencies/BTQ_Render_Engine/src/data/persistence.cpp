...
DataPersistence::~DataPersistence() { stopPeriodicSave(); }

bool DataPersistence::saveAggregatedData(
    const std::shared_ptr<RenderEngine::CacheManager>& cache_manager, const std::string& filename) {
  if (!cache_manager) {
  ...
  if (!file.is_open()) {
    std::cerr << "[DataPersistence] Error: Could not open file for writing: " << filepath
              << std::endl;
    return false;
  }
  ...
  if (success && file.good()) {
    ...
    std::cout << "[DataPersistence] Successfully saved aggregated data to: " << filepath
              << std::endl;
    return true;
  } else {
    std::cerr << "[DataPersistence] Error: Failed to save aggregated data to: " << filepath
              << std::endl;
    return false;
  }
}
...
bool DataPersistence::loadAggregatedData(
  ...
  if (!file.is_open()) {
    std::cerr << "[DataPersistence] Error: Could not open file for reading: " << filepath
              << std::endl;
    return false;
  }
  ...
  if (success && file.good()) {
    std::cout << "[DataPersistence] Successfully loaded aggregated data from: " << filepath
              << std::endl;
    return true;
  } else {
    std::cerr << "[DataPersistence] Error: Failed to load aggregated data from: " << filepath
              << std::endl;
    return false;
  }
}
...
bool DataPersistence::saveMarketAnalytics(
  ...
  for (uint32_t symbol_id : active_symbols) {
    ...
    file.write(reinterpret_cast<const char*>(&analytics.last_update_time),
               sizeof(analytics.last_update_time));
    file.write(reinterpret_cast<const char*>(&analytics.trade_count),
               sizeof(analytics.trade_count));
    file.write(reinterpret_cast<const char*>(&analytics.last_trade_price),
               sizeof(analytics.last_trade_price));
    file.write(reinterpret_cast<const char*>(&analytics.last_trade_size),
               sizeof(analytics.last_trade_size));
    file.write(reinterpret_cast<const char*>(&analytics.last_trade_time),
               sizeof(analytics.last_trade_time));
    ...
    file.write(reinterpret_cast<const char*>(&analytics.sell_volume),
               sizeof(analytics.sell_volume));
    ...
    file.write(reinterpret_cast<const char*>(&analytics.buy_sell_ratio),
               sizeof(analytics.buy_sell_ratio));

    // Write price analytics
    file.write(reinterpret_cast<const char*>(&analytics.vwap), sizeof(analytics.vwap));
    file.write(reinterpret_cast<const char*>(&analytics.vwap_deviation),
               sizeof(analytics.vwap_deviation));
    file.write(reinterpret_cast<const char*>(&analytics.momentum), sizeof(analytics.momentum));
    file.write(reinterpret_cast<const char*>(&analytics.momentum_strength),
               sizeof(analytics.momentum_strength));
    ...
    file.write(reinterpret_cast<const char*>(&analytics.price_position),
               sizeof(analytics.price_position));

    // Write volatility analytics
    file.write(reinterpret_cast<const char*>(&analytics.volatility), sizeof(analytics.volatility));
    file.write(reinterpret_cast<const char*>(&analytics.sharpe_ratio),
               sizeof(analytics.sharpe_ratio));

    // Write trade size analytics
    file.write(reinterpret_cast<const char*>(&analytics.avg_trade_size),
               sizeof(analytics.avg_trade_size));
    file.write(reinterpret_cast<const char*>(&analytics.large_trade_count),
               sizeof(analytics.large_trade_count));

    // Write orderbook analytics
    file.write(reinterpret_cast<const char*>(&analytics.current_spread),
               sizeof(analytics.current_spread));
    file.write(reinterpret_cast<const char*>(&analytics.current_spread_percent),
               sizeof(analytics.current_spread_percent));
    file.write(reinterpret_cast<const char*>(&analytics.avg_spread), sizeof(analytics.avg_spread));
    file.write(reinterpret_cast<const char*>(&analytics.avg_spread_percent),
               sizeof(analytics.avg_spread_percent));
    file.write(reinterpret_cast<const char*>(&analytics.current_imbalance),
               sizeof(analytics.current_imbalance));
    file.write(reinterpret_cast<const char*>(&analytics.avg_imbalance),
               sizeof(analytics.avg_imbalance));
    file.write(reinterpret_cast<const char*>(&analytics.market_depth),
               sizeof(analytics.market_depth));
    ...
    for (int tf = 0; tf < static_cast<int>(RenderEngine::TimeFrame::TF_1WEEK) + 1; ++tf) {
      ...
      if (candles_it != analytics.candles.end()) {
        for (const auto& candle : candles_it->second) {
          ...
          file.write(reinterpret_cast<const char*>(&candle.trade_count),
                     sizeof(candle.trade_count));
        }
      }
      ...
      if (has_current_candle) {
        ...
        file.write(reinterpret_cast<const char*>(&current_candle.timestamp),
                   sizeof(current_candle.timestamp));
        file.write(reinterpret_cast<const char*>(&current_candle.open),
                   sizeof(current_candle.open));
        file.write(reinterpret_cast<const char*>(&current_candle.high),
                   sizeof(current_candle.high));
        file.write(reinterpret_cast<const char*>(&current_candle.low), sizeof(current_candle.low));
        file.write(reinterpret_cast<const char*>(&current_candle.close),
                   sizeof(current_candle.close));
        file.write(reinterpret_cast<const char*>(&current_candle.volume),
                   sizeof(current_candle.volume));
        file.write(reinterpret_cast<const char*>(&current_candle.trade_count),
                   sizeof(current_candle.trade_count));
      }
    }
  ...
  if (file.good()) {
    last_saved_filename_ = output_filename;
    std::cout << "[DataPersistence] Successfully saved market analytics to: " << filepath << " for "
              << symbol_count << " symbols" << std::endl;
    return true;
  } else {
...
bool DataPersistence::loadMarketAnalytics(
  ...
  for (size_t i = 0; i < symbol_count; ++i) {
    ...
    file.read(reinterpret_cast<char*>(&analytics.last_update_time),
              sizeof(analytics.last_update_time));
    file.read(reinterpret_cast<char*>(&analytics.trade_count), sizeof(analytics.trade_count));
    file.read(reinterpret_cast<char*>(&analytics.last_trade_price),
              sizeof(analytics.last_trade_price));
    file.read(reinterpret_cast<char*>(&analytics.last_trade_size),
              sizeof(analytics.last_trade_size));
    file.read(reinterpret_cast<char*>(&analytics.last_trade_time),
              sizeof(analytics.last_trade_time));
    ...
    file.read(reinterpret_cast<char*>(&analytics.momentum_strength),
              sizeof(analytics.momentum_strength));
    ...
    file.read(reinterpret_cast<char*>(&analytics.large_trade_count),
              sizeof(analytics.large_trade_count));

    // Read orderbook analytics
    file.read(reinterpret_cast<char*>(&analytics.current_spread), sizeof(analytics.current_spread));
    file.read(reinterpret_cast<char*>(&analytics.current_spread_percent),
              sizeof(analytics.current_spread_percent));
    file.read(reinterpret_cast<char*>(&analytics.avg_spread), sizeof(analytics.avg_spread));
    file.read(reinterpret_cast<char*>(&analytics.avg_spread_percent),
              sizeof(analytics.avg_spread_percent));
    file.read(reinterpret_cast<char*>(&analytics.current_imbalance),
              sizeof(analytics.current_imbalance));
    ...
    for (int tf = 0; tf < static_cast<int>(RenderEngine::TimeFrame::TF_1WEEK) + 1; ++tf) {
      ...
      if (has_current_candle) {
        ...
        file.read(reinterpret_cast<char*>(&current_candle.timestamp),
                  sizeof(current_candle.timestamp));
        ...
        file.read(reinterpret_cast<char*>(&current_candle.trade_count),
                  sizeof(current_candle.trade_count));
...
void DataPersistence::startPeriodicSave(int interval_seconds) {
  ...
  std::cout << "[DataPersistence] Started periodic save with interval: " << interval_seconds
            << " seconds" << std::endl;
}
...
std::string DataPersistence::getDataDirectory() const { return data_directory_; }
  ...
  while (periodic_save_running_.load()) {
    ...
    if (auto cache_manager = cache_manager_ref_.lock()) {
      ...
      if (saveAggregatedData(cache_manager, filename)) {
        std::cout << "[DataPersistence] Periodic save of aggregated data completed: " << filename
                  << std::endl;
      } else {
    ...
    if (auto processor = processor_ref_.lock()) {
      ...
      if (saveMarketAnalytics(processor, filename)) {
        std::cout << "[DataPersistence] Periodic save of market analytics completed: " << filename
                  << std::endl;
      } else {
