...
bool SymbolRegistry::load_from_file(const std::string& filepath) {
  ...
  try {
    ...
    std::sregex_iterator flat_begin(content.begin(), content.end(), flat_pattern);
    ...
    if (flat_matches > 0) {
      std::cout << "Loaded " << flat_matches << " symbol mappings (flat format) from " << filepath
                << std::endl;
      return true;
    }
    ...
    for (auto it = exchanges_begin; it != exchanges_end; ++it) {
      ...
      std::string exchange_section = content.substr(exchange_pos, next_exchange_pos - exchange_pos);

      std::sregex_iterator symbols_begin(exchange_section.begin(), exchange_section.end(),
                                         symbol_pattern);
    ...
    if (hier_matches > 0) {
      std::cout << "Loaded " << hier_matches << " symbol mappings (hierarchical format) from "
                << filepath << std::endl;
      return true;
    }
...
bool SymbolRegistry::save_to_file(const std::string& filepath) const {
  ...
  try {
    ...
    for (const auto& [id, info] : id_to_info_) {
      if (!first) file << ",\n";
      first = false;
      file << "    {\"id\": " << id << ", \"exchange\": \"" << info.exchange << "\", \"symbol\": \""
           << info.symbol << "\"}";
    }
    ...
    std::cout << "Saved " << id_to_info_.size() << " symbol mappings to " << filepath << std::endl;
    return true;
