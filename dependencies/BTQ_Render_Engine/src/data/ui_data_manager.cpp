...
UIDataManager::~UIDataManager() { save_state("default"); }
...
void UIDataManager::set_ui_state(const UIState& state) { ui_state_ = state; }

UIDataManager::UIState UIDataManager::get_ui_state() const { return ui_state_; }

void UIDataManager::add_panel(const PanelState& panel) { panels_[panel.panel_id] = panel; }

void UIDataManager::remove_panel(const std::string& panel_id) { panels_.erase(panel_id); }
...
void UIDataManager::set_theme(const std::string& theme) { ui_state_.current_theme = theme; }

std::string UIDataManager::get_theme() const { return ui_state_.current_theme; }

void UIDataManager::set_current_symbol(const std::string& symbol) {
  if (ui_state_.current_symbol != symbol) {
    ui_state_.current_symbol = symbol;

    // Notify all registered callbacks about the symbol change
    for (const auto& callback : symbol_change_callbacks_) {
      if (callback) {
        callback(symbol);
      }
    }
...
std::string UIDataManager::get_current_symbol() const { return ui_state_.current_symbol; }

void UIDataManager::register_symbol_change_callback(SymbolChangeCallback callback) {
  symbol_change_callbacks_.push_back(callback);
}
...
std::string UIDataManager::get_current_timeframe() const { return ui_state_.current_timeframe; }
