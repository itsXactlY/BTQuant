...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_WordWrap = 1
                                 << 24,  // InputTextMultiline(): word-wrap lines that are too long.
...
struct ImGuiIO {
  ...
  // void ClearInputCharacters() { InputQueueCharacters.resize(0); } // [Obsoleted in 1.89.8] Clear
  // the current frame text input buffer. Now included within ClearInputKeys(). Removed this as it
  // is ambiguous/misleading and generally incorrect to use with the existence of a higher-level
  // input queue.
#endif
...
struct ImDrawData {
  ...
  ImVector<ImTextureData*>*
      Textures;  // List of textures to update. Most of the times the list is shared by all
                 ...
                 // always points to ImGui::GetPlatformIO().Textures[]. May be overridden or set to
...
struct ImFontGlyph {
  unsigned int Colored
      : 1;  // Flag to indicate glyph is colored and should generally ignore tinting (make it usable
...
struct ImFontAtlas {
  ...
  // in 1.92.0 typedef ImFontAtlasCustomRect     CustomRect;              // OBSOLETED in 1.72+
...
struct ImFontBaked {
  ...
  unsigned int LoadNoRenderOnLayout
      : 1;  // 0  //     // Enable a two-steps mode where CalcTextSize() calls will load AdvanceX
            ...
            // unlikely to actually be rendered, otherwise it is slower because we'd do one query on
...
struct ImGuiPlatformIO {
  ...
  // (default to use ShellExecuteW() on Windows, system() on Linux/Mac. expected to return false on
  // failure, but some platforms may always return true)
...
// reported in IDE) OBSOLETED in 1.89.7 (from June 2023)
