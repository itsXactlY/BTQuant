...
static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const char* new_text,
                                    int new_text_len) {
  ...
  if (!is_resizable && new_text_len > avail)
    new_text_len =
        (int)(ImTextFindValidUtf8CodepointEnd(new_text, new_text + new_text_len, new_text + avail) -
              new_text);  // Truncate to closest UTF-8 codepoint. Alternative: return 0 to cancel
                          // insertion.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size,
  ...
  if (is_multiline) {
    ...
    // FIXME-OPT: Could append to index when new value of line_visible_n1 becomes bigger, see second
    // call to CalcClipRectVisibleItemsY() below.
    bool will_scroll_y = state && ((state->CursorFollow && render_cursor) ||
                                   (state->CursorCenterY && (render_cursor || render_selection)));
    line_count = InputTextLineIndexBuild(flags, line_index, buf_display, buf_display_end,
                                         wrap_width, will_scroll_y ? INT_MAX : line_visible_n1 + 1,
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected, bool* p_pressed) {
  ...
  if (hovered && IsMouseClicked(1) &&
      (flags & (ImGuiMultiSelectFlags_NoAutoSelect | ImGuiMultiSelectFlags_NoSelectOnRightClick)) ==
          0) {
