...
void AnchoredVWAP::calculate(const std::vector<BTQuant::RenderEngine::OHLCVCandle>& bars) {
  ...
  // Note: lower_bound comparator receives (element, value) so candle must come first
  auto timestampComparator = [](const BTQuant::RenderEngine::OHLCVCandle& bar, uint64_t ts) {
