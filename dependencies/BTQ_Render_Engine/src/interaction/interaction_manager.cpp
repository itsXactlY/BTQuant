...
ContextMenu::ContextMenu(const glm::vec2& position) 
    : position_(position) {
}
...
bool ContextMenu::handle_input(const InputEvent& event) {
    ...
    if (event.type == InputEventType::MouseMove) {
        ...
        int item_index = static_cast<int>((event.position.y - position_.y) / item_height);
    ...
    if (event.type == InputEventType::MouseButton && 
        ...
        if (selected_item_ >= 0 && selected_item_ < static_cast<int>(items_.size())) {
...
void TooltipManager::show_tooltip(const glm::vec2& position, const TooltipData& data) {
...
void DragDropManager::start_drag(const DragDropData& data, const glm::vec2& start_position) {
    drag_data_ = data;
...
void DragDropManager::cancel_drag() {
    dragging_ = false;
}

void DragDropManager::register_drop_target(UIComponent* component, DropCallback callback) {
...
void GestureRecognizer::set_gesture_callback(GestureType type, GestureCallback callback) {
    gesture_callbacks_[type] = callback;
}

void GestureRecognizer::update(float delta_time) {
    detect_gestures();
}
...
void GestureRecognizer::detect_pinch() {
    if (active_touches_.size() < 2) return;
...
float GestureRecognizer::calculate_distance() {
    if (active_touches_.size() < 2) return 0.0f;
...
void SelectionManager::select_component(UIComponent* component, bool multi_select) {
    if (!multi_select) {
...
const std::unordered_set<UIComponent*>& SelectionManager::get_selected_components() const {
...
void ZoomPanController::set_bounds(const glm::vec2& min_bounds, const glm::vec2& max_bounds) {
...
glm::mat4 ZoomPanController::get_transform_matrix() const {
    ...
    transform = glm::scale(transform, glm::vec3(zoom_factor_, zoom_factor_, 1.0f));
    transform = glm::translate(transform, glm::vec3(-pan_offset_, 0.0f));
    return transform;
}

glm::vec2 ZoomPanController::screen_to_world(const glm::vec2& screen_pos) const {
...
void HotkeyManager::register_hotkey(const std::string& name, int key_code, uint32_t modifiers,
...
std::vector<std::pair<std::string, HotkeyBinding>> HotkeyManager::get_all_hotkeys() const {
...
void InteractionManager::register_component(UIComponent* component) {
    auto it = std::find(registered_components_.begin(), registered_components_.end(), component);
...
ContextMenu* InteractionManager::create_context_menu(const glm::vec2& position) {
...
bool InteractionManager::is_mouse_button_pressed(MouseButton button) const {
    return pressed_mouse_buttons_.find(static_cast<int>(button)) != pressed_mouse_buttons_.end();
}
...
void InteractionManager::process_mouse_event(XEvent* event) {
    ...
    if (event->type == ButtonPress) {
        ...
        if (button_event->button == static_cast<int>(MouseButton::Left)) {
            UIComponent* component = find_component_at_position(mouse_position_);
            if (component) {
                bool multi_select = (active_modifiers_ & static_cast<uint32_t>(KeyModifier::Ctrl)) != 0;
...
UIComponent* InteractionManager::find_component_at_position(const glm::vec2& position) {
    // Find the topmost component at the given position
    for (auto it = registered_components_.rbegin(); it != registered_components_.rend(); ++it) {
...
void InteractionManager::dispatch_event_to_component(UIComponent* component, const InputEvent& event) {
...
void InteractionManager::setup_xinput2() {
    ...
    if (!XQueryExtension(display_, "XInputExtension", &xi_opcode_, &first_event, &first_error)) {
...
uint32_t InteractionManager::x11_modifiers_to_internal(unsigned int x11_mods) {
    ...
    if (x11_mods & ShiftMask) internal_mods |= static_cast<uint32_t>(KeyModifier::Shift);
    if (x11_mods & ControlMask) internal_mods |= static_cast<uint32_t>(KeyModifier::Ctrl);
    if (x11_mods & Mod1Mask) internal_mods |= static_cast<uint32_t>(KeyModifier::Alt);
    if (x11_mods & Mod4Mask) internal_mods |= static_cast<uint32_t>(KeyModifier::Super);
