...
private:
  enum class LayoutMode { Standard, Compact, Widescreen, Mobile };

public:
  DashboardLayoutManager(uint32_t viewport_width, uint32_t viewport_height,
...
private:
  ...
  std::array<PanelConfig, static_cast<size_t>(PanelType::Count)>
      target_panels_; // For animations
...
DashboardLayoutManager::DashboardLayoutManager(uint32_t viewport_width,
...
void DashboardLayoutManager::initialize_default_layout() {
  ...
  panels_[static_cast<size_t>(PanelType::SymbolGrid)] = {
      ...
      .z_order = 1,
      .title = "Symbol Grid",
      .background_color = to_glm(theme_.background_panel),
      .border_color = to_glm(theme_.border_color),
...
void DashboardLayoutManager::update_viewport_size(uint32_t width,
                                                  uint32_t height) {
...
void DashboardLayoutManager::calculate_standard_layout() {
  ...
  // Top row
  target_panels_[static_cast<size_t>(PanelType::SymbolGrid)].position = {
      panel_spacing_, panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::SymbolGrid)].size = {
  ...
  target_panels_[static_cast<size_t>(PanelType::MomentumHeatmap)].size = {
      col_width, row_height};
  ...
  target_panels_[static_cast<size_t>(PanelType::ConnectionStatus)].size = {
      col_width, row_height};
...
void DashboardLayoutManager::calculate_compact_layout() {
  ...
  target_panels_[static_cast<size_t>(PanelType::PriceChart)].position = {
      panel_spacing_ + col_width + panel_spacing_,
      panel_spacing_ + row_height + panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::PriceChart)].size = {
      col_width, row_height};

  target_panels_[static_cast<size_t>(PanelType::ConnectionStatus)].position = {
      panel_spacing_ + col_width + panel_spacing_,
      panel_spacing_ + 2 * (row_height + panel_spacing_)};
  target_panels_[static_cast<size_t>(PanelType::ConnectionStatus)].size = {
      col_width, row_height};
}
...
void DashboardLayoutManager::calculate_widescreen_layout() {
  ...
  float side_width =
      (available_space.x - chart_width - 3 * panel_spacing_) / 2.0f;
  float row_height = (available_space.y - panel_spacing_) / 2.0f;
  ...
  target_panels_[static_cast<size_t>(PanelType::OrderBook)].position = {
      panel_spacing_, panel_spacing_ + row_height + panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::OrderBook)].size = {side_width,
                                                                    row_height};

  // Center (charts)
  float chart_x = panel_spacing_ + side_width + panel_spacing_;
  target_panels_[static_cast<size_t>(PanelType::PriceChart)].position = {
      chart_x, panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::PriceChart)].size = {
      chart_width, available_space.y};

  // Right column
  float right_x = chart_x + chart_width + panel_spacing_;
  target_panels_[static_cast<size_t>(PanelType::MomentumHeatmap)].position = {
      right_x, panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::MomentumHeatmap)].size = {
      side_width, row_height * 0.6f};

  target_panels_[static_cast<size_t>(PanelType::ConnectionStatus)].position = {
      right_x, panel_spacing_ + row_height * 0.6f + panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::ConnectionStatus)].size = {
      side_width, row_height * 0.4f - panel_spacing_};

  target_panels_[static_cast<size_t>(PanelType::SystemLog)].position = {
      right_x, panel_spacing_ + row_height + panel_spacing_};
  target_panels_[static_cast<size_t>(PanelType::SystemLog)].size = {side_width,
                                                                    row_height};
}
...
void DashboardLayoutManager::calculate_mobile_layout() {
  ...
  for (size_t i = 0; i < static_cast<size_t>(PanelType::Count); ++i) {
    target_panels_[i].position = {
        panel_spacing_, panel_spacing_ + i * (panel_height + panel_spacing_)};
...
void DashboardLayoutManager::interpolate_panel_configs(float t) {
  for (size_t i = 0; i < static_cast<size_t>(PanelType::Count); ++i) {
    panels_[i].position =
        glm::mix(panels_[i].position, target_panels_[i].position, t);
...
glm::vec2 DashboardLayoutManager::calculate_available_space() const {
  return glm::vec2(static_cast<float>(viewport_width_) - 2 * panel_spacing_,
...
const PanelConfig &
DashboardLayoutManager::get_panel_config(PanelType type) const {
  return panels_[static_cast<size_t>(type)];
}
...
PanelType
DashboardLayoutManager::get_panel_at_position(const glm::vec2 &position) const {
  for (size_t i = 0; i < static_cast<size_t>(PanelType::Count); ++i) {
...
bool DashboardLayoutManager::is_position_in_panel(const glm::vec2 &position,
