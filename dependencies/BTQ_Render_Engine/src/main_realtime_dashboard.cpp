...
public:
  RealTimeDashboard() : running_(false) {
    ...
    std::cout << "Initializing advanced trading dashboard with HotSpine "
                 "integration..."
              << std::endl;
  }
  ...
  bool initialize() {
    try {
      ...
      std::cout << "  HotSpine SHM: " << data_config.hotspine_shm_name
                << std::endl;
      std::cout << "  Symbols file: " << data_config.symbols_file << std::endl;
      std::cout << "  Max symbols: " << data_config.max_symbols << std::endl;
      std::cout << "  Display: " << display_config.window_width << "x"
                << display_config.window_height << std::endl;
      ...
      performance_monitor_->setMonitoringInterval(
          perf_config.monitoring_interval_ms);
      ...
      hotspine_bridge_ = std::make_unique<HotSpineDataBridge>(
          data_config.hotspine_shm_name, data_config.symbols_file);

      if (!hotspine_bridge_->isConnected()) {
        std::cout << "Warning: HotSpine not available, running in demo mode"
                  << std::endl;
        demo_mode_ = true;
      } else {
      ...
      try {
        vulkan_dashboard_ = std::make_unique<BTQuant::VulkanDashboard>(
        ...
        // Note: VulkanDashboard doesn't provide direct access to
        // device/physical device This will need to be handled differently
  ...
  void run() {
    ...
    std::cout << "\n🚀 BTQuant Real-Time Dashboard is now running!"
              << std::endl;
    std::cout << "Features active:" << std::endl;
    std::cout << "  📊 Real-time market data visualization" << std::endl;
    std::cout << "  🔥 HotSpine integration: "
              << (demo_mode_ ? "Demo mode" : "Live data") << std::endl;
    std::cout << "  📈 Advanced analytics (VWAP, momentum, volatility)"
              << std::endl;
    ...
    const auto data_update_interval =
        std::chrono::milliseconds(33); // ~30 FPS data updates
    const auto stats_interval =
        std::chrono::seconds(10); // Print stats every 10 seconds
...
private:
  // Core components
  std::unique_ptr<BTQuant::RenderEngine::DashboardConfig> config_;
    ...
    if (!updates.empty()) {
      ...
      for (uint32_t symbol_id : active_symbols) {
        ...
        if (!analytics.recent_orderbooks.empty()) {
          ...
          data_viz_engine_->updateOrderbookData(symbol_id, latest_ob.bids,
                                                latest_ob.asks);
        }
      }

      auto data_end = std::chrono::high_resolution_clock::now();
      auto data_latency = std::chrono::duration_cast<std::chrono::microseconds>(
          data_end - data_start);
      ...
      std::cout << "[Dashboard] Processed " << updates.size() << " updates for "
                << symbols.size() << " symbols in " << data_latency.count()
                << " µs" << std::endl;
    }
  }
  ...
  void printStatistics() {
    ...
    if (hotspine_bridge_) {
      ...
      std::cout << "  Connection: "
                << (hotspine_metrics.connection_healthy ? "Healthy"
                                                        : "Disconnected")
                << std::endl;
      std::cout << "  Trades/sec: " << std::fixed << std::setprecision(1)
                << hotspine_metrics.trades_per_second << std::endl;
      std::cout << "  Orderbooks/sec: "
                << hotspine_metrics.orderbooks_per_second << std::endl;
      std::cout << "  Buffer utilization: "
                << hotspine_metrics.buffer_utilization_percent << "%"
                << std::endl;
    }
    ...
    if (market_processor_) {
      ...
      std::cout << "  Active symbols: " << market_summary.active_symbols
                << std::endl;
      std::cout << "  Trending up: " << market_summary.trending_up << std::endl;
      std::cout << "  Trending down: " << market_summary.trending_down
                << std::endl;
      std::cout << "  Avg momentum: " << std::fixed << std::setprecision(2)
                << market_summary.avg_momentum << "%" << std::endl;
    }
    ...
    if (data_viz_engine_) {
      ...
      std::cout << "  Grid updates: " << viz_metrics.grid_update_count
                << std::endl;
      std::cout << "  Heatmap updates: " << viz_metrics.heatmap_update_count
                << std::endl;
      std::cout << "  Chart updates: " << viz_metrics.chart_update_count
                << std::endl;
      std::cout << "  Avg grid latency: " << viz_metrics.grid_update_latency_us
                << " µs" << std::endl;
    }
    ...
    if (performance_monitor_) {
      ...
      std::cout << "  FPS: " << std::fixed << std::setprecision(1)
                << system_metrics.fps << std::endl;
      std::cout << "  Frame time: " << std::setprecision(2)
                << system_metrics.frame_time_ms << " ms" << std::endl;
      std::cout << "  Data latency: "
                << (system_metrics.data_to_display_latency_us / 1000.0) << " ms"
                << std::endl;
      std::cout << "  Memory usage: " << system_metrics.memory_usage_mb << " MB"
                << std::endl;
      std::cout << "  Uptime: " << perf_stats.uptime_seconds << " seconds"
                << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cout << "\n🎉 BTQuant Real-Time Dashboard completed successfully!"
              << std::endl;
    std::cout << "\nKey achievements:" << std::endl;
    std::cout << "✅ Real-time HotSpine market data integration" << std::endl;
    std::cout << "✅ Advanced market analytics (VWAP, momentum, volatility)"
              << std::endl;
    std::cout << "✅ High-performance GPU visualization pipeline" << std::endl;
    std::cout << "✅ Professional trading interface with 1000+ symbol support"
              << std::endl;
