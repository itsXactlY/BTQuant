...
void PerformanceMonitor::end_frame() {
  ...
  auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - frame_start_);

  current_frame_time_ms_ = duration.count() / 1000.0;
  update_frame_time(current_frame_time_ms_);
}

double PerformanceMonitor::get_frame_time_ms() const { return current_frame_time_ms_; }
...
double PerformanceMonitor::get_max_fps() const { return max_fps_; }

double PerformanceMonitor::get_min_fps() const { return min_fps_; }
...
size_t PerformanceMonitor::get_used_memory_bytes() const { return used_memory_bytes_; }

size_t PerformanceMonitor::get_total_memory_bytes() const { return total_memory_bytes_; }

void PerformanceMonitor::increment_data_processed(size_t count) { data_processed_count_ += count; }

void PerformanceMonitor::increment_indicators_calculated(size_t count) {
  indicators_calculated_count_ += count;
}

void PerformanceMonitor::set_data_processing_time(double ms) { data_processing_time_ms_ = ms; }

size_t PerformanceMonitor::get_data_processed_count() const { return data_processed_count_.load(); }

size_t PerformanceMonitor::get_indicators_calculated_count() const {
  return indicators_calculated_count_.load();
}

double PerformanceMonitor::get_data_processing_time_ms() const { return data_processing_time_ms_; }
...
std::vector<PerformanceMetric> PerformanceMonitor::get_metrics() const {
  ...
  // Frame time metrics
  metrics.push_back({"Frame Time", current_frame_time_ms_, "ms", get_min_frame_time(),
                     get_max_frame_time(), get_avg_frame_time_ms(),
                     std::chrono::high_resolution_clock::now()});

  metrics.push_back({"FPS", get_fps(), "fps", min_fps_, max_fps_, get_avg_fps(),
                     std::chrono::high_resolution_clock::now()});

  // Memory metrics
  metrics.push_back({"Memory Usage", get_memory_usage_percent(), "%", 0.0, 100.0,
                     get_memory_usage_percent(), std::chrono::high_resolution_clock::now()});

  // Data processing metrics
  metrics.push_back({"Data Processed", static_cast<double>(get_data_processed_count()), "count",
                     0.0, static_cast<double>(get_data_processed_count()),
                     static_cast<double>(get_data_processed_count()),
                     std::chrono::high_resolution_clock::now()});

  metrics.push_back({"Indicators Calculated",
                     static_cast<double>(get_indicators_calculated_count()), "count", 0.0,
                     static_cast<double>(get_indicators_calculated_count()),
                     static_cast<double>(get_indicators_calculated_count()),
                     std::chrono::high_resolution_clock::now()});

  metrics.push_back({"Data Processing Time", data_processing_time_ms_, "ms", 0.0,
                     data_processing_time_ms_, data_processing_time_ms_,
                     std::chrono::high_resolution_clock::now()});
...
bool PerformanceMonitor::is_enabled() const { return enabled_; }
