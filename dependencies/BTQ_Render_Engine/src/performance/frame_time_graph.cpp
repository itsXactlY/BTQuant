...
void FrameTimeGraph::end_frame() {
  ...
  auto duration =
      std::chrono::duration_cast<std::chrono::microseconds>(end_time - frame_start_time_);
...
double FrameTimeGraph::get_current_frame_time_ms() const { return current_frame_time_ms_; }
...
double FrameTimeGraph::get_current_fps() const { return current_fps_; }
...
bool FrameTimeGraph::is_enabled() const { return enabled_; }
...
void FrameTimeGraph::set_warning_threshold(double ms) { frame_time_threshold_warning_ = ms; }

void FrameTimeGraph::set_critical_threshold(double ms) { frame_time_threshold_critical_ = ms; }

double FrameTimeGraph::get_warning_threshold() const { return frame_time_threshold_warning_; }

double FrameTimeGraph::get_critical_threshold() const { return frame_time_threshold_critical_; }

bool FrameTimeGraph::is_performance_issue_detected() const { return performance_issue_detected_; }

double FrameTimeGraph::get_last_frame_time_deviation() const { return last_frame_time_deviation_; }
...
double FrameTimeGraph::get_standard_deviation() const { return std::sqrt(get_variance()); }
...
std::vector<std::pair<size_t, double>> FrameTimeGraph::get_spike_frames(
    double threshold_multiplier) const {
  std::lock_guard<std::mutex> lock(data_mutex_);
...
size_t FrameTimeGraph::get_consecutive_frame_drops(size_t min_drop_count,
...
void FrameTimeGraph::render(const char* title, float width, float height) {
  ...
  if (ImPlot::BeginPlot(title, plotSize)) {
    ...
    ImPlot::PlotLine("Critical Threshold", x_values.data(), critical_values.data(),
                     frame_times_.size());
    ImPlot::PopStyleVar();
    ...
    ImPlot::PlotLine("Warning Threshold", x_values.data(), warning_values.data(),
                     frame_times_.size());
    ImPlot::PopStyleVar();
    ...
    if (!warning_x.empty()) {
      ...
      ImPlot::PlotShaded("Warning Spikes", warning_x.data(), warning_y.data(), warning_x.size(),
                         frame_time_threshold_warning_);
    }

    if (!critical_x.empty()) {
      ImPlot::SetNextFillStyle(ImVec4(0.8f, 0.0f, 0.0f, 0.3f));  // Semi-transparent red
      ImPlot::PlotShaded("Critical Spikes", critical_x.data(), critical_y.data(), critical_x.size(),
                         frame_time_threshold_critical_);
    }
      ...
      if (frame_times_[i] > frame_time_threshold_warning_) {
        ...
        if (frame_times_[i] > frame_time_threshold_critical_) {
          ImPlot::SetNextMarkerStyle(
              ImPlotMarker_Circle, 8, ImVec4(0.8f, 0.0f, 0.0f, 1.0f), 3.0f,
              ImVec4(1.0f, 1.0f, 1.0f, 0.8f));  // Large red circle for critical
          ImPlot::PlotScatter("Critical", &x_pos, &frame_times_[i], 1);
        } else {
          ImPlot::SetNextMarkerStyle(ImPlotMarker_Diamond, 7, ImVec4(1.0f, 0.6f, 0.0f, 1.0f), 2.5f,
                                     ImVec4(0.0f, 0.0f, 0.0f, 0.8f));  // Orange diamond for warning
    ...
    if (!frame_times_.empty()) {
      ...
      if (frame_times_.size() >= 2) {
        ...
        ImVec4 trend_color = slope > 0.1 ? ImVec4(0.8f, 0.0f, 0.0f, 1.0f) :  // Red if degrading
                                 slope < -0.1 ? ImVec4(0.0f, 0.8f, 0.0f, 1.0f)
                                              :                   // Green if improving
      ...
      if (frame_times_.size() >= 10) {
        ...
        ImPlot::SetNextLineStyle(ImVec4(1.0f, 0.0f, 1.0f, 0.8f),
                                 1.2f);  // Purple for rolling average
        ImPlot::PushStyleVar(ImPlotStyleVar_LineWeight, 1.2f);
        ImPlot::PlotLine("Rolling Avg", rolling_avg_x.data(), rolling_avg_y.data(),
                         rolling_avg_x.size());
        ImPlot::PopStyleVar();
      }
    }
  ...
  ImGui::Text("Sample Count: %zu | Range: %.2f ms", frame_times_.size(),
              max_frame_time_ms_ - min_frame_time_ms_);

  // Performance issue status with more detail
  if (performance_issue_detected_) {
    auto now = std::chrono::high_resolution_clock::now();
    auto duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - last_performance_issue_time_);
    ...
    if (current_frame_time_ms_ > frame_time_threshold_critical_) {
      ImGui::TextColored(ImVec4(0.8f, 0.0f, 0.0f, 1.0f),
                         "CRITICAL: Frame time exceeds 30 FPS threshold!");
    } else {
      ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.0f, 1.0f),
                         "WARNING: Frame time exceeds 60 FPS threshold!");
    }
  } else {
  ...
  if (current_frame_time_ms_ > 0) {
    ...
    health_ratio =
        std::min(1.0f, static_cast<float>(frame_time_threshold_critical_ / current_frame_time_ms_));
  }
  ...
  ImU32 health_color = health_ratio > 0.7f ? IM_COL32(0, 255, 0, 255)
                                           :  // Green for good performance
                           health_ratio > 0.4f ? IM_COL32(255, 255, 0, 255)
                                               :      // Yellow for moderate
                           IM_COL32(255, 0, 0, 255);  // Red for poor

  draw_list->AddRectFilled(pos, ImVec2(pos.x + (bar_size.x * health_ratio), pos.y + bar_size.y),
                           health_color);
  ...
  ImGui::DragScalar("##Warning", ImGuiDataType_Double, &frame_time_threshold_warning_, 0.1f,
                    &warning_min, &warning_max, "%.2f ms (~%.1f FPS)");
  ImGui::PopItemWidth();
  ImGui::NextColumn();
  ...
  ImGui::DragScalar("##Critical", ImGuiDataType_Double, &frame_time_threshold_critical_, 0.1f,
                    &critical_min, &critical_max, "%.2f ms (~%.1f FPS)");
  ImGui::PopItemWidth();
  ImGui::NextColumn();
  ...
  if (ImGui::CollapsingHeader("Performance Analysis")) {
    ...
    ImGui::Indent();
    ImGui::Text("Warning (>%.2f ms): %zu frames (%.2f%%)", frame_time_threshold_warning_,
                warning_count, total_frames > 0 ? (warning_count * 100.0) / total_frames : 0.0);
    ImGui::Text("Critical (>%.2f ms): %zu frames (%.2f%%)", frame_time_threshold_critical_,
                critical_count, total_frames > 0 ? (critical_count * 100.0) / total_frames : 0.0);
    ImGui::Unindent();
    ...
    size_t consecutive_drops =
        get_consecutive_frame_drops(3, 33.33);  // 3+ consecutive frames > 33.33ms
    ...
    if (frame_times_.size() >= 10) {
      ...
      if (trend_change > 5.0) {
        ImGui::TextColored(ImVec4(0.8f, 0.0f, 0.0f, 1.0f), "Performance Degrading: +%.2f%%",
                           trend_change);
      } else if (trend_change < -5.0) {
        ImGui::TextColored(ImVec4(0.0f, 0.8f, 0.0f, 1.0f), "Performance Improving: %.2f%%",
                           trend_change);
      } else {
