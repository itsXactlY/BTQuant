...
VulkanChartPipeline::VulkanChartPipeline(VkDevice device, VkPhysicalDevice physical_device, VkRenderPass render_pass)
...
void VulkanChartPipeline::build_pipeline() {
    ...
    auto vertShaderCode = read_file("shaders/candlestick.vert.spv");
    auto fragShaderCode = read_file("shaders/candlestick.frag.spv");
...
void VulkanChartPipeline::bind_and_draw(VkCommandBuffer cb, 
    ...
    vkCmdPushConstants(cb, pipeline_layout_, 
                       ...
                       0, sizeof(ChartPushConstants), &push_constants);
