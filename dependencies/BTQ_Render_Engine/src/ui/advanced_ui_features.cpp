...
private:
    ...
    glm::vec2 snap_to_grid_size(const glm::vec2& size) {
        return glm::vec2(
            std::round(size.x / grid_size_) * grid_size_,
    ...
    glm::vec2 get_content_offset() {
        return glm::vec2(0, title_bar_height_);
    }
    ...
    void save_preset_to_file(const LayoutPreset& preset) {
        ...
        root["created_time"] = static_cast<int64_t>(
            std::chrono::duration_cast<std::chrono::seconds>(
                preset.created_time.time_since_epoch()).count());
        root["modified_time"] = static_cast<int64_t>(
            std::chrono::duration_cast<std::chrono::seconds>(
                preset.modified_time.time_since_epoch()).count());
...
public:
    ...
    const std::vector<FilterCriteria>& get_filters() const {
        return filters_;
    }
...
private:
    ...
    template<typename T>
    std::string get_field_value(const T& item, const std::string& field_name) const {
    ...
    bool matches_date_filter(const std::string& value, const FilterCriteria& filter) const {
...
private:
    ...
    float calculate_relevance_score(const std::string& query, const std::string& title, const std::string& description) {
    ...
    std::vector<std::pair<size_t, size_t>> find_highlight_ranges(const std::string& query, const std::string& text) {
...
private:
    ...
    void create_bloomberg_theme() {
        ...
        scheme.theme.text_primary = glm::vec4(1.0f, 0.6f, 0.0f, 1.0f); // Orange text
        scheme.theme.text_secondary = glm::vec4(0.8f, 0.8f, 0.0f, 1.0f); // Yellow text
        scheme.theme.text_muted = glm::vec4(0.5f, 0.5f, 0.5f, 1.0f);
    ...
    void apply_custom_color(const std::string& color_name, const glm::vec4& color) {
        // Apply custom color override
    }
    
    glm::vec4 get_theme_color(const DashboardTheme& theme, const std::string& color_name) const {
