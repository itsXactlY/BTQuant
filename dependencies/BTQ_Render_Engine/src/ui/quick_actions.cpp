...
void QuickActionsToolbar::initialize_default_actions() {
  ...
  actions_.push_back(QuickAction(
      "New Watchlist", "👁️", []() { printf("New Watchlist action triggered\n"); },
      "Create a new watchlist panel"));

  actions_.push_back(QuickAction(
      "New Order Book", "📊", []() { printf("New Order Book action triggered\n"); },
      "Create a new order book panel"));

  actions_.push_back(QuickAction(
      "Screenshot", "📷", []() { printf("Screenshot action triggered\n"); },
      "Take screenshot of active panel"));

  actions_.push_back(QuickAction(
      "Settings", "⚙️", []() { printf("Settings action triggered\n"); },
      "Open application settings"));

  actions_.push_back(QuickAction(
      "Export Data", "📤", []() { printf("Export Data action triggered\n"); },
      "Export current data"));

  actions_.push_back(QuickAction(
      "Refresh", "🔄", []() { printf("Refresh action triggered\n"); }, "Refresh current view"));

  actions_.push_back(QuickAction(
      "Zoom Reset", "🔍", []() { printf("Zoom Reset action triggered\n"); }, "Reset zoom level"));
}
...
void QuickActionsToolbar::render_toolbar_window() {
  ...
  ImGui::SetNextWindowSize(ImVec2(button_size_.x * actions_.size() + 40, button_size_.y + 20),
...
void QuickActionsToolbar::add_action(const QuickAction& action) {
  ...
  auto it = std::find_if(actions_.begin(), actions_.end(), [&action](const QuickAction& existing) {
...
void QuickActionsToolbar::remove_action(const std::string& name) {
  actions_.erase(std::remove_if(actions_.begin(), actions_.end(),
                                [&name](const QuickAction& action) { return action.name == name; }),
                 actions_.end());
}

void QuickActionsToolbar::set_position(const ImVec2& pos) { position_ = pos; }
