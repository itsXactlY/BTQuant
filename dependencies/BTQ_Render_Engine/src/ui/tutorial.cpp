...
TutorialStep::TutorialStep(const std::string& title, const std::string& description,
                           const std::string& target_control_id,
                           const glm::vec2& highlight_position, const glm::vec2& highlight_size)
    : title(title),
...
TutorialManager::TutorialManager()
    : is_active_(false),
      ...
      has_been_shown_(false)  // Initialize to false so tutorial shows on first run
      ,
      window_alpha_(0.95f) {
...
void TutorialManager::initialize_tutorial_steps() {
  ...
  steps_.emplace_back("Dashboard Overview",
                      ...
                      "dashboard_overview", glm::vec2(0.0f, 0.0f), glm::vec2(800.0f, 100.0f));
  ...
  steps_.emplace_back(
      ...
      "exchange_selector_button", glm::vec2(10.0f, 10.0f), glm::vec2(150.0f, 30.0f));

  // Symbol Search
  steps_.emplace_back("Symbol Search",
                      "Search for trading symbols across all selected exchanges. "
                      "You can quickly find and add symbols to your watchlist or charts.",
                      "symbol_search_top", glm::vec2(170.0f, 10.0f), glm::vec2(200.0f, 30.0f));
  ...
  steps_.emplace_back(
      ...
      "add_chart_panel", glm::vec2(380.0f, 10.0f), glm::vec2(200.0f, 30.0f));
  ...
  steps_.emplace_back(
      ...
      "chart_timeframe_selector", glm::vec2(10.0f, 50.0f), glm::vec2(300.0f, 40.0f));

  // Trading Controls
  steps_.emplace_back("Trading Controls",
                      ...
                      "place_buy_order", glm::vec2(10.0f, 100.0f), glm::vec2(200.0f, 60.0f));
  ...
  steps_.emplace_back(
      ...
      "indicator_selector", glm::vec2(320.0f, 100.0f), glm::vec2(250.0f, 60.0f));

  // Alerts and Notifications
  steps_.emplace_back("Alerts & Notifications",
                      ...
                      "alerts_menu", glm::vec2(10.0f, 170.0f), glm::vec2(150.0f, 40.0f));

  // Settings and Customization
  steps_.emplace_back("Settings & Customization",
                      ...
                      "settings_menu", glm::vec2(170.0f, 170.0f), glm::vec2(140.0f, 40.0f));

  // Completion
  steps_.emplace_back("Tutorial Complete!",
...
void TutorialManager::render_tutorial_window() {
  ...
  ImGui::SetNextWindowPos(
      ImVec2(ImGui::GetIO().DisplaySize.x * 0.5f - 250, ImGui::GetIO().DisplaySize.y * 0.5f - 150),
...
void TutorialManager::draw_highlight_overlay() {
  ...
  if (current.highlight_size.x > 0 && current.highlight_size.y > 0) {
    ...
    // REMOVED: Full screen blocking overlay
    ...
    draw_list->AddRect(min_pos, max_pos, IM_COL32(0, 150, 255, 255), 0.0f, 0,
                       3.0f);  // Bright blue border
  }
}
...
void TutorialManager::set_show_on_startup(bool show) { show_tutorial_on_startup_ = show; }
...
void TutorialManager::set_has_been_shown(bool shown) { has_been_shown_ = shown; }

bool TutorialManager::has_been_shown() const { return has_been_shown_; }

// Global tutorial manager instance
static TutorialManager g_tutorial_manager;

TutorialManager& get_global_tutorial_manager() { return g_tutorial_manager; }
...
void start_tutorial() { get_global_tutorial_manager().start_tutorial(); }

void stop_tutorial() { get_global_tutorial_manager().stop_tutorial(); }

void render_tutorial() { get_global_tutorial_manager().render_tutorial_window(); }
