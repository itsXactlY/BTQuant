...
DataVisualizationEngine::DataVisualizationEngine(VkDevice device, VkPhysicalDevice physical_device)
    : device_(device)
    ...
    , max_symbols_(1000)
    , max_chart_points_(10000)
    , max_orderbook_levels_(40)
{
    initializeBuffers();
    std::cout << "[DataVisualizationEngine] Initialized with support for " << max_symbols_ << " symbols" << std::endl;
}
...
bool DataVisualizationEngine::initializeBuffers() {
    ...
    if (vkCreateCommandPool(device_, &pool_info, nullptr, &command_pool_) != VK_SUCCESS) {
        std::cerr << "[DataVisualizationEngine] Failed to create command pool" << std::endl;
        return false;
    }
    ...
    size_t orderbook_size = max_symbols_ * max_orderbook_levels_ * sizeof(OrderbookLevelGPU);
    size_t staging_size = std::max({grid_size, heatmap_size, chart_size, orderbook_size});
    ...
    if (!createBuffer(staging_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        ...
        std::cerr << "[DataVisualizationEngine] Failed to create staging buffer" << std::endl;
        return false;
    }
    
    // Create GPU buffers
    if (!createBuffer(grid_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, grid_buffer_, grid_memory_)) {
        std::cerr << "[DataVisualizationEngine] Failed to create grid buffer" << std::endl;
        return false;
    }
    
    if (!createBuffer(heatmap_size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, heatmap_buffer_, heatmap_memory_)) {
        std::cerr << "[DataVisualizationEngine] Failed to create heatmap buffer" << std::endl;
        return false;
    }
    
    if (!createBuffer(chart_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, chart_buffer_, chart_memory_)) {
        std::cerr << "[DataVisualizationEngine] Failed to create chart buffer" << std::endl;
        return false;
    }
    
    if (!createBuffer(orderbook_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, orderbook_buffer_, orderbook_memory_)) {
        std::cerr << "[DataVisualizationEngine] Failed to create orderbook buffer" << std::endl;
        return false;
    }
...
void DataVisualizationEngine::updateGridData(const std::vector<SymbolData>& symbols) {
    ...
    for (const auto& symbol : symbols) {
        ...
        gpu_data.price_change_percent = static_cast<float>(symbol.price_change_percent);
        ...
        float volume_intensity = std::min(1.0f, static_cast<float>(symbol.volume_24h) / 1000000.0f);
        float momentum_intensity = std::min(1.0f, std::abs(static_cast<float>(symbol.momentum)) / 10.0f);
    ...
    // Transfer to GPU
    transferDataToGPU(grid_data.data(), grid_data.size() * sizeof(GridDataGPU), grid_buffer_);
    ...
    std::cout << "[DataVisualizationEngine] Updated grid data for " << symbols.size() 
              << " symbols in " << duration.count() << " µs" << std::endl;
}

void DataVisualizationEngine::updateHeatmapData(const std::vector<SymbolData>& symbols) {
    if (symbols.empty() || symbols.size() > max_symbols_) {
        return;
    }
    ...
    if (!momentums.empty()) {
        momentum_mean = std::accumulate(momentums.begin(), momentums.end(), 0.0f) / momentums.size();
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
        ...
        gpu_data.volatility = static_cast<float>(symbol.momentum);  // Use momentum as volatility proxy
        
        // Normalize momentum for heatmap intensity
        float normalized_momentum = 0.5f;  // Default neutral
        if (momentum_std > 0) {
            normalized_momentum = 0.5f + ((gpu_data.momentum - momentum_mean) / (momentum_std * 3.0f));
    ...
    // Transfer to GPU
    transferDataToGPU(heatmap_data.data(), heatmap_data.size() * sizeof(HeatmapDataGPU), heatmap_buffer_);
    ...
    std::cout << "[DataVisualizationEngine] Updated heatmap data for " << symbols.size() 
              << " symbols in " << duration.count() << " µs" << std::endl;
}

void DataVisualizationEngine::updateChartData(uint32_t symbol_id, const std::vector<ChartPoint>& points) {
    ...
    if (price_range == 0.0f) price_range = 1.0f;
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        if (i > 0) {
            ...
            float change_percent = ((gpu_point.price - prev_price) / prev_price) * 100.0f;
            gpu_point.color = calculatePriceChangeColor(change_percent);
        } else {
            gpu_point.color = {0.5f, 0.5f, 0.5f, 1.0f};  // Neutral color for first point
        }
        
        chart_data.push_back(gpu_point);
    }
    
    // Transfer to GPU (offset by symbol_id for multiple charts)
    size_t offset = symbol_id * max_chart_points_ * sizeof(ChartPointGPU);
    transferDataToGPU(chart_data.data(), chart_data.size() * sizeof(ChartPointGPU), chart_buffer_, offset);
    ...
    std::cout << "[DataVisualizationEngine] Updated chart data for symbol " << symbol_id 
              << " with " << points.size() << " points in " << duration.count() << " µs" << std::endl;
}

void DataVisualizationEngine::updateOrderbookData(uint32_t symbol_id, const std::vector<PriceLevel>& bids, 
    ...
    // Transfer to GPU (offset by symbol_id for multiple orderbooks)
    size_t offset = symbol_id * max_orderbook_levels_ * sizeof(OrderbookLevelGPU);
    transferDataToGPU(orderbook_data.data(), orderbook_data.size() * sizeof(OrderbookLevelGPU), 
    ...
    std::cout << "[DataVisualizationEngine] Updated orderbook data for symbol " << symbol_id 
              << " with " << (bids.size() + asks.size()) << " levels in " << duration.count() << " µs" << std::endl;
}

VisualizationPerformanceMetrics DataVisualizationEngine::getPerformanceMetrics() const {
    std::lock_guard<std::mutex> lock(perf_mutex_);
    return performance_metrics_;
}

VkBuffer DataVisualizationEngine::getGridBuffer() const {
    return grid_buffer_;
}
...
bool DataVisualizationEngine::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, 
...
uint32_t DataVisualizationEngine::findMemoryType(uint32_t type_filter, VkMemoryPropertyFlags properties) {
...
void DataVisualizationEngine::transferDataToGPU(const void* data, size_t size, VkBuffer dst_buffer, size_t offset) {
...
ColorRGBA DataVisualizationEngine::calculatePriceChangeColor(float change_percent) {
    ColorRGBA color;
