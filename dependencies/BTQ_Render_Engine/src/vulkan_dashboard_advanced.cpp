...
void VulkanDashboard::initialize() {
  ...
  // Initialize new Interactive Dashboard Layer
  dashboard_layer_ = std::make_unique<DashboardLayer>(vulkan_core_.get());
}
...
void VulkanDashboard::init_components() {
  ...
  // Legacy components removed as per user request (Interactive Charting now
  // handles this)
  /*
...
void VulkanCore::init_imgui() {
  ...
  // ImGui_ImplVulkan_DestroyFontUploadObjects() is no longer needed in 1.91
    ...
    for (const auto &c_json : j["components"]) {
      ...
      for (auto &comp : components_) {
        if (comp->get_name() == name) {
          ...
          comp->set_position(glm::vec2(c_json["pos"][0].get<float>(),
                                       c_json["pos"][1].get<float>()));
          comp->set_size(glm::vec2(c_json["size"][0].get<float>(),
                                   c_json["size"][1].get<float>()));
          break;
        }
      }
    }
