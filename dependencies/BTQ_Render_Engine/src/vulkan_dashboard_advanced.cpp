...
void VulkanDashboard::render_frame() {
  ...
  if (current_dpi_scale != last_dpi_scale_) {
    ...
    // Rebuild font atlas BEFORE starting the ImGui frame
...
void VulkanDashboard::framebuffer_size_callback(GLFWwindow* window, int width, int height) {
  ...
  // The DPI scale change will be detected and handled safely in render_frame()
  // to avoid mid-frame font atlas rebuilding which causes SIGSEGV in Vulkan backend.
}
...
void VulkanDashboard::render_performance_overlay() {
  ...
  if (ImGui::Begin("Performance", nullptr,
    ...
    if (market_data_processor_) {
      ...
      ImGui::Text("Trades/sec: ");
      ImGui::SameLine();
      BTQuant::UI::FontManager::getInstance().renderFormattedNumericalValue(stats.trades_per_second,
                                                                            "%.0f");
      ImGui::Text("Orderbook Updates/sec: ");
      ImGui::SameLine();
      BTQuant::UI::FontManager::getInstance().renderFormattedNumericalValue(
          stats.orderbooks_per_second, "%.0f");
    }
  }
