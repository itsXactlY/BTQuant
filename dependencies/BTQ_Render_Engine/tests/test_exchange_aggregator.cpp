...
class ExchangeAggregatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    bridge_ = std::make_shared<HotSpineDataBridge>("/tmp/test_shm");
    processor_ = std::make_shared<RenderEngine::MarketDataProcessor>();
    symbol_manager_ = std::make_shared<RenderEngine::SymbolManager>();
    
    aggregator_ = std::make_unique<ExchangeAggregator>(bridge_, processor_, symbol_manager_);
...
TEST_F(ExchangeAggregatorTest, TimeSynchronizationStrategies) {
  ...
  uint64_t avg_ts = aggregator_->calculateSynchronizedTimestamp("BTCUSDT");
