...
class MockMarketDataProcessor
    : public BTQuant::RenderEngine::MarketDataProcessor {
public:
  MOCK_METHOD(void, processTradeUpdate,
              (const BTQuant::RenderEngine::MarketDataUpdate &));
  MOCK_METHOD(void, processTradeUpdates,
              (const std::vector<BTQuant::RenderEngine::MarketDataUpdate> &));
  MOCK_METHOD(void, processOrderbookUpdate,
              (const BTQuant::RenderEngine::MarketDataUpdate &));
  MOCK_METHOD(BTQuant::RenderEngine::SymbolAnalytics, getSymbolAnalytics,
              (uint32_t), (const));
  ...
  for (size_t i = 0; i < BATCH_SIZE + 100; ++i) {
    ...
    if (batch.size() >= BATCH_SIZE) {
      EXPECT_CALL(*mock_processor,
                  processTradeUpdates(testing::SizeIs(BATCH_SIZE)))
          .Times(1);
...
TEST_F(HotSpineDataBridgeTest, TradeDataValidation) {
  ...
  invalid_trade.ts_exchange =
      1000000000000ULL;      // Invalid timestamp (< MIN_VALID_TS)
...
TEST_F(HotSpineDataBridgeTest, InstrumentStoreAtomicAccess) {
  ...
  std::thread writer([&store]() { store.symbol_id.store(12345); });
  ...
  std::thread snapshot_writer(
      [&store, snapshot]() { store.latest_snapshot.store(snapshot); });

  snapshot_writer.join();
  EXPECT_EQ(store.latest_snapshot.load(), snapshot);

  // Cleanup handled by InstrumentStore destructor automatically
}
...
TEST_F(HotSpineDataBridgeTest, DISABLED_AtomicPerformance) {
  ...
  auto regular_duration =
      std::chrono::duration_cast<std::chrono::microseconds>(end - start);

  std::cout << "Atomic operations: " << atomic_duration.count() << " µs"
            << std::endl;
  std::cout << "Regular operations: " << regular_duration.count() << " µs"
            << std::endl;
