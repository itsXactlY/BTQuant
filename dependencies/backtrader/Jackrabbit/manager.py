"""
jrr_websocket/manager.py

WebSocket manager orchestrating multiple exchange connections and data distribution.
"""
...
from backtrader.Jackrabbit.storages import MarketDataStorage, StorageConfig


class WebSocketManager:
    """
    Central coordinator for multiple websocket connections.
    ...
    async def _handle_message(self, message: Message) -> None:
        ...
        try:
            ...
            if message.type == 'ohlcv':
                await self.storage.store_ohlcv(message.data)
            elif message.type == 'trade':
                await self.storage.store_trade(message.data)
            elif message.type == 'orderbook':
                await self.storage.store_orderbook(message.data)
...
class ManagerBuilder:
    """Builder for easy manager setup with common exchanges"""
