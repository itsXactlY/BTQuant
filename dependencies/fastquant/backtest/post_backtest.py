...
# TODO :: Make end stats more readable.
...
def analyze_strategies(
    ...
    **kwargs
    ...
    for strat_idx, stratrun in enumerate(stratruns):
        ...
        if verbose > 0:
            print("--------------------------------------------------")
            print_dict(strats_params, "Strategy Parameters", format="indent")
            print_dict(returns, "Returns", format="indent")
            print_dict(sharpe, "Sharpe", format="indent")
            print_dict(drawdown, "Drawdown", format="indent")
            print_dict(timedraw, "Timedraw", format="indent")
...
def get_optim_metrics_and_params(sorted_metrics_df, sorted_params_df, verbose):
    # Save the optimal parameters and metrics as dictionaries
    optim_params = sorted_params_df.iloc[0].to_dict()
    optim_metrics = sorted_metrics_df.iloc[0].to_dict()

    # If verbose is enabled, print the optimal parameters and metrics
    if verbose > 0:
        print("\nOptimal Parameters:\n" + "-"*20)
    ...
    return optim_params, optim_metrics
...
def print_dict(d, title="", format="inline"):
    ...
    format : str : 'inline' or 'indent' for different output styles.
    """
    if format is None:
        print(title, d)
    ...
    if format == "indent":
        if title:
            print(f"\n{title}")
            print("-" * len(title))  # Title underline
