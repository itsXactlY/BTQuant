...
def livetrade_crypto_binance_liquidations(
    coin: str,
    collateral: str,
    exchange: str,
    account: str,
    ...
    """
    Live trade a strategy on Binance Futures based on liquidation data.

    Args:
    - strategy (str): The strategy name as a string or strategy class.
    - min_liquidation_size (float): Minimum liquidation size to monitor in USD.
    - account (str): The account type to use (e.g., 'JackRabbit_Binance').
    - start_hours_ago (int): The number of hours ago to start monitoring.
    """
    ...
    # Initialize the liquidation store
    store = binance_liquidation_store.BinanceLiquidationStore()
    ...
    print(f"Current time (UTC+2): {current_time}")
    print(f"Starting liquidation monitoring from (UTC+2): {from_date}")
    
    # Get liquidation data feed
    data = store.getdata()
    data._dataname = "LIQUIDATIONS"
    
    # Add data collector strategy if needed
    cerebro.addstrategy(DataCollectorStrategy)
    
    # Add the main strategy with liquidation parameters
    cerebro.addstrategy(
        strategy_class,
        exchange=exchange,
        account=account,
        ...
        coin=coin,
        collateral=collateral,
        min_liquidation_size=min_liquidation_size,
        backtest=False
    )
    
    # Add the liquidation data feed
    cerebro.adddata(data=data, name=data._dataname)
