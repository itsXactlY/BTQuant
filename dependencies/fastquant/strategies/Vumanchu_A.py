...
from fastquant.strategies.custom_indicators.VumanchuMarketCipher_A import VuManchCipherA

class VuManchCipherAStrategy(BaseStrategy):
    params = (
        ('take_profit', 2),
        ('percent_sizer', 0.05),
        ('dca_deviation', 1.5),
    ...
    def buy_or_short_condition(self):
        if not self.buy_executed:
            # All 3 methods are legit to use, or combine
            # if self.cipher.long_ema > 0 or self.cipher.blue_triangle > 0:
            # if self.cipher.lines.long_ema[0] > 0 or self.cipher.lines.blue_triangle[0] > 0:
