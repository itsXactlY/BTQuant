...
class DecyclerOscillator(bt.Indicator):
    ...
    params = (('hp_period', 37),)
    plotlines = dict(decycle=dict(_plotskip=True,), hp=dict(_plotskip=True,),)
    plotinfo = dict(plot=True, plotname='Elhers Decycler Oscillator', subplot=True, plotlinelabels=True)
    ...
    def __init__(self):
        self.addminperiod(self.p.hp_period)
