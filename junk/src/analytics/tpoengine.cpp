...
TPOProfile::TPOProfile() : letter_counter(0), poc_cached(false), cached_poc(0.0), 
                           value_area_cached(false), cached_value_area(0.0, 0.0), cached_value_area_percent(70.0) {}
...
std::string TPOProfile::assign_letter_to_time_bracket(const std::chrono::system_clock::time_point& time_bracket) {
...
void TPOProfile::add_price_to_time_bracket(double price_level, const std::chrono::system_clock::time_point& time_bracket) {
...
void TPOProfile::clear() {
    ...
    cached_poc = 0.0;
    value_area_cached = false;
    cached_value_area = std::make_pair(0.0, 0.0);
    cached_value_area_percent = 70.0;
}

// Get all price levels that were touched in a specific time bracket
std::vector<double> TPOProfile::get_prices_for_time_bracket(const std::chrono::system_clock::time_point& time_bracket) const {
...
std::chrono::system_clock::time_point TPOProfile::get_time_bracket_for_letter(const std::string& letter) const {
...
size_t TPOProfile::get_unique_price_count() const {
    return price_to_letters.size();
}

// Get the total number of unique time brackets in the profile
size_t TPOProfile::get_unique_time_bracket_count() const {
...
bool TPOEngine::is_price_in_value_area(double price_level, double percent) const {
    return tpo_profile.is_in_value_area(price_level, percent);
}

// Get the current Point of Control (POC)
double TPOEngine::get_current_poc() const {
    return tpo_profile.get_current_poc();
}

// Get the current Value Area boundaries
std::pair<double, double> TPOEngine::get_current_value_area_bounds(double percent) const {
    return tpo_profile.get_value_area(percent);
}
...
std::vector<std::pair<TPONode, float>> TPOEngine::get_tpo_data_with_opacity(double va_percent) const {
    std::vector<std::pair<TPONode, float>> result;
    
    // Get the current value area bounds
    auto va_bounds = get_current_value_area_bounds(va_percent);
    double va_low = va_bounds.first;
    double va_high = va_bounds.second;
    
    // Iterate through all TPO data
    for (const auto& [time_bucket, price_buckets] : tpo_data) {
        for (const auto& [price_level, node] : price_buckets) {
            // Determine if this price level is within the value area
            float opacity = 0.3f; // Default to 30% opacity outside VA
            if (price_level >= va_low && price_level <= va_high) {
                opacity = 1.0f; // Full opacity inside VA
            }
            
            result.push_back(std::make_pair(node, opacity));
        }
    }
...
double TPOEngine::get_poc_line_data() const {
    return get_current_poc();
}
...
std::pair<double, double> TPOProfile::get_value_area(double percent) const {
    // Check if we have a cached value for the same percent
    if (value_area_cached && cached_value_area_percent == percent) {
        return cached_value_area;
    }
    
    if (price_to_letters.empty()) {
        cached_value_area = std::make_pair(0.0, 0.0);
        cached_value_area_percent = percent;
        value_area_cached = true;
        return cached_value_area;
    }
    ...
    cached_value_area = std::make_pair(min_price, max_price);
    cached_value_area_percent = percent;
    value_area_cached = true;
    return cached_value_area;
}
...
// Check if a price level is within the current value area
bool TPOProfile::is_in_value_area(double price_level, double percent) const {
    auto va = get_value_area(percent);
    return price_level >= va.first && price_level <= va.second;
}

// Get the current value area boundaries
std::pair<double, double> TPOProfile::get_current_value_area_bounds() const {
    return cached_value_area;
}

// Get the current POC price
double TPOProfile::get_current_poc() const {
    return cached_poc;
}

// Integration with LiquiditySweepDetector
void TPOEngine::integrate_with_liquidity_detector(LiquiditySweepDetector& detector) {
    // Extract price-volume data from TPO engine and feed it to the liquidity detector
    auto price_volume_data = get_price_volume_profile();
    
    // Convert TPO data to liquidity snapshots for the detector
    std::vector<LiquiditySnapshot> snapshots;
    
    // Use current time as the timestamp for all snapshots (in a real implementation, 
    // this would come from actual market data)
    auto current_time = std::chrono::system_clock::now();
    
    for (const auto& [price, volume] : price_volume_data) {
        // For simplicity, we'll assume equal bid/ask volumes in this integration
        // In a real implementation, this would come from actual order book data
        LiquiditySnapshot snapshot(current_time, price, volume/2.0, volume/2.0);
        snapshots.push_back(snapshot);
    }
    
    // Process the snapshots with the detector
    detector.process_liquidity_snapshots(snapshots);
    
    // Trigger sweep detection
    detector.detect_sweeps();
}

// Get price-volume data that can be used by liquidity detectors
std::vector<std::pair<double, double>> TPOEngine::get_price_volume_profile() const {
    std::vector<std::pair<double, double>> result;
    
    // Aggregate volume across all time buckets for each price level
    std::map<double, double> price_volume_map;
    
    for (const auto& [time_bucket, price_buckets] : tpo_data) {
        for (const auto& [price_level, node] : price_buckets) {
            price_volume_map[price_level] += node.total_volume;
        }
    }
