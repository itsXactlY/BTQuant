...
\
        ...
        for i in range(min(num_samples, len(self.test_features) - 50)):
            ...
            if pred['exit_signals']['stop_loss'] is not None:
                ...
                if sl_prob > 0.7:
                    if min_return < unrealized_pnl * 1.5:
                        results['cut_before_crash'].append(1)
                    elif recovery > 0:
                        results['cut_too_early'].append(1)
                else:
                    if recovery > unrealized_pnl * 0.5:
                        results['held_through_recovery'].append(1)
