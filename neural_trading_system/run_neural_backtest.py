...
#     ff_w = state_dict.get("transformer_blocks.0.ff.0.weight")
...
#             # Forward pass
#             with torch.no_grad():
#                 # Provide dummy position_context (all zeros during backtest)
#                 position_context = {
#                     'unrealized_pnl': torch.zeros(1, 1, device=self.device),
#                     'time_in_position': torch.zeros(1, 1, device=self.device)
#                 }
#                 out = self.model(self._input_tensor, position_context=position_context)
...
#                     "unified_exit_prob", "exit_prob", "exit_probs", "exit_probability", "prob_exit",
...
#!/usr/bin/env python3
"""
UNIFIED BACKTEST - OPTIMIZED VERSION
✅ Fast warmup with periodic feature checking
✅ Loads training config for exact parity
...
class FastUnifiedNeuralStrategy(DataCollectionStrategy):
    """
    ⚡ OPTIMIZED: Fast warmup + config-driven architecture
    """
    
    params = dict(
        model_path='models/best_exit_aware_model.pt',
        feature_extractor_path='models/exit_aware_feature_extractor.pkl',
        config_path='models/exit_aware_config.json',  # ← NEW: Load training config
        
        # Trading params (can override from config)
        min_entry_prob=0.35,
        min_expected_return=0.01,
        
        # Performance tuning
        min_warmup_bars=500,
        feature_check_interval=100,  # ← Only check features every N bars during warmup
        prediction_interval=5,
    ...
    def __init__(self):
        ...
        # ✅ STEP 1: Load training config for exact parity
        ...
        # ✅ STEP 2: Load model with config-driven architecture
    ...
    def _fast_feature_check(self):
        """
        ⚡ FAST PATH: Just check feature count, don't extract yet
        Returns: (is_ready: bool, feature_count: int)
        """
        try:
            batch = optimized_patch.current_batch
            if not batch:
            ...
            # Quick count of available indicators (no extraction)
            available_keys = sum(1 for key in batch[0].keys() if key not in ('bar', 'datetime'))
            
            # Rough estimate: each indicator contributes ~25-30 features on average
            # 25100 features / ~90 indicators ≈ 279 features per indicator
    ...
    def _extract_features(self):
        """
        Extract and validate features (only called after fast check passes)
        """
        try:
            batch = optimized_patch.current_batch
            if not batch:
    ...
    def _predict(self):
        """Model inference with position context"""
        try:
            seq = np.array(self.feature_buffer, dtype=np.float32)
    ...
    def next(self):
        """
        ⚡ OPTIMIZED: Fast warmup with periodic checking
        """
        capture_patch(self)
        ...
        # ⚡ FAST PATH: During warmup, only check features periodically
        if not self.warmup_complete:
            # Only check every N bars to avoid expensive operations
            if (len(self) % self.p.feature_check_interval) == 0:
                ...
                # If fast check says we're ready, try full extraction
                if is_ready:
                    f = self._extract_features()
                    if f is not None:
                        # Success! Continue to trading logic
                        self.feature_buffer.append(f)
                        if len(self.feature_buffer) > self.seq_len:
                            self.feature_buffer.pop(0)
            return  # Skip rest during warmup
        
        # ✅ After warmup complete, extract every bar
        f = self._extract_features()
        if f is None:
            return
...
def run_unified_backtest(
    ...
    """
    ⚡ Optimized backtest with config loading
    """
    console.print(Panel.fit(
    ...
    # ⚡ OPTIMIZATION: exactbars + stdstats=False for speed
    cerebro = bt.Cerebro(
        oldbuysell=True,
        runonce=False,
        stdstats=False,
        exactbars=False,  # Keep False for indicator history
    )
