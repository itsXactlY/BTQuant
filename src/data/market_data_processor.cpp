...
// Constructor for MarketDataProcessor
    ...
    , local_read_tail_(0)
{
    ...
    std::cout << "[MarketDataProcessor] Initialized with polling loop" << std::endl;
}

// Destructor for MarketDataProcessor
MarketDataProcessor::~MarketDataProcessor() {
    running_ = false;
...
// Process a trade update (asynchronous) - now adapted to polling model
void MarketDataProcessor::processTradeUpdate(const MarketDataUpdate& update) {
    // In the polling model, we don't queue updates since they come from shared memory
...
std::vector<OrderbookData> MarketDataProcessor::getHistoricalOrderbooks(uint32_t symbol_id, size_t count) const {
    auto& shard = getShard(symbol_id);
    std::shared_lock lock(shard.mutex);
...
std::vector<VolumeProfileLevel> MarketDataProcessor::getVolumeProfile(uint32_t symbol_id, TimeFrame timeframe) const {
...
void MarketDataProcessor::setMomentumWindow(size_t window_size) { momentum_window_size_ = window_size; }
void MarketDataProcessor::setVolatilityWindow(size_t window_size) { volatility_window_size_ = window_size; }
...
uint64_t MarketDataProcessor::subscribe(uint32_t symbol_id, NotificationType filter, SymbolCallback callback) {
...
void MarketDataProcessor::processUpdate(const MarketDataUpdate& update) {
    // In the polling model, we use atomic operations instead of mutex locks
    // The data is stored in atomic structures that can be updated lock-free
    
    // For the hot path, we update atomic values directly without locking
    auto& shard = getShard(update.symbol_id);
    
    // Use atomic operations to update the symbol data without mutex
    {
        std::unique_lock lock(shard.mutex); // Temporarily keeping mutex for this transition
    ...
    } // Release shard lock before notifying subscribers
