...
void test_copy_operations() {
    ...
    // At this point, both buffers contain 123, front buffer is what read() returns
    
    original.write() = 456;  // Write to back buffer
    original.swap();         // Now back becomes front, so read() returns 456
    
    // Test copy constructor
    DoubleBufferedState<int> copied(original);
    assert(copied.read() == 456);  // Should copy the current state (what read() returns)
    
    // Test copy assignment
    DoubleBufferedState<int> assigned;
    assigned = original;
    assert(assigned.read() == 456);  // Should copy the current state
