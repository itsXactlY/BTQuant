...
from transparencypatch import activate_patch, capture_patch, print_patch
...
class ZeroLagSineMomentumDCAStrategy(bt.Strategy):
    ...
    def stop(self):
        print_patch(auto_export=True, filename='zerolag_sine_momentum_intermediates')
        
        if self.p.debug:
            print("Final Portfolio Value: %.2f" % self.broker.getvalue())
            print("Final Cash: %.2f" % self.broker.get_cash())
            if self.active_orders:
                for order in self.active_orders:
                    print(f"Open Order - Entry: {order.entry_price}, Size: {order.size}, TP Price: {order.take_profit_price}")
