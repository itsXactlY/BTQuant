...
public:
  ...
  explicit ConfigValue(int64_t value)
      : type_(Type::INT64), int64_value_(value) {}
  ...
  explicit ConfigValue(const std::vector<std::string> &value)
      : type_(Type::STRING_VECTOR), string_vector_value_(value) {}
  explicit ConfigValue(const std::vector<int64_t> &value)
      : type_(Type::INT64_VECTOR), int64_vector_value_(value) {}
  explicit ConfigValue(const std::vector<double> &value)
      : type_(Type::DOUBLE_VECTOR), double_vector_value_(value) {}
...
inline bool ConfigValue::operator==(const ConfigValue &other) const {
  ...
  case Type::STRING_VECTOR:
    return string_vector_value_ == other.string_vector_value_;
  case Type::INT64_VECTOR:
    return int64_vector_value_ == other.int64_vector_value_;
  case Type::DOUBLE_VECTOR:
    return double_vector_value_ == other.double_vector_value_;
  case Type::NULL_VALUE:
    return true;
  default:
    return false;
...
inline std::string ConfigValue::to_string() const { return as_string(); }
...
using ConfigChangeCallback = std::function<void(
    ...
    const ConfigValue &old_value, const ConfigValue &new_value)>;
...
public:
  ...
  static bool get_env_var_as_bool(const std::string &name,
                                  bool default_val = false);
  static int64_t get_env_var_as_int(const std::string &name,
                                    int64_t default_val = 0);
  static double get_env_var_as_double(const std::string &name,
...
template <>
inline bool convert_from_string<int64_t>(const std::string &str,
                                         int64_t &result) {
  try {
...
template <>
inline bool convert_from_string<double>(const std::string &str,
                                        double &result) {
  try {
    result = std::stod(str);
    return true;
