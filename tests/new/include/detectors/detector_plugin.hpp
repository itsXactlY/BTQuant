...
public:
    ...
    // Detection methods
    virtual std::optional<BTQuant::DetectionSignal> detect(const std::string& symbol) = 0;
    virtual std::vector<BTQuant::DetectionSignal> detect_all(const std::string& symbol);
    virtual void update_orderbook(const std::string& exchange, const std::string& symbol);
    virtual void update_trade(const BTQuant::TradeData& trade);
    
    // Configuration
    virtual void configure(const std::unordered_map<std::string, Config::ConfigValue>& config) = 0;
    virtual std::unordered_map<std::string, Config::ConfigValue> get_configuration() const = 0;
    ...
    virtual void set_dependency(const std::string& name, std::shared_ptr<void> dependency);
