...
public:
  ...
  static constexpr const char PLUGIN_DESCRIPTION[] =
      "Detects cross-exchange spread arbitrage opportunities";
  static constexpr const char *PLUGIN_DEPENDENCIES[] = {
      "HotSpineExtendedReader", nullptr};

  // Detect arbitrage opportunities for a symbol
  std::vector<BTQuant::DetectionSignal>
  detect_all(const std::string &symbol) override;
  ...
  void update_orderbook(const std::string &exchange,
                        const std::string &symbol) override;
  void update_trade(const TradeData &trade) override;
  void configure(const std::unordered_map<std::string, Config::ConfigValue>
                     &config) override;
  std::unordered_map<std::string, Config::ConfigValue>
  get_configuration() const override;
  bool initialize() override;
  void shutdown() override;
  std::vector<std::string> get_dependencies() const override;
  void set_dependency(const std::string &name,
                      std::shared_ptr<void> dependency) override;
