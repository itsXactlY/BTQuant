...
struct OrderbookData {
    ...
    double best_bid() const {
        return bids.empty() ? 0.0 : bids[0].first;
    }
    
    double best_ask() const {
        return asks.empty() ? 0.0 : asks[0].first;
    }
...
public:
    explicit HotSpineExtendedReader(const std::string& shm_name = "/btquant_hotspine");
    ...
    std::map<std::string, double> get_all_exchange_prices(const std::string& symbol);
    
    // Multi-exchange orderbook comparison
    std::map<std::string, OrderbookData> get_all_exchange_orderbooks(const std::string& symbol);
