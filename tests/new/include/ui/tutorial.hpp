...
// Using a simple 2D vector type instead of GLM to avoid dependency
struct Vec2 {
    float x, y;
...
struct TutorialStep {
    ...
    Vec2 highlight_position;   ///< Position to highlight on screen
    Vec2 highlight_size;       ///< Size of the highlight area
    ...
    TutorialStep(const std::string& title,
                 ...
                 const Vec2& highlight_position = Vec2(0.0f, 0.0f),
                 const Vec2& highlight_size = Vec2(0.0f, 0.0f));
};
