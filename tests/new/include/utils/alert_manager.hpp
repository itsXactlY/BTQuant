...
struct Alert {
  ...
  Alert()
      : type(AlertType::CUSTOM), severity(AlertSeverity::INFO),
        timestamp_us(0) {}

  Alert(AlertType t, AlertSeverity s, const std::string &msg,
        const std::string &comp = "")
      : type(t), severity(s), message(msg), component(comp) {
...
public:
  explicit FileAlertSink(const std::string &path,
                         AlertSeverity min_level = AlertSeverity::INFO);
...
public:
  ...
  void raise_alert(AlertType type, AlertSeverity severity,
  ...
  void suppress_duplicates(bool suppress) {
    suppress_duplicates_.store(suppress);
  }
...
public:
  ...
  static Alert create_detector_error_alert(const std::string &detector,
                                           const std::string &error);
  static Alert create_recovery_success_alert(const std::string &action);
  static Alert create_recovery_failed_alert(const std::string &action,
