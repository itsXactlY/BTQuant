...
public:
  ...
  struct RecoveryState {
    ...
    RecoveryState()
        : last_action(RecoveryAction::NONE),
          current_action(RecoveryAction::NONE), attempt_count(0),
          success_count(0), failure_count(0), last_attempt_us(0),
          last_success_us(0), is_recovering(false) {}
  };
  ...
  // Recovery operations
  RecoveryAction
  determine_action(const Monitoring::ShmHealthMonitor::HealthReport &report);
  bool execute_recovery(RecoveryAction action);
  bool
  attempt_recovery(const Monitoring::ShmHealthMonitor::HealthReport &report);

  // State management
  const RecoveryState &get_state() const { return state_; }
  void reset_state();
  void record_attempt(RecoveryAction action, bool success,
...
private:
  ...
  void raise_recovery_alert(RecoveryAction action, bool success,
...
using RecoveryCallback =
    std::function<bool(RecoveryController::RecoveryAction)>;
...
struct RecoveryStats {
  ...
  RecoveryStats()
      : total_recovery_attempts(0), successful_recoveries(0),
        failed_recoveries(0), total_downtime_us(0), longest_downtime_us(0),
        consecutive_failures(0) {}

  double success_rate() const {
    if (total_recovery_attempts == 0)
      return 0.0;
    return static_cast<double>(successful_recoveries) /
           total_recovery_attempts * 100.0;
  }
};
