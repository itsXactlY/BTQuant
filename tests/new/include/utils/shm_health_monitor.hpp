...
public:
    ...
    struct HealthReport {
        ...
        std::array<char, 256> shm_path{};  // Fixed-size buffer for atomic compatibility
        ...
        std::array<char, 512> error_message{};  // Fixed-size buffer for atomic compatibility
    ...
    explicit ShmHealthMonitor(const std::string& shm_path, const MonitorConfig& config);
    ~ShmHealthMonitor();
    ...
    uint32_t get_consecutive_failures() const { return consecutive_failures_.load(); }
...
private:
    ...
    mutable std::mutex current_report_mutex_; // For atomic-like access to current_report_
...
public:
    ...
    static std::string read_file_header(const std::string& path, size_t bytes = 256);
};
