...
void test_diagonal_imbalance_detection() {
    ...
    std::cout << "Number of detected imbalances: " << imbalances.size() << std::endl;
    
    // We expect to find an imbalance where buy_volume at price 100 is much higher than sell_volume at price 99
    ...
    for (const auto& imbalance : imbalances) {
        ...
        // Check if this is the expected imbalance (buy_volume at 100 vs sell_volume at 99)
        if (price_index == 100 && time_bucket == 0) {
            // The ratio should be buy_volume_at_100 / sell_volume_at_99
            // We had 200 buy volume at 100 (two trades of 100 each) and 20 sell volume at 99
            // So ratio should be 200/20 = 10.0
            if (ratio >= 3.0) {
    ...
    assert(found_expected_imbalance && "Expected diagonal imbalance was not detected!");
...
void test_reverse_diagonal_imbalance_detection() {
    ...
    std::cout << "Number of detected reverse imbalances: " << imbalances.size() << std::endl;
    
    // We expect to find an imbalance where sell_volume at price 100 is much higher than buy_volume at price 99
    bool found_expected_imbalance = false;
    for (const auto& imbalance : imbalances) {
        ...
        // The sell volume at price 100 should be much higher than buy volume at price 99
        // Ratio should be 150/25 = 6.0 which is > 4.0 threshold
        if (ratio >= 4.0) {
            found_expected_imbalance = true;
        }
    }
    
    assert(found_expected_imbalance && "Expected reverse diagonal imbalance was not detected!");
    
    std::cout << "Reverse diagonal imbalance detection test PASSED!" << std::endl;
}

void test_no_imbalance_below_threshold() {
    std::cout << "Testing that no imbalances are detected below threshold..." << std::endl;
    ...
    assert(imbalances.empty() && "Unexpected imbalances detected below threshold!");
