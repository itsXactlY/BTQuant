...
void test_reset_functionality() {
    ...
    assert(tracker.get_cumulative_delta() == 15.0 && "Initial cumulative delta should be 15.0");
    assert(!tracker.is_reset() && "Tracker should not be marked as reset initially");

    // Reset the tracker
    tracker.reset();

    assert(tracker.get_cumulative_delta() == 0.0 && "Cumulative delta should be 0.0 after reset");
    assert(tracker.is_reset() && "Tracker should be marked as reset after reset");

    // Add more deltas after reset
    tracker.add_delta(7.0);
    assert(tracker.get_cumulative_delta() == 7.0 && "Cumulative delta should be 7.0 after reset and new addition");
...
void test_session_boundary_functionality() {
    ...
    assert(tracker.get_cumulative_delta() == 0.0 && "Cumulative delta should be 0.0 after session boundary");
    assert(tracker.is_session_active() && "Session should be active after setting boundary");

    // Add more deltas after session boundary
    tracker.add_delta(8.0);
    assert(tracker.get_cumulative_delta() == 8.0 && "Cumulative delta should be 8.0 after session boundary and new addition");
...
void test_delta_count_tracking() {
    ...
    assert(tracker.get_delta_count_since_reset() == 0 && "Initial delta count should be 0");

    tracker.add_delta(10.0);
    assert(tracker.get_delta_count_since_reset() == 1 && "Delta count should be 1 after first addition");

    tracker.add_delta(5.0);
    assert(tracker.get_delta_count_since_reset() == 2 && "Delta count should be 2 after second addition");

    tracker.add_delta(-3.0);
    assert(tracker.get_delta_count_since_reset() == 3 && "Delta count should be 3 after third addition");

    // Reset and check count
    tracker.reset();
    assert(tracker.get_delta_count_since_reset() == 0 && "Delta count should be 0 after reset");

    tracker.add_delta(7.0);
    assert(tracker.get_delta_count_since_reset() == 1 && "Delta count should be 1 after reset and new addition");
...
void test_session_duration_management() {
    ...
    assert(default_duration == std::chrono::minutes(60) && "Default session duration should be 60 minutes");
    ...
    assert(custom_duration == std::chrono::minutes(30) && "Custom session duration should be 30 minutes");
...
void test_time_based_session_reset() {
    ...
    // Add some delta values
    tracker.add_delta(10.0);
    assert(tracker.get_cumulative_delta() == 10.0 && "Initial cumulative delta should be 10.0");
