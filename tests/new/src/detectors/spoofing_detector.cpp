...
SpoofingDetector::SpoofingDetector(HotSpineExtendedReader &reader)
    : reader_(reader), min_cancel_count_(3), max_duration_ms_(5000.0),
      min_size_(0.1), detections_(0) {
...
std::optional<SpoofingSignal>
SpoofingDetector::detect(const std::string &exchange,
                         const std::string &symbol) {
  std::string key = make_key(exchange, symbol);
  uint64_t now = reader_.get_current_time_us();
  ...
  for (const auto &[price, level] : bid_history_[key]) {
    ...
    if (level.cancel_count >= min_cancel_count_ &&
        duration_ms < max_duration_ms_ && level.size >= min_size_) {
...
std::string SpoofingSignal::to_string() const {
  ...
  oss << "Spoofing(symbol=" << symbol << ", exchange=" << exchange
      << ", side=" << (is_bid_spoof ? "BID" : "ASK")
      << ", price=" << spoof_price << ", size=" << spoof_size
      << ", cancels=" << cancel_count << ", confidence=";

  switch (confidence) {
  case SpoofConfidence::LOW:
    oss << "LOW";
    break;
  case SpoofConfidence::MEDIUM:
    oss << "MEDIUM";
    break;
  case SpoofConfidence::HIGH:
    oss << "HIGH";
    break;
  }
