...
std::string SpreadSignal::to_string() const {
  ...
  oss << "SpreadArbitrage(symbol=" << symbol
      ...
      << ", sell_exchange=" << sell_exchange << ", buy_price=" << buy_price
      << ", sell_price=" << sell_price << ", spread=" << spread_bps << "bps"
...
std::vector<BTQuant::DetectionSignal>
SpreadArbitrageDetector::detect_all(const std::string &symbol) {
  ...
  if (best_bid > 0 && best_ask < std::numeric_limits<double>::max() &&
    ...
    if (net_profit_bps >= min_profit_bps_) {
      ...
      detection.severity =
          net_profit_bps > 100.0 ? Severity::HIGH : Severity::MEDIUM;
      detection.symbol = symbol;
      detection.exchange = best_ask_exchange + "-" + best_bid_exchange;
      detection.description =
          "Spread arbitrage opportunity: " + std::to_string(net_profit_bps) +
          "bps profit";
...
std::string SpreadArbitrageDetector::get_name() const { return PLUGIN_NAME; }
...
std::optional<BTQuant::DetectionSignal>
SpreadArbitrageDetector::detect(const std::string &symbol) {
  auto signals = detect_all(symbol);
  if (!signals.empty()) {
    // The detect_all method already returns DetectionSignal, so we can just
...
void SpreadArbitrageDetector::update_orderbook(const std::string &exchange,
...
void SpreadArbitrageDetector::configure(
    const std::unordered_map<std::string, BTQuant::Config::ConfigValue>
        &config) {
...
std::vector<std::string> SpreadArbitrageDetector::get_dependencies() const {
  return std::vector<std::string>(PLUGIN_DEPENDENCIES,
                                  PLUGIN_DEPENDENCIES +
                                      sizeof(PLUGIN_DEPENDENCIES) /
                                          sizeof(PLUGIN_DEPENDENCIES[0]));
}

void SpreadArbitrageDetector::set_dependency(const std::string &name,
...
#define BTQUANT_DETECTOR_PLUGIN_WITH_READER(DetectorClass)                     \
  ...
  std::unique_ptr<BTQuant::Detectors::IDetector> create_detector() {           \
    ...
    detector->set_dependency(                                                  \
        "HotSpineExtendedReader",                                              \
        BTQuant::Detectors::DetectorRegistry::instance().get_detector(         \
            "HotSpineExtendedReader"));                                        \
    return detector;                                                           \
  }                                                                            \
...
// Use the standard macro for now - detector will be configured with reader
// later
