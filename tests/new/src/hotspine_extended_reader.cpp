...
HotSpineExtendedReader::HotSpineExtendedReader(const std::string &shm_name)
  ...
  logger.info("HotSpineExtendedReader",
              "[HotSpineExtendedReader] Initialized with SHM: " + shm_name);
}
...
std::string
HotSpineExtendedReader::make_cache_key(const std::string &exchange,
...
std::optional<TradeData> HotSpineExtendedReader::poll_trade() {
  ...
  if (trades_read_ <= 5) {
    auto &logger = Logging::DynamicLogger::instance();
    // logger.info("HotSpineExtendedReader", "[HotSpineExtendedReader] Trade #"
    // + std::to_string(trades_read_) +
...
std::optional<double>
HotSpineExtendedReader::get_latest_price(const std::string &exchange,
...
HotSpineExtendedReader::get_latest_orderbook(const std::string &exchange,
                                             const std::string &symbol) {
  std::string key = make_cache_key(exchange, symbol);
...
std::vector<TradeData> HotSpineExtendedReader::get_recent_trades(
    const std::string &exchange, const std::string &symbol, size_t limit) {
  std::string key = make_cache_key(exchange, symbol);
...
HotSpineExtendedReader::get_all_exchange_prices(const std::string &symbol) {
...
HotSpineExtendedReader::get_all_exchange_orderbooks(const std::string &symbol) {
...
HotSpineExtendedReader::get_buffer_utilization() const {
  ...
  if (status.second > 0) {
    double usage_pct =
        (static_cast<double>(status.first) / status.second) * 100.0;
    // logger.info(
    //     "HotSpineExtendedReader",
    //     "[HotSpineExtendedReader] Buffer: " + std::to_string(status.first) +
...
uint64_t HotSpineExtendedReader::get_lost_count() const {
  ...
  if (status.second > 0 && status.first >= status.second * 0.95) {
    ...
    logger.warning(
        "HotSpineExtendedReader",
        "[HotSpineExtendedReader] Buffer near capacity - potential trade loss");
  }
