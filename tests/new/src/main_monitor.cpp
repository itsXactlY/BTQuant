...
void print_statistics(const StatsSnapshot& stats) {
    ...
    std::cout << "│ Buffer Usage:      " << std::setw(17) << std::fixed << std::setprecision(1) 
              << stats.buffer_usage << "% │\n";
...
void print_market_info(HotSpineExtendedReader& reader) {
    ...
    for (const auto& exchange : exchanges) {
        ...
        for (const auto& symbol_info : symbols) {
            ...
            auto orderbook = reader.get_latest_orderbook(exchange, symbol_info.symbol);
            ...
            if (orderbook) {
                std::cout << " │ Best Bid: $" << std::fixed << std::setprecision(2) << orderbook->best_bid()
                          << " │ Best Ask: $" << orderbook->best_ask()
                          << " │ Spread: " << std::fixed << std::setprecision(2) << orderbook->spread_bps() << " bps";
            }
...
int main(int argc, char* argv[]) {
    ...
    std::string shm_name = config.get_as<std::string>("shared_memory", "name").value_or("/btquant_hotspine");
    int poll_interval_ms = config.get_as<int64_t>("monitoring", "poll_interval_ms").value_or(100);
    int stats_interval_sec = config.get_as<int64_t>("monitoring", "stats_interval_sec").value_or(5);
    int market_info_interval_sec = config.get_as<int64_t>("monitoring", "market_info_interval_sec").value_or(60);
    ...
    try {
        ...
        int total_symbols = 0;
        for (const auto& exchange : exchanges) {
            auto exchange_symbols = SymbolRegistry::instance().get_exchange_symbols(exchange);
            std::cout << "   " << exchange << ": " << exchange_symbols.size() << " symbols\n";
        ...
        stop_hunt.set_threshold_pct(config.get_as<double>("stop_hunt", "threshold_pct").value_or(0.5));
        stop_hunt.set_min_exchanges(config.get_as<int64_t>("stop_hunt", "min_exchanges").value_or(3));
        
        LiquidityImbalanceDetector liquidity(reader);
        liquidity.set_depth_ratio_threshold(config.get_as<double>("liquidity_imbalance", "depth_ratio_threshold").value_or(3.0));
        
        WhaleFrontRunDetector whale(reader);
        whale.set_threshold_usd(config.get_as<double>("whale_frontrun", "threshold_usd").value_or(100000));
        
        SpreadArbitrageDetector arbitrage(reader);
        arbitrage.set_min_profit_bps(config.get_as<double>("spread_arbitrage", "min_profit_bps").value_or(50));
        arbitrage.set_fees(
            config.get_as<double>("spread_arbitrage", "maker_fee_bps").value_or(10),
            config.get_as<double>("spread_arbitrage", "taker_fee_bps").value_or(20)
        );
        
        SpoofingDetector spoofing(reader);
        spoofing.set_min_cancel_count(config.get_as<int64_t>("spoofing", "min_cancel_count").value_or(3));
        ...
        while (running) {
            ...
            if (now - last_market_info_print >= std::chrono::seconds(market_info_interval_sec)) {
            ...
            if (now - last_stats_print >= std::chrono::seconds(stats_interval_sec)) {
                StatsSnapshot current_stats = capture_stats(reader, stop_hunt, liquidity, whale, arbitrage, spoofing);
        ...
        // Final statistics
        std::cout << "\n📊 Final Statistics:\n";
        print_statistics(capture_stats(reader, stop_hunt, liquidity, whale, arbitrage, spoofing));
