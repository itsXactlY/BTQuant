...
TEST_F(TestFootprintPanelWithZoom, CellSizeAdjustmentAtDifferentZoomLevels) {
    ...
    double adjusted_padding_low = base_padding * std::pow(zoom_factor_low * 1.0, 1.5);
    ...
    double adjusted_padding_high = base_padding / std::pow(zoom_factor_high * 1.0, 0.7);
...
TEST_F(TestFootprintPanelWithZoom, CellSizeAdjustmentWithSensitivity) {
    ...
    double adjusted_padding_high_sens = base_padding / std::pow(zoom_factor * sensitivity_high, 0.7);
    adjusted_padding_high_sens = std::max(0.01, std::min(0.48, adjusted_padding_high_sens));
    
    // With low sensitivity (0.5)
    double sensitivity_low = 0.5;
    double adjusted_padding_low_sens = base_padding / std::pow(zoom_factor * sensitivity_low, 0.7);
    adjusted_padding_low_sens = std::max(0.01, std::min(0.48, adjusted_padding_low_sens));
    
    // Higher sensitivity should result in more dramatic cell size changes (smaller padding when zoomed in)
...
TEST_F(TestFootprintPanelWithZoom, CellSizeBoundsCheck) {
    ...
    double base_padding = 0.48;
    double adjusted_padding_extreme_low = base_padding * std::pow(zoom_factor_extreme_low * 1.0, 1.5);
    adjusted_padding_extreme_low = std::max(0.01, std::min(0.48, adjusted_padding_extreme_low));
    ...
    double adjusted_padding_extreme_high = base_padding / std::pow(zoom_factor_extreme_high * 1.0, 0.7);
    adjusted_padding_extreme_high = std::max(0.01, std::min(0.48, adjusted_padding_extreme_high));
...
TEST_F(TestFootprintPanelWithZoom, ZoomFactorEffectOnCellDimensions) {
    ...
    // When zoomed in, cells should appear larger (less padding means more visible area)
...
TEST_F(TestFootprintPanelWithZoom, RenderCellMethodAcceptsZoomFactor) {
    // Verify that the renderCell method accepts and uses the zoom factor parameter
    FootprintCell cell(1.0, 100.0, 0.1, 0.5, 150.0, 75.0, 10, 100.25);
    ...
    // This test verifies that the method signature is correct and can accept different zoom factors
    // The actual rendering is tested elsewhere, here we just verify the interface works
    EXPECT_NO_THROW({
        // Call the method with different zoom factors
        panel_->renderCell(cell, nullptr, 10000.0, empty_diagonal, empty_stacked, 0.5);
        panel_->renderCell(cell, nullptr, 10000.0, empty_diagonal, empty_stacked, 1.0);
        panel_->renderCell(cell, nullptr, 10000.0, empty_diagonal, empty_stacked, 2.0);
    });
}