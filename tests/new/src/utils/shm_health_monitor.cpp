...
uint64_t ShmUtils::get_file_mtime(const std::string &path) {
  ...
  return static_cast<uint64_t>(st.st_mtim.tv_sec) * 1000000ULL +
...
ShmHealthMonitor::ShmHealthMonitor(const std::string &shm_path,
                                   const MonitorConfig &config)
    : shm_path_(shm_path), config_(config), current_report_(HealthReport()) {

  Logging::DynamicLogger::instance().info(
      "SHM-MONITOR", "Initializing health monitor for: " + shm_path);
...
void ShmHealthMonitor::start_monitoring() {
  ...
  Logging::DynamicLogger::instance().info(
      "SHM-MONITOR", "Starting health monitoring - interval: " +
...
void ShmHealthMonitor::stop_monitoring() {
  ...
  Logging::DynamicLogger::instance().info("SHM-MONITOR",
...
void ShmHealthMonitor::monitor_loop() {
  Logging::DynamicLogger::instance().debug("SHM-MONITOR",
  ...
  while (monitoring_.load()) {
    ...
    if (report.status == HealthStatus::FAILED ||
      ...
      if (consecutive_failures_.load() >= config_.max_consecutive_failures) {
        Logging::DynamicLogger::instance().error(
            "SHM-MONITOR", "Consecutive failures threshold reached: " +
    ...
    std::this_thread::sleep_for(
        std::chrono::microseconds(config_.check_interval_us));
  }
...
ShmHealthMonitor::HealthReport ShmHealthMonitor::perform_full_check() {
  ...
  if (!report.file_exists) {
    ...
    Logging::DynamicLogger::instance().warning(
        "SHM-MONITOR", array_to_string(report.error_message));
    return report;
  }
  ...
  if (!report.permissions_ok) {
    ...
    std::string error_msg =
        "Insufficient permissions for shared memory: " + shm_path_;
    report.set_error_message(error_msg);
    Logging::DynamicLogger::instance().warning(
        "SHM-MONITOR", array_to_string(report.error_message));
    return report;
...
bool ShmHealthMonitor::check_header_integrity(size_t &capacity, size_t &used) {
  ...
  if (fd < 0) {
    Logging::DynamicLogger::instance().debug(
        "SHM-MONITOR",
        "Failed to open SHM for header check: " + std::string(strerror(errno)));
    return false;
  }
  ...
  if (magic != HotSpine::HOTSPINE_MAGIC) {
    Logging::DynamicLogger::instance().debug(
        "SHM-MONITOR", "Invalid magic number: 0x" + std::to_string(magic) +
                           " (expected 0x" +
                           std::to_string(HotSpine::HOTSPINE_MAGIC) + ")");
    return false;
  }
  ...
  if (version != EXPECTED_VERSION) {
    Logging::DynamicLogger::instance().debug(
        "SHM-MONITOR", "Invalid version: " + std::to_string(version));
    return false;
  }
