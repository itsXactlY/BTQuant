...
// This tests the core zoom functionality without requiring the full GUI
// framework

// Test the improved zoom algorithm functions directly
double calculateAdjustedPaddingImproved(double zoom_factor,
  ...
  if (zoom_factor >= 1.0) {
    ...
    double zoom_effect =
        std::pow(1.0 / (zoom_factor * zoom_sensitivity), 0.8) - 1.0;
...
TEST(ImprovedFootprintZoomAlgorithm, CellPaddingAdjustmentZoomedIn) {
  ...
  double padding_before =
      calculateAdjustedPaddingImproved(1.0, sensitivity); // Normal zoom
  double padding_after = calculateAdjustedPaddingImproved(
      zoom_factor_high, sensitivity); // Zoomed in
...
TEST(ImprovedFootprintZoomAlgorithm, CellPaddingAdjustmentZoomedOut) {
  ...
  double padding_before =
      calculateAdjustedPaddingImproved(1.0, sensitivity); // Normal zoom
  double padding_after = calculateAdjustedPaddingImproved(
      zoom_factor_low, sensitivity); // Zoomed out
...
TEST(ImprovedFootprintZoomAlgorithm, CellPaddingBoundsCheck) {
  ...
  double padding_extreme_in =
      calculateAdjustedPaddingImproved(zoom_factor_extreme_in, sensitivity);
...
TEST(ImprovedFootprintZoomAlgorithm, ImprovedZoomFactorEdgeCases) {
  ...
  double padding_small =
      calculateAdjustedPaddingImproved(zoom_factor_small, sensitivity);
...
TEST(ImprovedFootprintZoomAlgorithm, DifferentSensitivityEffects) {
  ...
  double result_low_sensitivity =
      calculateAdjustedPaddingImproved(zoom_factor, 0.5);
  double result_high_sensitivity =
      calculateAdjustedPaddingImproved(zoom_factor, 2.0);

  // Higher sensitivity should result in more dramatic size changes (lower
  // padding when zoomed in) When zoomed in, higher sensitivity should result in
  // smaller padding (larger cells)
...
TEST(ImprovedFootprintZoomAlgorithm, ComparisonWithOriginalAlgorithm) {
  ...
  auto calculateAdjustedPaddingOriginal =
      [](double zoom_factor, double zoom_sensitivity) -> double {
    double base_padding = 0.48;
    double adjusted_padding;
    ...
    if (zoom_factor >= 1.0) {
      ...
      // Use a logarithmic approach for smoother transitions at high zoom levels
      double zoom_effect =
          std::log10(zoom_factor * zoom_sensitivity + 1.0) * 0.5;
      adjusted_padding = std::max(0.01, base_padding - zoom_effect);
    } else {
      // When zoomed out: shrink cells to show more of them, approaching squares
      // Use an exponential approach to make cells shrink more aggressively when
      // zoomed out FIXED: The original algorithm was making cells larger when
      // zoomed out, which is wrong
      double zoom_effect =
          std::pow(1.0 / (zoom_factor * zoom_sensitivity), 1.2) - 1.0;
      // To make cells smaller when zoomed out, we need MORE padding, not less
      double padding_factor = 0.8 * (zoom_effect / (zoom_effect + 1.0));
      adjusted_padding =
          std::min(base_padding, base_padding * (1.0 + padding_factor));
    }
  ...
  double original_result =
      calculateAdjustedPaddingOriginal(zoom_in, sensitivity);
  double improved_result =
      calculateAdjustedPaddingImproved(zoom_in, sensitivity);

  // The improved algorithm should result in slightly different padding values
  // The improved algorithm uses 0.3 multiplier instead of 0.5 for zoom in
  // effect
  ...
  double zoom_out = 0.5;
  double original_result_out =
      calculateAdjustedPaddingOriginal(zoom_out, sensitivity);
  double improved_result_out =
      calculateAdjustedPaddingImproved(zoom_out, sensitivity);
...
TEST(ImprovedFootprintZoomAlgorithm, LevelOfDetailCalculations) {
  ...
  // The improved algorithm calculates cell dimensions in pixels for LOD
  // decisions
  ...
  // Calculate effective cell dimensions (these would be converted to pixels in
  // actual code)
  ...
  double cell_size_low_px =
      effective_width_low * 10.0; // Arbitrary scale factor
  double cell_size_high_px =
      effective_width_high * 10.0; // Arbitrary scale factor
  ...
  // The algorithm should allow showing labels at high zoom more often than at
  // low zoom Note: This depends on the arbitrary scale factor, so we just
  // verify the relationship
  EXPECT_TRUE(
      show_labels_high_zoom ||
