...
TEST_F(TestFootprintPanel, ProportionalSplitCalculationZeroVolumes) {
  ...
  double buy_proportion =
      (total_volume > 0.0) ? cell.bid_volume / total_volume : 0.5;
...
TEST_F(TestFootprintPanel, SplitVolumeRenderCellHandlesBothVolumes) {
  // Test that the renderCell method properly handles both buy and sell volumes
  // in split mode
  panel_->setVolumeDataType(Data::VolumeDataType::SplitVolume);
...
TEST_F(TestFootprintPanel, SplitVolumeRenderCellHandlesOnlyBuyVolume) {
  // Test that the renderCell method properly handles only buy volume in split
  // mode
  panel_->setVolumeDataType(Data::VolumeDataType::SplitVolume);
...
TEST_F(TestFootprintPanel, SplitVolumeRenderCellHandlesOnlySellVolume) {
  // Test that the renderCell method properly handles only sell volume in split
  // mode
  panel_->setVolumeDataType(Data::VolumeDataType::SplitVolume);
...
TEST_F(TestFootprintPanel, SplitVolumeDividerLineVisibilityBothVolumes) {
  ...
  EXPECT_NE(buy_proportion,
            0.5); // Should not be 50-50 since volumes are unequal
}
...
TEST_F(TestFootprintPanel, SplitVolumeDividerLinePositionOnlyOneVolume) {
  ...
  double buy_proportion1 =
      total_volume1 > 0.0 ? cell1.bid_volume / total_volume1 : 0.5; // 1.0
  ...
  double buy_proportion2 =
      total_volume2 > 0.0 ? cell2.bid_volume / total_volume2 : 0.5; // 0.0
